...
private:
  struct _node {
    ...
    _node() : left( nullptr ), right( nullptr ), parent( nullptr ), color( RED ) {}
    ~_node() {}
  };
  ...
  _node*   _insert_node(T_key key, T_value value) {
    ...
    while ( current != nullptr ) {
      parent  = current;
      current = _key_compare(key, current->key) ? current->left : current->right;
    }
    ...
    // head/tail setting
    if ( !_head || _key_compare(key, _head->key) ) {
      // if x < _head
      _head = x;
    }
    if ( !_tail || _key_compare(_tail->key, key) ) {
      // if x > _tail
      _tail = x;
    }
  ...
  void _delete_fixup(_node* x, _node* x_parent) {
    ...
    while ( x != _root && (!x || x->color == BLACK) ) {
      if ( x == x_parent->left ) {
        ...
        if ( w == nullptr ) {
          // dirty case. It made me crazy for 2 days...
          x        = x_parent;
...
public:
  ...
  public:
    iterator() : _current( nullptr ), _reverse( false ) {}
    ~iterator() {}
...
public:
  ...
  iterator upper_bound(T_key key) {
    _node* y = nullptr; // last node which is greater than the key
  ...
  iterator lower_bound(T_key key) {
    _node* y = nullptr; // last node which is not less than the key
    _node* x = _root;   // current node
  ...
  zz_tree() : _root( nullptr ), _head( nullptr ), _tail( nullptr ), _count( 0 ) { }
