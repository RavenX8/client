...
private:
  struct _slot {
    ...
    _slot() : key( nullptr ), prev( nullptr ), next( nullptr ), list_prev( nullptr ), list_next( nullptr ) {}

    ~_slot() {
      if ( key ) {
        // if allocated name exist, delete it.
  ...
  _slot* _find_slot(const char* key) {
    ...
    while ( found_slot ) {
      ...
      if ( strcmp( found_slot->key, key ) == 0 ) {
        // same key found
        return found_slot;
      }
...
template <class T>
typename zz_hash_table<T>::iterator zz_hash_table<T>::erase(iterator it) {
  ...
  if ( !strcmp( this_slot->key, _bucket[hash_code]->key ) ) {
    // if it is the head of the slot
