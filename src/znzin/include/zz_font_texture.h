...
protected:
  enum e_state {
    E_EMPTY = 1,
    // initial state. no data written. has empty string
    E_INSERT = 2,
    // created and will be inserted in update()
    E_RENDER = 4,
    // rendered, but will be erased in update()
    E_RERENDER = 8,
    // prepared in the texture and also will be valid in the next frame
  };
...
public:
  zz_font_texture() :
    line_height( 0 ), num_lines( 0 ), tex_width( 0 ), tex_height( 0 ) { }
