...
zz_particle_event_sequence::zz_particle_event_sequence() {
  ...
  m_texParticle    = nullptr;
  m_WorldParticles = nullptr;
  m_Particles      = nullptr;
  Reset();
}
...
zz_particle_event*   zz_particle_event_sequence::EventFactory(int iEventType) {
  zz_particle_event* pEvent = nullptr;

  switch ( iEventType ) {
    case EVENT_SIZE: pEvent = zz_new zz_particle_event_size();
      break;
    case EVENT_EVENTTIMER: pEvent = zz_new zz_particle_event_event_timer();
      break;
    case EVENT_REDCOLOR: pEvent = zz_new zz_particle_event_red_color();
      break;
    case EVENT_GREENCOLOR: pEvent = zz_new zz_particle_event_green_color();
      break;
    case EVENT_BLUECOLOR: pEvent = zz_new zz_particle_event_blue_color();
      break;
    case EVENT_ALPHA: pEvent = zz_new zz_particle_event_alpha();
      break;
    case EVENT_COLOR: pEvent = zz_new zz_particle_event_color();
      break;
    case EVENT_VELOCITYX: pEvent = zz_new zz_particle_event_velocityx();
      break;
    case EVENT_VELOCITYY: pEvent = zz_new zz_particle_event_velocityy();
      break;
    case EVENT_VELOCITYZ: pEvent = zz_new zz_particle_event_velocityz();
      break;
    case EVENT_VELOCITY: pEvent = zz_new zz_particle_event_velocity();
      break;
    case EVENT_TEXTURE: pEvent = zz_new zz_particle_event_texture();
      break;
    case EVENT_ROTATION: pEvent = zz_new zz_particle_event_rotation();
      break;
  }
...
bool zz_particle_event_sequence::Render(LPDIRECT3DVERTEXBUFFER9 pVB, int iVBSize) {
  ...
  if ( this->m_iImplementType == 1 ) {
    ...
    switch ( m_iAlignType ) {
      case 0: cam->get_rotation_facing( rot );
        break;
      case 1: rot = mat3_id;
        break;
      case 2: cam->get_rotation_facing_axis( rot, 2 );
        break;
    }
  }
  ...
  for ( int q = 0; q < m_iNumParticles; q++ ) {
    ...
    if ( (m_Particles && m_Particles->IsAlive( q )) || (m_WorldParticles && m_WorldParticles->IsAlive( q )) ) {
      ...
      switch ( GetUpdateCoord() ) {
        case ZZ_UPDATE_COORD_WORLD: part_pos = part->m_vPos;
          break;
        ...
        break;
        case ZZ_UPDATE_COORD_LOCAL_WORLD: part_pos = part->m_vPos + world_tm->get_position();
          break;
      }
