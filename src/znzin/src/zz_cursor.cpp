...
zz_cursor* zz_cursor::current = nullptr;
...
HRESULT zz_cursor::CreateCursorSurface(LPDIRECT3DDEVICE9 pd3dDevice, LPDIRECT3DSURFACE9& pCursorSurface, HCURSOR hCursor,
  ...
  D3DLOCKED_RECT lr;
  pCursorSurface->LockRect( &lr, nullptr, 0 );
...
LPDIRECT3DDEVICE9 zz_cursor::get_device() {
  if ( !znzin ) return nullptr;
  if ( !znzin->renderer ) return nullptr;
  if ( !znzin->renderer->is_a( ZZ_RUNTIME_TYPE(zz_renderer_d3d) ) ) return nullptr;
