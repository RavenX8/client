...
zz_sfx_ocean* sfx_ocean = nullptr;
...
zz_sfx_ocean::zz_sfx_ocean() : time_interval( ZZ_MSEC_TO_TIME(100) ), texture_index( -1 ) {
  ...
  for ( int i = 0; i < 25; ++i ) {
    ...
    tex = static_cast<zz_texture*>(znzin->textures->spawn( nullptr, ZZ_RUNTIME_TYPE(zz_texture), false /* not to autoload */ ));
...
bool zz_manager_sfx::push_sfx(e_type type_in) {
  switch ( type_in ) {
    case OCEAN: if ( !sfx_ocean ) return false;
...
void zz_screen_sfx::pre_make_texture() {
  ...
  d3d_device->CreateTexture( screen_texture_width, screen_texture_height, 1,D3DUSAGE_RENDERTARGET, render_format, D3DPOOL_DEFAULT, &screen_texture, nullptr );
...
void zz_screen_sfx::make_tiles1() {
  ...
  for ( i      = 0; i < num_current_tile; i += 1 ) {
    ...
    d3d_device->CreateVertexBuffer( 6 * sizeof( VERTEX_TRAIL ),D3DUSAGE_WRITEONLY, D3DFVF_XYZ | D3DFVF_TEX1,
                                    D3DPOOL_MANAGED, &m_tiles[i]->m_pVBTile, nullptr );
    ...
    vec[0].position.x = -1.0f;
    vec[0].position.y = 1.0f;
    vec[0].position.z = z_depth;
    vec[0].uv.x       = center_x - length_x;
    vec[0].uv.y       = center_y - length_y;
    vec[1].position.x = -1.0f;
    vec[1].position.y = -1.0f;
    vec[1].position.z = z_depth;
    vec[1].uv.x       = center_x - length_x;
    vec[1].uv.y       = center_y + length_y;
    vec[2].position.x = 1.0f;
    vec[2].position.y = -1.0f;
    vec[2].position.z = z_depth;
    vec[2].uv.x       = center_x + length_x;
    vec[2].uv.y       = center_y + length_y;

    vec[3].position.x = -1.0f;
    vec[3].position.y = 1.0f;
    vec[3].position.z = z_depth;
    vec[3].uv.x       = center_x - length_x;
    vec[3].uv.y       = center_y - length_y;
    vec[4].position.x = 1.0f;
    vec[4].position.y = -1.0f;
    vec[4].position.z = z_depth;
    vec[4].uv.x       = center_x + length_x;
    vec[4].uv.y       = center_y + length_y;
    vec[5].position.x = 1.0f;
    vec[5].position.y = 1.0f;
    vec[5].position.z = z_depth;
    vec[5].uv.x       = center_x + length_x;
    vec[5].uv.y       = center_y - length_y;
...
void      zz_screen_sfx::make_tiles4() {
  ...
  vec[0].pos.x   = -region[0];
  vec[0].pos.y   = region[1];
  vec[0].pos.z   = z_depth;
  vec[0].diffuse = D3DCOLOR_COLORVALUE(1.0f,1.0f,1.0f,1.0f);
  vec[1].pos.x   = -region[0];
  vec[1].pos.y   = -region[1];
  vec[1].pos.z   = z_depth;
  vec[1].diffuse = D3DCOLOR_COLORVALUE(1.0f,1.0f,1.0f,1.0f);
  vec[2].pos.x   = region[0];
  vec[2].pos.y   = -region[1];
  vec[2].pos.z   = z_depth;
  vec[2].diffuse = D3DCOLOR_COLORVALUE(1.0f,1.0f,1.0f,1.0f);
  vec[3].pos.x   = -region[0];
  vec[3].pos.y   = region[1];
  vec[3].pos.z   = z_depth;
  vec[3].diffuse = D3DCOLOR_COLORVALUE(1.0f,1.0f,1.0f,1.0f);
  vec[4].pos.x   = region[0];
  vec[4].pos.y   = -region[1];
  vec[4].pos.z   = z_depth;
  vec[4].diffuse = D3DCOLOR_COLORVALUE(1.0f,1.0f,1.0f,1.0f);
  vec[5].pos.x   = region[0];
  vec[5].pos.y   = region[1];
  vec[5].pos.z   = z_depth;
  vec[5].diffuse = D3DCOLOR_COLORVALUE(1.0f,1.0f,1.0f,1.0f);
...
void zz_screen_sfx::update_screen3() {
  ...
  for ( int i = 0; i < num_current_tile; i += 1 ) {
    ...
    switch ( i ) {
      ...
      case 0: D3DXMatrixTranslation( &t_m, -region[0] * (1.0f - time), region[1] * (1.0f - time), 0.0f );
        break;

      case 1: D3DXMatrixTranslation( &t_m, region[0] * (1.0f - time), region[1] * (1.0f - time), 0.0f );
        break;

      case 2: D3DXMatrixTranslation( &t_m, -region[0] * (1.0f - time), -region[1] * (1.0f - time), 0.0f );
        break;

      case 3: D3DXMatrixTranslation( &t_m, region[0] * (1.0f - time), -region[1] * (1.0f - time), 0.0f );
        break;
    }
...
void zz_screen_sfx::post_render() {
  ...
  if ( play_onoff ) {
    ...
    switch ( state_number ) {
      case 1: update_screen();
        break;

      case 2: update_screen2();
        break;

      case 3: update_screen3();
        break;
...
void    zz_camera_sfx::Init_observer_Camera(float xPos, float yPos, float zPos) {
  ...
  base_dir.x = -inv_m._13;
  base_dir.y = -inv_m._23;
  base_dir.z = 0.0f;
...
void   zz_camera_sfx::get_camera_sfx_matrix(vec3& t) {
  ...
  m1._11 = -sinf( phi * angle_ratio );
  m1._21 = -cosf( seta * angle_ratio ) * cosf( phi * angle_ratio );
  m1._31 = sinf( seta * angle_ratio ) * cosf( phi * angle_ratio );
  m1._41 = 0;
  m1._12 = cosf( phi * angle_ratio );
  m1._22 = -cosf( seta * angle_ratio ) * sinf( phi * angle_ratio );
  m1._32 = sinf( seta * angle_ratio ) * sinf( phi * angle_ratio );
  m1._42 = 0;
  m1._13 = 0;
  m1._23 = sinf( seta * angle_ratio );
  m1._33 = cosf( seta * angle_ratio );
  m1._43 = 0;
  m1._14 = 0;
  m1._24 = 0;
  m1._34 = -length;
...
void zz_camera_sfx::update_time(float* xPos, float* yPos) {
  ...
  m2._11 = cosf( base_angle );
  m2._21 = sinf( base_angle );
  m2._31 = 0.0f;
  m2._41 = 0.0f;
  m2._12 = -sinf( base_angle );
  m2._22 = cosf( base_angle );
  m2._32 = 0.0f;
  m2._42 = 0.0f;
  m2._13 = 0.0f;
  m2._23 = 0.0f;
...
void zz_sprite_sfx::draw_sprite() {
  ...
  if ( play_onoff && !pause_onoff ) {
    ...
    } else if ( accumulate_time < fade_t2 ) { } else {
...
void zz_avatar_selection_sfx::calculate_avatar_view_matrix() {
  ...
  */
  inv_avatar_view_m._11 = -sinf( view_seta );
  inv_avatar_view_m._12 = cosf( view_seta );
  inv_avatar_view_m._13 = 0;
  inv_avatar_view_m._14 = 0;
  inv_avatar_view_m._21 = 0.0f;
  inv_avatar_view_m._22 = 0.0f;
  inv_avatar_view_m._23 = 1.0f;
  inv_avatar_view_m._24 = 0;
  inv_avatar_view_m._31 = cosf( view_seta );
  inv_avatar_view_m._32 = sinf( view_seta );
  inv_avatar_view_m._33 = 0.0f;
  inv_avatar_view_m._34 = 0;
  inv_avatar_view_m._41 = view_length * cosf( view_seta );
  inv_avatar_view_m._42 = view_length * sinf( view_seta );
  inv_avatar_view_m._43 = view_height;
  inv_avatar_view_m._44 = 1;
  D3DXMatrixInverse( &avatar_view_m, nullptr, &inv_avatar_view_m );
}
...
void           zz_avatar_selection_sfx::draw_background() {
  ...
  vec[0].position.x = -max_length[0];
  vec[0].position.y = max_length[1];
  vec[0].position.z = -z_length;
  vec[0].uv.x       = 0.0f;
  vec[0].uv.y       = 0.0f;
  vec[1].position.x = -max_length[0];
  vec[1].position.y = -max_length[1];
  vec[1].position.z = -z_length;
  vec[1].uv.x       = 0.0f;
  vec[1].uv.y       = 1.0f;
  vec[2].position.x = max_length[0];
  vec[2].position.y = -max_length[1];
  vec[2].position.z = -z_length;
  vec[2].uv.x       = 1.0f;
  vec[2].uv.y       = 1.0f;

  vec[3].position.x = -max_length[0];
  vec[3].position.y = max_length[1];
  vec[3].position.z = -z_length;
  vec[3].uv.x       = 0.0f;
  vec[3].uv.y       = 0.0f;
  vec[4].position.x = max_length[0];
  vec[4].position.y = -max_length[1];
  vec[4].position.z = -z_length;
  vec[4].uv.x       = 1.0f;
  vec[4].uv.y       = 1.0f;
  vec[5].position.x = max_length[0];
  vec[5].position.y = max_length[1];
  vec[5].position.z = -z_length;
  vec[5].uv.x       = 1.0f;
  vec[5].uv.y       = 0.0f;
...
void   zz_moving_camera_screen_sfx::Input_camera_matrix(const vec3& position, const quat& rotation) {
  ...
  camera_view_m._11 = inv_buffer_m._11;
  camera_view_m._12 = inv_buffer_m._21;
  camera_view_m._13 = inv_buffer_m._31;
  camera_view_m._14 = inv_buffer_m._41;
  camera_view_m._21 = inv_buffer_m._12;
  camera_view_m._22 = inv_buffer_m._22;
  camera_view_m._23 = inv_buffer_m._32;
  camera_view_m._24 = inv_buffer_m._42;
  camera_view_m._31 = inv_buffer_m._13;
  camera_view_m._32 = inv_buffer_m._23;
  camera_view_m._33 = inv_buffer_m._33;
  camera_view_m._34 = inv_buffer_m._43;
  camera_view_m._41 = inv_buffer_m._14;
  camera_view_m._42 = inv_buffer_m._24;
  camera_view_m._43 = inv_buffer_m._34;
  camera_view_m._44 = inv_buffer_m._44;
