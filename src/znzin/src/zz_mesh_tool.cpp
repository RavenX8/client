...
vec2   zz_mesh_tool::get_uv_by_type(int face_width, int ix, int iy, zz_uv_type uv_type) {
  ...
  switch ( uv_type ) {
    case ZZ_UV_LR: uv.x = 1.0f - ouv.x;
      uv.y              = ouv.y;
      break;
    case ZZ_UV_TB: uv.x = ouv.x;
      uv.y              = 1.0f - ouv.y;
      break;
    case ZZ_UV_LRTB: uv.x = 1.0f - ouv.x;
      uv.y                = 1.0f - ouv.y;
      break;
    case ZZ_UV_90CW: uv.x = ouv.y;
      uv.y                = 1.0f - ouv.x;
      break;
    case ZZ_UV_90CCW: uv.x = 1.0f - ouv.y;
      uv.y                 = ouv.x;
      break;
    default: uv = ouv;
  }
...
bool zz_mesh_tool::load_mesh_6(const char* file_name, zz_mesh* mesh, bool text_mode) {
  ...
  if ( strcmp( str_buf, ZZ_MESH_VERSION6 ) == 0 ) {
    version = 6;
  } else if ( strcmp( str_buf, ZZ_MESH_VERSION5 ) == 0 ) {
    version = 5;
  } else {
  ...
  if ( format.use_skin() ) {
    ...
    for ( i   = 0; i < num_verts; ++i ) {
      ...
      bone_index     = mesh->bone_indices[uixyzw.x];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
      bone_index     = mesh->bone_indices[uixyzw.y];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
      bone_index     = mesh->bone_indices[uixyzw.z];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
      bone_index     = mesh->bone_indices[uixyzw.w];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
...
bool zz_mesh_tool::load_mesh_8(const char* file_name, zz_mesh* mesh, bool text_mode) {
  ...
  if ( format.use_skin() ) {
    ...
    for ( i   = 0; i < num_verts; ++i ) {
      ...
      bone_index     = mesh->bone_indices[usxyzw.x];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
      bone_index     = mesh->bone_indices[usxyzw.y];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
      bone_index     = mesh->bone_indices[usxyzw.z];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
      bone_index     = mesh->bone_indices[usxyzw.w];
      max_bone_index = (bone_index > max_bone_index) ? bone_index : max_bone_index;
...
bool     zz_mesh_tool::save_mesh(const char* file_name, zz_mesh* mesh, bool text_mode) {
  ...
  if ( format.use_skin() ) {
    for ( i = 0; i < num_verts; ++i ) {
      ...
      mesh_file.write_uint16( nullptr, (uint16)mesh->get_blend_index( i ).x );
      mesh_file.write_uint16( nullptr, (uint16)mesh->get_blend_index( i ).y );
      mesh_file.write_uint16( nullptr, (uint16)mesh->get_blend_index( i ).z );
      mesh_file.write_uint16( nullptr, (uint16)mesh->get_blend_index( i ).w );
      mesh_file.write_endl();
    }
  }
  ...
  if ( format.get_num_mapchannel() > 0 ) {
    for ( i = 0; i < num_verts; ++i ) {
      ...
      mesh_file.write_float( nullptr, mesh->get_uv( i, 0 ).y );
      mesh_file.write_endl();
    }
  }
  ...
  if ( format.get_num_mapchannel() > 1 ) {
    for ( i = 0; i < num_verts; ++i ) {
      ...
      mesh_file.write_float( nullptr, mesh->get_uv( i, 1 ).y );
      mesh_file.write_endl();
    }
  }
  ...
  if ( format.get_num_mapchannel() > 2 ) {
    for ( i = 0; i < num_verts; ++i ) {
      ...
      mesh_file.write_float( nullptr, mesh->get_uv( i, 2 ).y );
      mesh_file.write_endl();
    }
  }
  ...
  if ( format.get_num_mapchannel() > 3 ) {
    for ( i = 0; i < num_verts; ++i ) {
      ...
      mesh_file.write_float( nullptr, mesh->get_uv( i, 3 ).y );
      mesh_file.write_endl();
    }
  }
  ...
  for ( i = 0; i < num_faces; i++ ) {
    ...
    mesh_file.write_uint16( nullptr, mesh->get_face( i ).y );
    mesh_file.write_uint16( nullptr, mesh->get_face( i ).z );
    mesh_file.write_endl();
  }
...
bool zz_mesh_tool::load_terrain_mesh(zz_mesh_terrain* mesh) {
  ...
  for ( iy   = 0; iy < vwidth; iy++ ) {
    for ( ix = 0; ix < vwidth; ix++ ) {
      ...
      if ( (mesh->orig_y_ - mesh->blocksize_) < 0 ) {
        // negative coordinates
...
bool zz_mesh_tool::load_ocean_mesh(zz_mesh_ocean* mesh) {
  ...
  pos.set( 0, mesh->height_, 0 );
  mesh->set_pos( 0, pos );
  pos.set( 0, 0, 0 );
  mesh->set_pos( 1, pos );
  pos.set( mesh->width_, mesh->height_, 0 );
  mesh->set_pos( 2, pos );
  pos.set( mesh->width_, 0, 0 );
  mesh->set_pos( 3, pos );

  uv.set( 0.f, 0.f );
  mesh->set_uv( 0, 0, uv );
  uv.set( 0.f, (float)mesh->repeat_vert_ );
  mesh->set_uv( 1, 0, uv );
  uv.set( (float)mesh->repeat_hori_, 0.f );
  mesh->set_uv( 2, 0, uv );
  uv.set( (float)mesh->repeat_hori_, (float)mesh->repeat_vert_ );
  mesh->set_uv( 3, 0, uv );
  ...
  indices.y = 1;
  indices.z = 2;
  mesh->set_face( 0, indices );
  indices.x = 3;
  indices.y = 2;
  indices.z = 1;
  mesh->set_face( 1, indices );
