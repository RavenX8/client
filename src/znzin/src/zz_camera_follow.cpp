...
bool   zz_camera_follow::update_lookmode(float follow_yaw_last) {
  ...
  if ( now_following_ ) {
    // if we started moving camera position.
    if ( target_diff > MIN_DISTANCE_THRESHOLD ) {
      // now moving
      move_camera = true; // should move
    } else {
      // we are already very close to each other
      now_following_ = false; // quit following mode
      move_camera    = true;  // but keep in camera moving state for now
    }
  } else if ( target_diff > MAX_DISTANCE_THRESHOLD ) {
    // we should start moving the camera position now.
    now_following_ = true;
    move_camera    = true;
  } else {
    // we do not need to move the camera position
    move_camera = false;
  }
  ...
  if ( move_camera ) {
    // if we should move camera and target
    vec3  displacement;
...
// set to follow mode
//--------------------------------------------------------------------------------
void zz_camera_follow::set_follow_mode(zz_follow_mode mode) {
  if ( mode == BACK_MODE ) {
...
bool zz_camera_follow::get_distance_by_contact_point(float& adjusted_distance) {
  ...
  */
  ...
  for ( int i = 0; i < num_vis; i++ ) {
    ...
    if ( sphere && (sphere->radius < max_seethru_sphere_radius) ) {
      // too small
      continue;
    }
    ...
    if ( false && !v->is_a( ZZ_RUNTIME_TYPE(zz_terrain_block) ) ) {
      // if we want to skip too close objects.
