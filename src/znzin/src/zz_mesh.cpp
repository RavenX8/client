...
bool zz_mesh::init_device_objects() {
  ...
  if ( vbuf_res && vbuf_res->is_init_device_objects() ) {
    // managed, systemmem, scratch
  ...
  if ( ibuf_res && ibuf_res->is_init_device_objects() ) {
    // managed, systemmem, scratch
...
bool zz_mesh::restore_device_objects() {
  ...
  if ( vbuf_res && vbuf_res->is_restore_device_objects() ) {
    // managed, systemmem, scratch
    if ( vbuf_res->to_create() ) {
      create_vertex_buffer();
    }
  ...
  if ( ibuf_res && ibuf_res->is_restore_device_objects() ) {
    // managed, systemmem, scratch
    if ( ibuf_res->to_create() ) {
      create_index_buffer();
    }
...
void     zz_mesh::print_strip_indices(uint16 input_num_verts, uint16 output_num_verts) {
  ...
  for ( i = 0; i < num_tri_faces; ++i ) {
    ...
    if ( (i % width) == 0 ) {
      // for each horizontal starting vertex
      if ( i != 0 ) {
