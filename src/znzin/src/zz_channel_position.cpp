...
void    zz_channel_position::get_by_time(zz_time time, int fps, void* data_pointer) {
  ...
  switch ( interp_style ) {
    case ZZ_INTERP_LINEAR: time_to_frame( time, start_frame, next_frame, ratio, (int)num_positions, fps );
      lerp( new_pos, ratio, positions[start_frame], positions[next_frame] );
      break;
    case ZZ_INTERP_CATMULLROM: time_to_frame( time, start_frame, next_frame, ratio, (int)num_positions, fps );
