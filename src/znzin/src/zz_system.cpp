...
zz_vfs_pkg_system* zz_system::pkg_system_        = nullptr;
zz_vfs_pkg_system* zz_system::pkg_system_worker_ = nullptr;
zz_vfs_thread*     zz_system::vfs_thread         = nullptr;
...
zz_system::zz_system() :
  ...
  use_time_weight( true ), use_fixed_framerate( false ), use_motion_interpolation( true ),
  camdist_square_for_motion_interpolation( 1000.0f * 1000.0f ),
  camera( nullptr ),
  camera_light( nullptr ),
  light( nullptr ),
  sky( nullptr ) {
...
zz_system::~zz_system() {
  ...
  zz_material::s_renderer    = nullptr;
  zz_material::s_renderstate = nullptr;

  // for garbage collecting
  if ( cameras ) {
    cameras->release_children();
    cameras = nullptr;
  }
  if ( materials ) {
    materials->release_children();
    materials = nullptr;
  }
  if ( meshes ) {
    meshes->release_children();
    meshes = nullptr;
  }
  if ( terrain_meshes ) {
    terrain_meshes->release_children();
    terrain_meshes = nullptr;
  }
  if ( rough_terrain_meshes ) {
    rough_terrain_meshes->release_children();
    rough_terrain_meshes = nullptr;
  }
  if ( ocean_meshes ) {
    ocean_meshes->release_children();
    ocean_meshes = nullptr;
  }
  if ( materials ) {
    materials->release_children();
    materials = nullptr;
  ...
  if ( lights ) {
    lights->release_children();
    lights = nullptr;
  }
  if ( shaders ) {
    shaders->release_children();
    shaders = nullptr;
  }
  if ( visibles ) {
    visibles->release_children();
    visibles = nullptr;
  }
  if ( motions ) {
    motions->release_children();
    motions = nullptr;
  } // motions must precedes channels
  if ( skeletons ) {
    skeletons->release_children();
    skeletons = nullptr;
  }
  if ( textures ) {
    textures->release_children();
    textures = nullptr;
  }
  if ( terrain_blocks ) {
    terrain_blocks->release_children();
    terrain_blocks = nullptr;
  }
  if ( rough_terrain_blocks ) {
    rough_terrain_blocks->release_children();
    rough_terrain_blocks = nullptr;
  }
  if ( ocean_blocks ) {
    ocean_blocks->release_children();
    ocean_blocks = nullptr;
  }
  if ( fonts ) {
    fonts->release_children();
    fonts = nullptr;
  }
  if ( cursors ) {
    cursors->release_children();
    cursors = nullptr;
  }
...
void             zz_system::sleep() {
  ...
  if ( last_sleep_time_msec > 0 ) {
    // sleep conserving max framerate
