...
char zz_path::cwd_[ZZ_MAX_STRING] = "";
  ...
  zz_slash_converter converted_path( pathname, false, 1 ); // to slash, to lower
...
bool          zz_path::extract_name_ext(const zz_string& fullpath, zz_string& name_and_extension_only) {
  ...
  if ( slash == nullptr ) {
    // whole path is name_and_extension_only
...
bool zz_path::is_same_path(const char* path1, const char* path2, int len1, int len2) {
  ...
  if ( len1 < len2 ) {
    // swap if len1 is less than len2
    // swap len1 with len2
    int temp = len2;
    len2     = len1;
    len1     = temp;
  ...
  for ( int i1 = 0; i1 < len1; ) {
    ...
    if ( !path2 ) {
      // if path2 is shorter than path1
