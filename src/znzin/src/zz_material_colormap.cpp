...
void zz_material_colormap::_set_target_glow(void) {
  ...
  switch ( state.glow_type ) {
    case ZZ_GLOW_NONE: return;
      break;
    case ZZ_GLOW_SIMPLE: _set_glow();
      break;
    case ZZ_GLOW_LIGHT: _set_glow_lit();
      break;
    case ZZ_GLOW_TEXTURE: _set_glow_texture();
      break;
    case ZZ_GLOW_TEXTURE_LIGHT: _set_glow_texture_lit();
      break;
    case ZZ_GLOW_ALPHA: _set_glow_alpha();
      break;
  }
...
int   zz_material_colormap::get_shader_format() const {
  ...
  if ( use_specular && get_texture( SPECULARMAP_SLOT ) ) {
    // specular ignores receive_fog
...
void  zz_material_colormap::bind_texture() {
  ...
  zz_texture *diffusemap = nullptr, *specularmap = nullptr, *lightmap = nullptr;

  diffusemap  = (shader_format & SHADER_FORMAT_FIRSTMAP) ? get_texture( DIFFUSEMAP_SLOT ) : nullptr;
  specularmap = (shader_format & SHADER_FORMAT_SPECULARMAP) ? get_texture( SPECULARMAP_SLOT ) : nullptr;
  lightmap    = (shader_format & SHADER_FORMAT_LIGHTMAP) ? get_texture( LIGHTMAP_SLOT ) : nullptr;

  bool has_shadowmap = (receive_shadow && (shader_format & SHADER_FORMAT_SHADOWMAP));

  state.texture_binding[DIFFUSEMAP_SLOT]  = (diffusemap != nullptr);
  state.texture_binding[SPECULARMAP_SLOT] = (specularmap != nullptr);
  state.texture_binding[LIGHTMAP_SLOT]    = (lightmap != nullptr);
...
bool zz_material_colormap::set(int pass) {
  ...
  switch ( s_renderer->get_render_where() ) {
    case ZZ_RW_SHADOWMAP: _set_target_shadowmap();
      return true;
    ...
    case ZZ_RW_GLOW:
      ...
      _set_target_glow();
      return true;
    case ZZ_RW_SSAO: s_renderer->enable_zbuffer( true );
