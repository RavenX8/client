...
public:
  ...
  /// 상태정보함수 	
  bool         IsFocus() { return (m_dwStatus & STATUS_FOCUS) ? true : false; }
  bool         IsActive() { return (m_dwStatus & STATUS_ACTIVE) ? true : false; }
  virtual bool IsVision() { return (m_dwStatus & STATUS_VISION) ? true : false; }
  bool         IsEnable() { return (m_dwStatus & STATUS_ENABLE) ? true : false; }

  POINT GetPosition() { return m_sPosition; }
  void  SetPosition(POINT pt) { m_sPosition = pt; }
  ...
  void SetPositionX(int x) { m_sPosition.x = x; }
  void SetPositionY(int y) { m_sPosition.y = y; }
