...
CTEditBox* CTEditBox::s_pFocusEdit = nullptr;
...
void CTEditBox::SetFocus(bool bFocus) {
  ...
  if ( bFocus ) {
    ...
    if ( HIMC old_HIMC = CTIme::GetInstance().GetOldHIMC() ) {
      ...
      CTIme::GetInstance().SetOldHIMC( nullptr );
    ...
    if ( s_pFocusEdit && s_pFocusEdit == this ) {
      s_pFocusEdit  = nullptr;
      HIMC old_HIMC = ImmAssociateContext( CTControlMgr::GetInstance()->GetWindowHandle(), nullptr );
  ...
  switch ( uiMsg ) {
    ...
    case WM_KEYDOWN: ///Enter와 Esc처리하자.
    {
      if ( IsFocus() ) {
        ...
        if ( wParam == VK_RETURN ) {
          switch ( CTControlMgr::GetInstance()->GetKeyboardInputType() ) {
            case CTControlMgr::INPUTTYPE_NORMAL: SetFocus( true );
              return m_iControlID;
            case CTControlMgr::INPUTTYPE_AUTOENTER: {
              if ( s_pFocusEdit == nullptr ) {
                SetFocus( true );
                return m_iControlID;
              }
            }
    ...
    break;
    case WM_CHAR: if ( GetAsyncKeyState( VK_CONTROL ) >= 0 && IsFocus() && OnChar( wParam, lParam ) ) {
        //_RPT0( _CRT_WARN,"CTEditBox::WM_CHAR\n");		
        return m_iControlID;
      }
      break;
    case WM_LBUTTONDOWN: if ( IsInside( ptMouse.x, ptMouse.y ) ) {
        SetFocus( true );
        m_bShowCursor = true;
        return m_iControlID;
      }
...
void CTEditBox::Draw() {
  if ( !IsVision() || m_pFontMgr == nullptr ) return;
  ...
  switch ( m_iTextAlign ) {
    case TES_ALIGN_RIGHT: TextAlign = DT_RIGHT;
      break;
    case TES_ALIGN_CENTER: TextAlign = DT_CENTER;
      break;
        ...
        for ( int i = 0; i < dwClauseLen; ++i ) ///속성에 따라서 Underline이 틀리게 해야한다.
        {
          switch ( compAttr[*iterClauseInfo] ) {
            case ATTR_TARGET_CONVERTED: strunderline.append( TIme.GetStr_ATTR_TARGET_CONVERTED() );
              break;
            case ATTR_CONVERTED: strunderline.append( TIme.GetStr_ATTR_CONVERTED() );
              break;
            case ATTR_INPUT: strunderline.append( TIme.GetStr_ATTR_INPUT() ); ///그냥 입력중
            default: break;
          }
...
void CTEditBox::SetTextColor(int iColor) {
  switch ( iColor ) {
    case 1: m_dwTextColor = D3DCOLOR_RGBA(0,0,0,255);
      break;
    case 0:
    default: m_dwTextColor = D3DCOLOR_RGBA(255,255,255,255);
      break;
  }
}
