...
void    CObjCHAR::Adj_MoveSPEED(WORD wSrvDIST, const D3DVECTOR& PosGOTO) {
  ...
  if ( 0 == fCurSpeed ) {
    // 속도가 0이라면, 현재 위치와 PosGOTO 가 동일해야 하나?
    m_fAdjustSPEED = 0;
  ...
  if ( iClientDIST <= wSrvDIST ) {
    /// 현재 속도를 그대로 유지해도 된다.
    /// 클라이언트가 먼저가서 기다리면 되기 때문.
    m_fAdjustSPEED = fCurSpeed;
    return;
  }
  // wSrvDIST < iClientDIST. 클라이언트에서의 거리가 서버에서의 거리보다 먼 경우, 속도 증가.
...
CObjCHAR::CObjCHAR() : m_ChangeActionMode( this ), m_EndurancePack( this ), m_ObjVibration( this, 200 ) {
  ...
  m_ppBoneEFFECT = nullptr;
  m_hR_TRAIL     = m_hL_TRAIL = NULL;

  for ( short nP          = 0; nP < MAX_BODY_PART; nP++ ) {
    m_phPartVIS[nP]       = nullptr;
    m_pppEFFECT[nP]       = nullptr;
...
/*override*/
D3DXVECTOR3& CObjCHAR::Get_CurPOS() {
...
/*override*/
void CObjCHAR::SetEffectedSkillFlag(bool bResult) {
  m_bProcEffectedSkill = bResult;
  ///Log_String( LOG_NORMAL, "SetEffectedSkillFlag[ %d ]\n", bResult );
}

/*override*/
bool CObjCHAR::bCanActionActiveSkill() {
...
void CObjCHAR::LoadTRAIL(CMODEL<CCharPART>* pCharPART, short nPartIDX, bool bLinkBONE, int iColorNO, int iDuration, int iBaseDummyIDX) {
  ...
  m_hTRAIL[nTrailIDX] = loadTrail( nullptr,                         // ZSTRING pName, 
...
void                 CObjCHAR::Del_EFFECT(short nPartIDX) {
    ...
    if ( m_pppEFFECT[nPartIDX] ) {
      for ( short nT = 0; nT < pCharPART->m_nDummyPointCNT; nT++ ) {
        ...
        if ( m_pppEFFECT[nPartIDX][nT] ) {
          ...
          m_pppEFFECT[nPartIDX][nT] = nullptr;
        }
        m_pppEFFECT[nPartIDX][nT] = nullptr;
      }
    }
  ...
  switch ( nPartIDX ) {
    ...
    case BODY_PART_WEAPON_L: this->UnloadTRAIL( nPartIDX );
      break;
  }
}
...
bool    CObjCHAR::LoadModelNODE(char* szName) {
  ...
  if ( !this->Set_CurMOTION( this->Get_MOTION( 0 ) ) ) {
    // default stop motion ..
...
bool CObjCHAR::CreateCHAR(char* szName, CCharMODEL* pMODEL, short nCharPartCNT, const D3DVECTOR& Position) {
  ...
  if ( pMODEL == nullptr || nCharPartCNT <= 0 ) return false;
      ...
      if ( g_pAVATAR ) {
        if ( g_pAVATAR->Get_INDEX() == pAtkOBJ->Get_INDEX() ) {
          switch ( this->Get_TYPE() ) {
            case OBJ_MOB: sprintf( g_MsgBuf, F_STR_SUCCESS_HUNT, this->Get_NAME() );
              break;

            case OBJ_AVATAR: sprintf( g_MsgBuf, F_STR_WIN_PVP, this->Get_NAME() );
              break;
          }
...
void CObjCHAR::Dead() {
  ...
  if ( this->Is_AVATAR() ) {
    ...
    if ( pAVT->IsPersonalStoreMode() ) {
      pAVT->SetPersonalStoreTitle( nullptr );
...
void  CObjCHAR::ProcEffectOfSkillInDamageOfSkill(int iSkillIDX, int iObjIDX, CObjCHAR* pChar, stEFFECT_OF_SKILL* pEffectOfSkill) {
  ...
  switch ( iSkillType ) {
      ...
      if ( this->IsA( OBJ_USER ) ) {
        for ( int i = 0; i < SKILL_INCREASE_ABILITY_CNT; i++ ) {
          if ( (0x01 << i) & pEffectOfSkill->EffectOfSkill.m_btSuccessBITS ) {
            ...
            switch ( iAbilityType ) {
              case AT_HP: g_pAVATAR->Add_HP( SKILL_INCREASE_ABILITY_VALUE( iSkillIDX, i ) );
                break;
              case AT_MP: g_pAVATAR->Add_MP( SKILL_INCREASE_ABILITY_VALUE( iSkillIDX, i ) );
                break;

              case AT_STAMINA: g_pAVATAR->AddCur_STAMINA( SKILL_INCREASE_ABILITY_VALUE( iSkillIDX, i ) );
                break;
    ...
    default: ApplyEffectOfSkill( iSkillIDX, iObjIDX, pChar, pEffectOfSkill );
      break;
  }
}
        ...
        if ( iStateIndex != 0 )
          ...
          switch ( SKILL_INCREASE_ABILITY( iSkillIDX, i ) ) {
            ...
            case AT_STAMINA:
              ...
              break;
            default: g_itMGR.AppendChatMsg( "몰르는거네 추가해라..", IT_MGR::CHAT_TYPE_SYSTEM );
              break;
          }
        }
      }
  ...
  if ( this->IsA( OBJ_AVATAR ) || this->IsA( OBJ_USER ) ) {
    ...
    if ( pAvt->GetUpdateMotionFlag() || pAvt->GetChangeWeaponR() || pAvt->GetChangeWeaponL() ) {
      ...
      switch ( pAvt->Get_COMMAND() ) {
        ...
        break;
        case CMD_DIE: break;
        case CMD_PICK_ITEM: break;
        case CMD_SKILL2SELF: break;
        case CMD_SKILL2OBJ: break;
        case CMD_SKILL2POS: break;
        case CMD_RUNAWAY: break;
      }
...
int CObjCHAR::Proc(void) {
  ...
  */
  ...
  //--------------------------------------------------------------------------------
  if ( m_pChangeVisibility != nullptr ) {
    /// if expired?
  ...
  */
  ...
  switch ( Get_COMMAND() ) {
    case CMD_DIE: return this->IsUSER();

    case CMD_SIT: return ProcCMD_SIT();

    case CMD_STOP: return ProcCMD_STOP();

    case CMD_MOVE: return ProcCMD_MOVE();

    case CMD_PICK_ITEM: return ProcCMD_PICK_ITEM();

    case CMD_ATTACK: return ProcCMD_ATTACK();

    case CMD_SKILL2SELF: return ProcCMD_Skill2SELF();

    case CMD_SKILL2POS: return ProcCMD_Skill2POSITION();

    case CMD_SKILL2OBJ: return ProcCMD_Skill2OBJECT();
  }
...
int CObjCHAR::ProcCMD_Skill2OBJECT_PET() {
  ...
  if ( pTarget ) {
    ...
    if ( sType == -1 ) {
      ...
      ///아바타::캐스팅 / 실제동작 적용...
    else { this->Do_SKILL( this->Get_TargetIDX(), pTarget ); }
...
void CObjCHAR::ReleaseRideUser(WORD irideIDX) {
  ...
  pTarget->m_pObjCART->GetParent()->m_pRideUser = nullptr;
  //펫 모드 설정 
  pTarget->SetPetType( -1 );
  pTarget->m_pObjCART   = nullptr;
  pTarget->m_IsRideUser = FALSE;
...
/*override*/
void           CObjCHAR::PushCommandStand() {
...
/*override*/
void           CObjCHAR::PushCommandMove(const D3DVECTOR& PosTO, BYTE btRunMODE) {
...
/*override*/
void           CObjCHAR::PushCommandMove(WORD wSrvDIST, const D3DVECTOR& PosTO, int iServerTarget) {
  CObjCommand* pCommand = m_CommandQueue.GetObjCommand( OBJECT_COMMAND_MOVE );
...
/*override*/
void           CObjCHAR::PushCommandAttack(int iServerTarget, WORD wSrvDIST, const D3DVECTOR& PosTO) {
...
/*override*/
void           CObjCHAR::PushCommandToggle(BYTE btTYPE) {
...
/*override*/
void           CObjCHAR::PushCommandSkill2Self(short nSkillIDX) {
...
/*override*/
void           CObjCHAR::PushCommandSkill2Obj(WORD wSrvDIST, const D3DVECTOR& PosTO, int iServerTarget, short nSkillIDX) {
...
/*override*/
void           CObjCHAR::PushCommandSkill2Pos(const D3DVECTOR& PosGOTO, short nSkillIDX) {
...
bool    CObjMOB::Create(short nCharIdx, const D3DVECTOR& Position, short nQuestIDX, bool bRunMODE) {
  ...
  if ( CreateCHAR( szName, pMODEL, pMODEL->GetPartCNT(), Position ) ) {
...
bool CObjAVT::Create(const D3DVECTOR& Position, BYTE btCharRACE) {
  ...
  if ( CreateCHAR( (char*)m_Name.c_str(), &m_CharMODEL, MAX_BODY_PART, charPos ) ) {
  ...
  for ( int i        = BODY_PART_WEAPON_R; i <= BODY_PART_WEAPON_L; i++ ) {
    ...
    if ( m_sPartItemIDX[iPartIdx].m_bHasSocket && m_sPartItemIDX[iPartIdx].m_nGEM_OP > 300 ) {
      ...
      if ( iEffectIDX ) {
        ...
        int iStartPointNO = 0;
        switch ( iPartIdx ) {
          case BODY_PART_WEAPON_R: iStartPointNO = WEAPON_GEMMING_POSITION( iItemIDX );
            break;

          case BODY_PART_WEAPON_L: iStartPointNO = SUBWPN_GEMMING_POSITION( iItemIDX );
            break;
        }
...
/*override*/
int CObjAVT::GetANI_Move() { return m_bRunMODE ? AVT_ANI_RUN : AVT_ANI_WALK; }
/*override*/
int CObjAVT::GetANI_Attack() { return (AVT_ANI_ATTACK + RANDOM(3)); }
/*override*/
int CObjAVT::GetANI_Die() {
  if ( m_bStopDead )
    return AVT_ANI_STOP1;
  return AVT_ANI_DIE;
}
...
/*override*/
int CObjAVT::GetANI_Hit() { return AVT_ANI_HIT; }
/*override*/
int CObjAVT::GetANI_Casting() { return SKILL_ANI_CASTING(m_nActiveSkillIDX); }
/*override*/
int CObjAVT::GetANI_CastingRepeat() { return SKILL_ANI_CASTING_REPEAT(m_nActiveSkillIDX); } /// 루프동작은 캐스팅동작으로 사용..
/*override*/
int CObjAVT::GetANI_Skill() { return SKILL_ANI_ACTION_TYPE(m_nActiveSkillIDX); }
/*override*/
int CObjAVT::GetANI_Sitting() { return AVT_ANI_SITTING; }
/*override*/
int CObjAVT::GetANI_Standing() { return AVT_ANI_STANDUP; }
/*override*/
int CObjAVT::GetANI_Sit() { return AVT_ANI_SIT; }
/*override*/
int CObjAVT::GetANI_Ride() { return (m_pObjCART) ? m_pObjCART->GetRideAniPos() : 0; }
/*override*/
int CObjAVT::GetANI_PickITEM() { return AVT_ANI_PICKITEM; }
...
void CObjAVT::SetPersonalStoreTitle(char* strTitle, int iPersonalStoreType) {
  if ( strTitle == nullptr ) {
    if ( m_phPartVIS[0] == nullptr )
      m_pCharMODEL->ClearRenderUnitParts();

    // loadVisible된것들 삭제.
    for ( short nP = 0; nP < MAX_BODY_PART; nP++ ) {
      if ( m_phPartVIS[nP] == nullptr )
...
void CGoddessMgr::Release_List(GODDESSSTR* pGDS) {
  ...
  pGDS->pGODModel = nullptr;
  pGDS->pSprModel = nullptr;
...
#define TIME_APPEAR_GODDESS			3000
  ...
  for ( GODLIST itr = m_list.begin(); itr != m_list.end(); ++itr ) {
    ...
    if ( (pCHAR == nullptr) || (gds->god_State == GOD_NONE) ) {
