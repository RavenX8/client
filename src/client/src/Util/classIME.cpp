...
bool      classIME::Create(HWND hWnd, short nMaxTextLen) {
  ...
  for ( int i     = 0; i < MAX_LISTCAND; i++ ) {
    m_hwndCand[i] = nullptr;
    m_CandList[i] = nullptr;
  }
...
bool    classIME::GetResultString() {
  ...
  if ( Enter() ) {
    if ( (len = ImmGetCompositionString( m_hIMC, GCS_RESULTSTR, nullptr, 0 )) > 0 ) {
      str     = new char[len + 1];
...
bool    classIME::GetCompString(long flag) {
  ...
  if ( (len = ImmGetCompositionString( m_hIMC, GCS_COMPSTR, nullptr, 0 )) > 0 ) {
    str     = new char[len + 1];
    ImmGetCompositionString( m_hIMC, GCS_COMPSTR, str, len );
    str[len] = 0;
    strAttr  = nullptr;
    if ( flag & GCS_COMPATTR ) {
      if ( (lenAttr = ImmGetCompositionString( m_hIMC, GCS_COMPATTR, nullptr, 0 )) > 0 ) {
...
long classIME::OnIMENotify(WPARAM dwCommand, LPARAM dwData) {
  ...
  switch ( dwCommand ) {
    case IMN_OPENCANDIDATE: return OpenCandidate( dwData );

    case IMN_CLOSECANDIDATE: return CloseCandidate( dwData );

    case IMN_CHANGECANDIDATE: return ChangeCandidate( dwData );
...
bool              classIME::OpenCandidate(long lParam) {
  LPCANDIDATELIST lpCandList = nullptr; // Storage for LP to candidate list.
  ...
  for ( int index = 0; index < MAX_LISTCAND; index++ ) {
    if ( lParam & (1 << index) ) {
      ...
      dwPreferNumPerPage = (!lpCandList->dwPageSize) ? DEFAULT_CAND_NUM_PER_PAGE : lpCandList->dwPageSize;
      ...
      if ( m_hwndCand[index] == nullptr ) {
        delete m_CandList[index];
        m_CandList[index] = nullptr;
        goto exit_opencand;
      }
...
bool    classIME::DisplayCandStrings(HWND hwnd, LPCANDIDATELIST lpCandList) {
  ...
  dwEndIndex = (dwEndIndex >= lpCandList->dwCount) ? lpCandList->dwCount : dwEndIndex;
  ...
  ExtTextOut( hdc, 0, rect.top, ETO_OPAQUE, &rect, nullptr, 0, nullptr );
...
int classIME::ProcIMEMessage(UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  switch ( msg ) {
    ...
    case WM_INPUTLANGCHANGE: // Ű���� ���̾ƿ��� �ٲ�� ��� IME�� �ʱ�ȭ      
      OnInputLangChange( wparam, lparam );
      return CIME_HANDLED;

    case WM_IME_CONTROL: OnIMEControl( wparam, lparam );
      return CIME_HANDLED;

    case WM_IME_COMPOSITIONFULL: OnIMECompositionFull( wparam, lparam );
      return CIME_HANDLED;
...
int classIME::OnChar(UINT nChar) {
  switch ( nChar & 0x0ff ) {
    case VK_ESCAPE: return CIME_CANCEL;

    case VK_BACK: if ( !m_TextCurCount ) break;
    ...
    case VK_RETURN: m_pText[m_TextCurCount] = NULL;
      // m_TextCurCount -= 1;
      return CIME_RETURN;
