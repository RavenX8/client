...
unsigned int __stdcall ThreadDSStream(void* lpParameter) {
  ...
  while ( !bDone ) {
    ...
    switch ( dwResult ) {
      case WAIT_TIMEOUT: while ( pStream->IsCurrentBlockPassed() ) {
        ...
        break;
      case WAIT_OBJECT_0 + 0: bDone = TRUE;
        break;
    }
  }
...
ADirectSoundSTREAM::ADirectSoundSTREAM() {
  m_hCheckTimeEvent    = ::CreateEvent( nullptr, FALSE, FALSE, nullptr );
  m_dwPlayTimePerBlock = 1000;
  m_pDataBlock         = nullptr;

  _Init();

  m_hNotifyThread = (HANDLE)_beginthreadex( nullptr, 0, ThreadDSStream, this, CREATE_SUSPENDED, &m_dwNotifyThreadID );
}
...
#else
  if ( m_hCheckTimeEvent != nullptr ) {
    CloseHandle( m_hCheckTimeEvent );
    m_hCheckTimeEvent = nullptr;
  }
...
HRESULT ADirectSoundSTREAM::CreateStreamBuffer(LPDIRECTSOUND pDirectSound, WAVEFORMATEX* pWFX, int iBufferingSec, short nNumPlayNotifications) {
  ...
  hResult = pDirectSound->CreateSoundBuffer( &dsbd, &m_pDSBuffer, nullptr );
  if ( hResult != DS_OK )
    return hResult;
  m_pDSBuffer->SetVolume( DSBVOLUME_MIN ); // 초기 묵음으로..

  m_dwPlayTimePerBlock        = 1000 * iBufferingSec / (m_wDataBlockCount * 1);
  m_pDataBlock                = new tagDataBlock[ m_wDataBlockCount ];
...
HRESULT   ADirectSoundSTREAM::PlayStreamBuffer(bool bLooped, long lVolume) {
  HRESULT hResult;
  VOID*   pbBuffer = nullptr;

  if ( nullptr == m_pDSBuffer )
    return E_FAIL;
...
void ADirectSoundSTREAM::StopStreamBuffer(void) {
  ...
  m_pDSBuffer->Unlock( pbBuffer, m_dwTotalBuffSize, nullptr, 0 );
}
...
HRESULT   ADirectSoundSTREAM::FillBuffer(void) {
  ...
  // Now unlock the buffer
  m_pDSBuffer->Unlock( pbBuffer, dwBufferLength, nullptr, 0 );
...
bool AOggSTREAM::OpenFile(char* szOGGFile, int iBufferingSec, int iBufferBlock) {
  ...
  if ( ov_open( m_fp, m_pOVF, nullptr, 0 ) ) {
    this->CloseFile();
    return false;
  }
...
bool AOggSTREAM::OpenSoundStreamFile(char* szOggFile, LPDIRECTSOUND pDirectSound, int iBufferingSec, short nNumPlayNotifications) {
  ...
  if ( nullptr == pInfo || 0 == pInfo->channels )
    return false;
