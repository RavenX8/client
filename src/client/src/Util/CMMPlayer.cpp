...
classMMPLAYER::classMMPLAYER() {
  ...
  m_pSourceCurrent = nullptr;
  m_pSourceNext    = nullptr;
}
...
HRESULT   classMMPLAYER::Play(LPSTR argName) {
  ...
  if ( SUCCEEDED(hr) ) {
    ...
    IBaseFilter*  pFilterTemp = nullptr;
    if ( SUCCEEDED(hr = m_pGraphBuilder->EnumFilters(&pFilterEnum)) ) {
      ...
      pFilterEnum->Reset();
      while ( S_OK == pFilterEnum->Next( 1, &(ppFilters[iPos++]), nullptr ) );
      ...
      for ( iPos = 0; iPos < iFiltCount; iPos++ ) {
        ...
        if ( ppFilters[iPos] != m_pSourceCurrent ) {
          m_pGraphBuilder->AddFilter( ppFilters[iPos], nullptr );
        }
