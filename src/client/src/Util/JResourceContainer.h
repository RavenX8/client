...
namespace JED
{
  ...
  public:
    ...
    virtual bool operator <(const CResObject& Obj) const {
  ...
  void CResourceContainer<T>::ClearPool() {
    std::for_each( m_pObjectPool.begin(), m_pObjectPool.end(), DeletePoolObject );
    std::for_each( m_pUsedPool.begin(), m_pUsedPool.end(), DeletePoolObject );
  ...
  bool         CResourceContainer<T>::AddObject(const char* strName, T* obj) {
    ...
    typename std::map<CResObject, T*>::iterator itor = m_ObjectMap.find( keyObj );
