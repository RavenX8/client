...
public :
  ...
  char  m_TextComp[ 3 ];
       ...
       classIME();
       ~classIME();
  void InitIme(HWND hWnd = nullptr, short nMaxTextLen = MAX_TEXT_LENGTH) { Create( hWnd, nMaxTextLen ); }
...
public :
  bool Create(HWND hWnd = nullptr, short nMaxTextLen = MAX_TEXT_LENGTH);
  void Clear();
