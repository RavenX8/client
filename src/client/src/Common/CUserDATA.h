...
struct tagGrowAbility {
  ...
  int32_t m_lEXP;        // 경험치 1~100000
  short   m_nLevel;      // 0~250
  int32_t   m_nBonusPoint; // 1~999
  int32_t   m_nSkillPoint; // 1~
  BYTE    m_btBodySIZE;  // 몸통크기
  BYTE    m_btHeadSIZE;  // 머리크기
  int32_t m_lPenalEXP;   // 추가 경험치...
  ...
  int32_t   m_iGuildNO;    // 길드 번호	: 2004.5.27 추가
...
///		m_Battle.m_nWEIGHT -= m_Inventory.GetWEIGHT (wListRealNO);
  ...
  virtual void UpdateModelData() = 0 { *(int*)nullptr = 10; } /// 모델 데이터 갱신..
#endif
