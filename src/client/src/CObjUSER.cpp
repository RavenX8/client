...
CObjUSER* CObjUSER::m_pInstance = nullptr;
...
/*override*/
int CObjUSER::Quest_GetZoneNO() {
  return g_pTerrain->GetZoneNO();
}

/*override*/
int   CObjUSER::Quest_DistanceFrom(int iX, int iY, int iZ) {
  int iDistance = CD3DUtil::distance( (int)m_PosCUR.x, (int)m_PosCUR.y, iX, iY );
  return iDistance;
}

/*override*/
int CObjUSER::Quest_PartyMemberCNT() {
  return CParty::GetInstance().GetMemberCount();
}

//-------------------------------------------------------------------------------------------------
/*override*/
int       CObjUSER::Get_BulletNO() {
  tagITEM sItem;
  sItem.m_cType   = ITEM_TYPE_WEAPON;
  ...
  if ( iShotType < MAX_SHOT_TYPE && m_Inventory.m_ItemSHOT[iShotType].GetQuantity() > 0 )
...
void      CObjUSER::SubActiveBulletQuantity() {
  ...
  if ( m_ShotData[iShotType].m_nItemNo == 0 ) {
...
void       CObjUSER::Set_ITEM(short nListRealNO, tagITEM& sITEM) {
  tagITEM* pItem = &(m_Inventory.m_ItemLIST[nListRealNO]);
  ...
  if ( !pItem->IsEmpty() && !sITEM.IsEmpty() && sITEM.IsEnableDupCNT() && pItem->IsEqual( sITEM.GetTYPE(), sITEM.GetItemNO() ) ) {
    m_Inventory.m_ItemLIST[nListRealNO] = sITEM;
...
void CObjUSER::SetCur_STAMINA(short nStamina) {
  m_GrowAbility.m_nSTAMINA = nStamina;
}
...
void CObjUSER::ChangeModelInfo(short nType, int iValue) {
  ...
  switch ( nType ) {
    ...
    break;
    case AT_HAIR: this->SetPartITEM( BODY_PART_HAIR, iValue );
      m_BasicINFO.m_cHairIDX = iValue;
      this->Update();
      break;
    case AT_FACE: this->SetPartITEM( BODY_PART_FACE, iValue );
...
short CObjUSER::GetPsv_ATKSPEED(float fCurSpeed, short nRightWeaponItemNo) {
  return this->GetPassiveSkillAttackSpeed( fCurSpeed, nRightWeaponItemNo ) + m_iAddValue[AT_ATK_SPD];
}
