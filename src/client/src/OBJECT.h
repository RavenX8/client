...
public :
  ...
  CGameOBJ* Get_ClientOBJECT(WORD wServerObjectIndex, t_ObjTAG ObjTYPE = OBJ_NULL) {
    return this->Get_OBJECT( this->Get_ClientObjectIndex( wServerObjectIndex ), ObjTYPE );
  }

  /// 서버 인덱스로 캐릭터 오브젝트를 얻는다.
  CObjCHAR* Get_ClientCharOBJ(WORD wServerObjectIndex, bool bCheckHP) {
  ...
  CObjAVT* Get_ClientCharAVT(WORD wServerObjectIndex, bool bCheckHP) {
    return this->Get_CharAVT( this->Get_ClientObjectIndex( wServerObjectIndex ), bCheckHP );
  }

  /// 서버 인덱스로 아이템 오브젝트를 얻는다.
  CObjITEM* Get_ClientItemOBJ(WORD wServerObjectIndex) {
