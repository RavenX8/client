  ...
  for ( i   = 0; i < OBJECT_COMMAND_MAX; i++ ) {
    for ( j = 0; j < 10; j++ ) {
      switch ( i ) {
        ...
        case OBJECT_COMMAND_Skill2SELF: pCommand = new CObjSkill2SelfCommand();
          break;
        case OBJECT_COMMAND_Skill2OBJ: pCommand = new CObjSkill2ObjCommand();
          break;
        case OBJECT_COMMAND_Skill2POS: pCommand = new CObjSkill2PosCommand();
          break;
      }
  ...
  for ( ; begin != end; ) {
    ...
    if ( (dwCurrentTime - pSendedCommand->iElapsedTime) > SKILL_DUPLICATED_DELAY ) {
      if ( pCommand ) {
        ...
        pSendedCommand->pCommand     = nullptr;
        pSendedCommand->iElapsedTime = 0;
        m_CommandStructPool.push_back( pSendedCommand );
      }
...
SendedCommand* CPreventDuplicatedCommand::GetSendedCommand() {
  ...
  SendedCommand* pSendedCommand = nullptr;
