...
D3DXCOLOR s_FogColor[ MAX_FOG_COLOR ] = {
...
D3DXCOLOR      CDayNNightProc::GetColorStringFromSTB(int iType, int iState) {
  ...
  switch ( iType ) {
    case BG_AMBIENT: iColor = SKY_AMBIENT( m_iCurrentSkyNO, iState );
      break;
    case CHAR_AMBIENT: iColor = SKY_CHAR_AMBIENT( m_iCurrentSkyNO, iState );
      break;
    case CHAR_DIFFUSE: iColor = SKY_CHAR_DIFFUSE( m_iCurrentSkyNO, iState );
      break;
  }
...
void    CDayNNightProc::Proc() {
  ...
  if ( dwTempTime > (DWORD)m_iDAYENVIR_TICK ) /// 10 초마다 1씩 증가.
  {
    ...
    if ( CheckUpdateEnvironment() ) {
      ...
      switch ( m_iCurrentState ) {
        case DN_DAY: Start( ZONE_BG_IMAGE( m_iZoneNO ), m_iCurrentState, ZONE_DAY_TIME( m_iZoneNO ) - ZONE_MORNING_TIME( m_iZoneNO ) );
          break;
        case DN_NIGHT: Start( ZONE_BG_IMAGE( m_iZoneNO ), m_iCurrentState, ZONE_NIGHT_TIME( m_iZoneNO ) - ZONE_EVENING_TIME( m_iZoneNO ) );
          break;
      }
    }
  }
