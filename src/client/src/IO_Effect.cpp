...
bool CParticle::RegisterToNZIN(char* szName, char* szParticleFile) {
  ...
  m_hVisible = loadVisible( nullptr, NULL, NULL, NULL );
  if ( !m_hVisible )
    return false;
...
bool CMeshEffect::RegisterToNZIN(HNODE hMesh, HNODE hMotion, HNODE hMaterial) {
  ...
  m_hAnimatable = loadAnimatable( nullptr, 0, 0, 0, 0 );
  if ( !m_hAnimatable ) {
    unloadVisible( m_hVisible );
    m_hVisible = NULL;
    return false;
  ...
  m_hMorpher = loadMorpher( nullptr, hMesh, hMotion, hMaterial, NULL ); // At this moment, MORPHER does not need to use g_GameDATA.m_hLight.
  if ( !m_hMorpher ) {
    unloadVisible( m_hVisible );
    m_hVisible = NULL;
...
bool CEffect::Create(CEffectDATA* pEffectDATA) {
  ...
  char* szName  = CStr::Printf( "EFT_%s_%d", (pEffectDATA->m_Name.Get() == nullptr) ? "NULL" : pEffectDATA->m_Name.Get(), m_iEffectCnt );
  char* szName2 = CStr::Printf( "EFT_%s_%d_R2", (pEffectDATA->m_Name.Get() == nullptr) ? "NULL" : pEffectDATA->m_Name.Get(), m_iEffectCnt );
  ...
  for ( iC    = 0; iC < m_pEffectDATA->m_iParticleCNT; iC++ ) {
    if ( !m_pParticle[iC].RegisterToNZIN( nullptr, m_pEffectDATA->m_pParticle[iC].m_FileName.Get() ) )
      continue;
...
CEffectDATA* CEffectLIST::Get_EffectDATA(t_HASHKEY uiHashKEY) {
  ...
  return pHashNode ? pHashNode->m_DATA : nullptr;
}
...
CEffect* CEffectLIST::Add_EFFECT(t_HASHKEY uiHashKEY, bool bAdd2LIST) {
  ...
  CEffectDATA* pEffectDATA = pHashNode ? pHashNode->m_DATA : nullptr;
  if ( nullptr == pEffectDATA )
    return nullptr;
...
void                      CEffectLIST::Proc() {
  ...
  while ( pNode ) {
    if ( pNode->DATA->IsFinish() ) {
      ...
      /// 부모가 있다면 부모의 레퍼런스 갱신
      if ( pDelNode->DATA->GetParentCHAR() != nullptr )
