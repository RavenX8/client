...
CUpgrade::CUpgrade(void) {
  ...
  for ( int i                  = 0; i < 3; ++i ) {
    m_pMaterialItems[i]        = nullptr;
    m_pRequireMaterialItems[i] = nullptr; /// 제련에 요구되는 아이템리스트
  }
...
void       CUpgrade::SetTargetItem(CItem* pItem) {
  ...
  if ( iMaterialLineNo >= 0 && iMaterialLineNo <= g_TblPRODUCT.m_nDataCnt ) {
    CRequireMaterial* pRequireItem = nullptr;
  ...
  for ( int iIndex   = 1; iIndex < 3; ++iIndex ) {
    ...
    if ( pRequireMaterial ) {
      if ( pItem->GetType() == pRequireMaterial->GetItemType() &&
           ...
           m_pMaterialItems[iIndex] == nullptr &&
           pItem->GetQuantity() >= pRequireMaterial->GetRequireCount() ) {
        CItemFragment* pFragmentItem = new CItemFragment( pItem );
...
void CUpgrade::RemoveTargetItem() {
  if ( m_pTargetItem == nullptr ) return;
...
void CUpgrade::Update(CObservable* pObservable, CTObject* pObj) {
  assert( pObservable );
  assert( pObj );
  if ( pObj == nullptr || strcmp( pObj->toString(), "CTEventItem" ) ) {
...
int CUpgrade::IsValidUpgradeCondition() {
  if ( m_pTargetItem == nullptr ) return INVALID_TARGETITEM;
  if ( !m_pTargetItem->GetItem().IsEnableUpgrade() ) return INVALID_TARGETITEM;
  switch ( GetType() ) {
    case TYPE_SKILL: if ( g_pAVATAR->Get_MP() < m_iRequireMp )
        return NOTENOUGH_MP;
      break;
    case TYPE_NPC: if ( g_pAVATAR->Get_MONEY() < m_iRequireMoney )
  ...
  for ( int iIndex = 0; iIndex < 3; ++iIndex ) {
    if ( m_pRequireMaterialItems[iIndex] ) {
      if ( m_pMaterialItems[iIndex] == nullptr )
...
bool CUpgrade::SendPacketUpgrade() {
  ...
  if ( int iError = IsValidUpgradeCondition() ) {
    switch ( iError ) {
      case NOTENOUGH_MP: g_itMGR.OpenMsgBox( STR_NOT_ENOUGH_MANA );
        break;
...
int CUpgrade::GetType() {
  if ( m_iNpcSvrIdx && m_iSkillSlotIdx == 0 && m_iUseItemInvenIdx == 0 )
    return TYPE_NPC;
  if ( m_iNpcSvrIdx == 0 && m_iSkillSlotIdx && m_iUseItemInvenIdx == 0 )
    return TYPE_SKILL;
