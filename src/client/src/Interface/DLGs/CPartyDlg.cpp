...
void CPartyDlg::OnLButtonUp(unsigned iProcID) {
  ...
  switch ( iProcID ) {
    case IID_BTN_OPTION: if ( CTDialog* pDlg = g_itMGR.FindDlg( DLG_TYPE_PARTYOPTION ) ) {
    ...
    case IID_BTN_ENTRUST: {
      if ( CPartyMember* pMember = GetSelectedMember() ) {
        ...
        g_itMGR.OpenMsgBox( g_MsgBuf, CMsgBox::BT_OK | CMsgBox::BT_CANCEL, true, 0, pCmd, nullptr );
    ...
    case IID_BTN_LEAVE: {
      ...
      g_itMGR.OpenMsgBox( STR_PARTY_QUERY_LEAVE, CMsgBox::BT_OK | CMsgBox::BT_CANCEL, true, 0, pMacroCmd, nullptr );
    }
      break;
    case IID_BTN_BAN: {
      if ( CPartyMember* pMember = GetSelectedMember() ) {
...
void CPartyDlg::Update(CObservable* pObservable, CTObject* pObj) {
  ...
  switch ( pEvent->GetID() ) {
    ...
    case CTEventParty::EID_INIT: ClearMemberList();
      Show();
      break;
    default: break;
  }
