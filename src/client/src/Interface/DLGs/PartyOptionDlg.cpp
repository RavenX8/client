...
unsigned CPartyOptionDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if ( unsigned uiProcID = CTDialog::Process( uiMsg, wParam, lParam ) ) {
    switch ( uiMsg ) {
      case WM_LBUTTONUP: OnLButtonUp( uiProcID );
        break;
      default: break;
    }
    return uiMsg;
...
void CPartyOptionDlg::OnLButtonUp(unsigned uiProcID) {
  switch ( uiProcID ) {
    ...
    case IID_BTN_CONFIRM: {
      ...
      switch ( uiPressedBtn ) {
        case IID_RADIOBUTTON_ITEM_PICK: break;
        case IID_RADIOBUTTON_ITEM_SEQUENCE: btPartyRule |= BIT_PARTY_RULE_ITEM_TO_ORDER;
          break;
        default:
      ...
      switch ( uiPressedBtn ) {
        case IID_RADIOBOX_EXP_EQUALITY: break;
        case IID_RADIOBOX_EXP_RATIO_LEVEL: btPartyRule |= BIT_PARTY_RULE_EXP_PER_PLAYER;
          break;
        default:
          assert( 0 && "Invalid Selected RadioButton" );
          break;
