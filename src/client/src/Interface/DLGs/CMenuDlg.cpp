...
unsigned        CMenuDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
  if ( unsigned uiProcID = CTDialog::Process( uiMsg, wParam, lParam ) ) {
    switch ( uiMsg ) {
      case WM_LBUTTONUP: OnLButtonUp( uiProcID, wParam, lParam );
        break;
      case WM_LBUTTONDOWN: OnLButtonDown( uiProcID, wParam, lParam );
        break;
      default: break;
    }
    return uiProcID;
...
void CMenuDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  ...
  switch ( uiProcID ) {
    case IID_BTN_CHARACTER: g_itMGR.OpenDialog( DLG_TYPE_CHAR );
#ifdef _NEWUI
#else
      Hide();
#endif
      break;
    case IID_BTN_ITEM: g_itMGR.OpenDialog( DLG_TYPE_ITEM );
#ifdef _NEWUI
#else
      Hide();
#endif
      break;
    case IID_BTN_SKILL: g_itMGR.OpenDialog( DLG_TYPE_SKILL );
#ifdef _NEWUI
#else
      Hide();
#endif
      break;
    case IID_BTN_QUEST: g_itMGR.OpenDialog( DLG_TYPE_QUEST );
#ifdef _NEWUI
#else
      Hide();
#endif
      break;
    case IID_BTN_CUMMUNITY: g_itMGR.OpenDialog( DLG_TYPE_COMMUNITY );
#ifdef _NEWUI
#else
      Hide();
#endif
      break;
    case IID_BTN_GUILD: g_itMGR.OpenDialog( DLG_TYPE_CLAN );
#ifdef _NEWUI
#else
      Hide();
#endif
      break;
    case IID_BTN_HELP: g_itMGR.OpenDialog( DLG_TYPE_HELP );
#ifdef _NEWUI
#else
      Hide();
#endif
...
#endif
      break;
    case IID_BTN_OPTION: g_itMGR.OpenDialog( DLG_TYPE_OPTION );
#ifdef _NEWUI
#else
      Hide();
#endif
      break;
    case IID_BTN_EXIT: g_itMGR.OpenDialog( DLG_TYPE_SYSTEM );
#ifdef _NEWUI
#else
      Hide();
#endif
...
void CMenuDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  switch ( uiProcID ) {
    ...
    case IID_BTN_HELP:
    case IID_BTN_OPTION:
    case IID_BTN_GUILD: m_uiClickedButtonID = uiProcID;
  ...
  if ( m_uiClickedButtonID ) {
    ...
    if ( abs( m_ptClickedButton.x - ptMouse.x ) >= g_pCApp->GetWIDTH() / 100 ||
      ...
      switch ( m_uiClickedButtonID ) {
        case IID_BTN_GUILD: iDialogType = DLG_TYPE_CLAN;
          break;
        case IID_BTN_CHARACTER: iDialogType = DLG_TYPE_CHAR;
          break;
        case IID_BTN_ITEM: iDialogType = DLG_TYPE_ITEM;
          break;
        case IID_BTN_SKILL: iDialogType = DLG_TYPE_SKILL;
          break;
        case IID_BTN_QUEST: iDialogType = DLG_TYPE_QUEST;
          break;
        case IID_BTN_CUMMUNITY: iDialogType = DLG_TYPE_COMMUNITY;
          break;
        case IID_BTN_HELP: iDialogType = DLG_TYPE_HELP;
          break;
        case IID_BTN_OPTION: iDialogType = DLG_TYPE_OPTION;
          break;
        default: break;
      }
      ...
      if ( iDialogType ) {
        ...
        if ( g_itMGR.FindDialogIcon( iDialogType ) == nullptr ) {
...
void CMenuDlg::SetOpen(bool b) {
  ...
  CWinCtrl* ctrl = nullptr;
  ctrl           = Find( "CLOSE_BTN" );
  if ( ctrl ) {
    if ( b ) { ctrl->Show(); } else { ctrl->Hide(); }

  }
  ctrl = Find( "OPEN_BTN" );
  if ( ctrl ) {
    if ( b ) { ctrl->Hide(); } else { ctrl->Show(); }
  }
