...
#include "../TypeResource.h"
  ...
  m_pDragItem->AddTarget( iType, nullptr );
...
void CSeparateDlg::Update(CObservable* pObservable, CTObject* pObj) {
  ...
  if ( pObj == nullptr || strcmp( pObj->toString(), "Separate" ) ) return;

  CTEventSeparate* pEvent = (CTEventSeparate*)pObj;
  switch ( pEvent->GetID() ) {
    case CTEventSeparate::EID_REMOVE_MATERIAL_ITEM: m_MaterialItemSlot.DetachIcon();
      break;
    ...
    case CTEventSeparate::EID_SET_OUTPUT_ITEM: {
      ...
      if ( iIndex >= 0 && iIndex < (int)m_OutputItemSlots.size() ) {
        ...
        if ( pItem && m_OutputItemSlots[iIndex].GetIcon() == nullptr )
    ...
    if ( uiMsg == WM_LBUTTONUP ) {
      switch ( uiProcID ) {
        case IID_BTN_START: {
          ...
          if ( pIcon == nullptr ) ///재료가 없다면 Disable
          {
...
void CSeparateDlg::Show() {
  ...
  switch ( CSeparate::GetInstance().GetType() ) {
    case CSeparate::TYPE_SKILL: HideChild( IID_TEXT_COST );
      break;
    default: break;
  }
}
...
bool CSeparateDlg::HasEnoughInventoryEmptySlot(CIconItem* pItemIcon, std::vector<CSlot>& OutputItems) {
  assert( pItemIcon );
  if ( pItemIcon == nullptr ) return false;
  ...
  CIcon*                       pIcon           = nullptr;
  CIconItem*                   pOutputItemIcon = nullptr;
  std::vector<CSlot>::iterator iter;
...
void CSeparateDlg::Draw() {
  ...
  if ( pIcon = m_MaterialItemSlot.GetIcon() ) {
    ...
    CSeparate& Separate = CSeparate::GetInstance();
    switch ( Separate.GetType() ) {
      case CSeparate::TYPE_SKILL: drawFontf( g_GameDATA.m_hFONT[FONT_NORMAL], true, &rcDraw, g_dwWHITE,DT_RIGHT, "%d", Separate.GetRequireMp() );
        break;
      case CSeparate::TYPE_NPC: drawFontf( g_GameDATA.m_hFONT[FONT_NORMAL], true, &rcDraw, g_dwWHITE,DT_RIGHT, "%d", Separate.GetRequireMoney() );
        break;
      default: break;
