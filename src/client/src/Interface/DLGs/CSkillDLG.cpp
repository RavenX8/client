...
bool CSkillDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
  switch ( iProcID ) {
    ...
    case IID_BTN_CLOSE: Hide();
      break;
    case IID_BTN_OPEN_SKILLTREE: g_itMGR.OpenDialog( DLG_TYPE_SKILLTREE );
      break;
    default: break;
  }
...
void           CSkillDLG::UpdateSkillLists() {
  ...
  CJContainer* pContainer  = nullptr;
  CTabbedPane* pTabbedPane = nullptr;
...
void CSkillDLG::UpdateSkillListBySkillLevelUp(CZListBox* pListBox) {
  ...
  CSkillListItem* pSkillListItem = nullptr;
...
void CSkillDLG::Update(CObservable* pObservable, CTObject* pObj) {
  ...
  if ( pObj == nullptr || strcmp( pObj->toString(), "CTEventSkill" ) ) {
  ...
  switch ( pEvent->GetID() ) {
    ...
    case CTEventSkill::EID_ADD_SKILL: {
      ...
      switch ( iPage ) {
        case TAB_BASIC: pContainer = pTabbedPane->GetTabContainer( IID_TAB_BASIC );
          pCtrl                    = pContainer->Find( IID_ZLISTBOX_BASIC );
          assert( pCtrl );
          if ( pCtrl )
            pListBox = (CZListBox*)pCtrl;
          break;
        case TAB_ACTIVE: pContainer = pTabbedPane->GetTabContainer( IID_TAB_ACTIVE );
          pCtrl                     = pContainer->Find( IID_ZLISTBOX_ACTIVE );
          assert( pCtrl );
          if ( pCtrl )
            pListBox = (CZListBox*)pCtrl;
          break;
        case TAB_PASSIVE: pContainer = pTabbedPane->GetTabContainer( IID_TAB_PASSIVE );
          pCtrl                      = pContainer->Find( IID_ZLISTBOX_PASSIVE );
          assert( pCtrl );
          if ( pCtrl )
            pListBox = (CZListBox*)pCtrl;
