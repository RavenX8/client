...
void CCharacterDLG::Draw() {
  ...
  switch ( m_iTab ) {
    case BASIC_INFO: DrawBasicInfo();
      break;
    case ABILITY_INFO: DrawAbilityInfo();
      break;
    case UNION_INFO: DrawUnionInfo();
      break;
    default: break;
  }
}
...
unsigned int CCharacterDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if ( iProcID     = CTDialog::Process( uiMsg, wParam, lParam ) ) {
    switch ( uiMsg ) {
      case WM_LBUTTONDOWN: On_LButtonDN( iProcID, wParam, lParam );
        break;
...
bool CCharacterDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
  switch ( iProcID ) {
    case IID_BTN_DIALOG2ICON: g_itMGR.AddDialogIcon( GetDialogType() );
      break;
...
bool CCharacterDLG::On_LButtonDN(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
  ...
  short nNeedPoint = 0;
  switch ( iProcID ) {
    case IID_TAB_BASICINFO_BTN: m_iTab = BASIC_INFO;
      break;
    case IID_TAB_ABILITY_BTN: m_iTab = ABILITY_INFO;
      break;
    case IID_TAB_UNION_BTN: m_iTab = UNION_INFO;
      break;
    case IID_BTN_UP_STR: nNeedPoint = g_pAVATAR->Get_NeedPoint2AbilityUP( BA_STR );
      ...
      break;
    case IID_BTN_UP_DEX: nNeedPoint = g_pAVATAR->Get_NeedPoint2AbilityUP( BA_DEX );
      if ( (g_pAVATAR->Get_BonusPOINT() - nNeedPoint) >= 0 ) {
        //			g_pAVATAR->Set_BonusPOINT( g_pAVATAR->Get_BonusPOINT() - nNeedPoint );
        g_pNet->Send_cli_USE_BPOINT_REQ( BA_DEX );
      }
      break;
    case IID_BTN_UP_INT: nNeedPoint = g_pAVATAR->Get_NeedPoint2AbilityUP( BA_INT );
      if ( (g_pAVATAR->Get_BonusPOINT() - nNeedPoint) >= 0 ) {
        //			g_pAVATAR->Set_BonusPOINT( g_pAVATAR->Get_BonusPOINT() - nNeedPoint );
        g_pNet->Send_cli_USE_BPOINT_REQ( BA_INT );
      }
      break;
    case IID_BTN_UP_CON: nNeedPoint = g_pAVATAR->Get_NeedPoint2AbilityUP( BA_CON );
      if ( (g_pAVATAR->Get_BonusPOINT() - nNeedPoint) >= 0 ) {
        //			g_pAVATAR->Set_BonusPOINT( g_pAVATAR->Get_BonusPOINT() - nNeedPoint );
        g_pNet->Send_cli_USE_BPOINT_REQ( BA_CON );
      }
      break;
    case IID_BTN_UP_CHARM: nNeedPoint = g_pAVATAR->Get_NeedPoint2AbilityUP( BA_CHARM );
      if ( (g_pAVATAR->Get_BonusPOINT() - nNeedPoint) >= 0 ) {
        //			g_pAVATAR->Set_BonusPOINT( g_pAVATAR->Get_BonusPOINT() - nNeedPoint );
        g_pNet->Send_cli_USE_BPOINT_REQ( BA_CHARM );
      }
      break;
    case IID_BTN_UP_SENSE: nNeedPoint = g_pAVATAR->Get_NeedPoint2AbilityUP( BA_SENSE );
      if ( (g_pAVATAR->Get_BonusPOINT() - nNeedPoint) >= 0 ) {
        //			g_pAVATAR->Set_BonusPOINT( g_pAVATAR->Get_BonusPOINT() - nNeedPoint );
...
void CCharacterDLG::Update(POINT ptMouse) {
  ...
  switch ( m_iTab ) {
    ...
    case ABILITY_INFO: {
      ...
      CWinCtrl* pCtrl = GetProcessMouseOverCtrl();
      if ( pCtrl ) {
        switch ( pCtrl->GetControlID() ) {
          case IID_BTN_UP_STR: CToolTipMgr::GetInstance().RegToolTip( ptMouse.x, ptMouse.y,
                                                                      CStr::Printf( "%s %d",STR_REQUIRE_POINT, g_pAVATAR->Get_NeedPoint2AbilityUP( BA_STR ) ) );
            break;
          case IID_BTN_UP_DEX: CToolTipMgr::GetInstance().RegToolTip( ptMouse.x, ptMouse.y,
                                                                      CStr::Printf( "%s %d",STR_REQUIRE_POINT, g_pAVATAR->Get_NeedPoint2AbilityUP( BA_DEX ) ) );
            break;
          case IID_BTN_UP_INT: CToolTipMgr::GetInstance().RegToolTip( ptMouse.x, ptMouse.y,
                                                                      CStr::Printf( "%s %d",STR_REQUIRE_POINT, g_pAVATAR->Get_NeedPoint2AbilityUP( BA_INT ) ) );
            break;
          case IID_BTN_UP_CON: CToolTipMgr::GetInstance().RegToolTip( ptMouse.x, ptMouse.y,
                                                                      CStr::Printf( "%s %d",STR_REQUIRE_POINT, g_pAVATAR->Get_NeedPoint2AbilityUP( BA_CON ) ) );
            break;
          case IID_BTN_UP_CHARM: CToolTipMgr::GetInstance().RegToolTip( ptMouse.x, ptMouse.y,
                                                                        CStr::Printf( "%s %d",STR_REQUIRE_POINT, g_pAVATAR->Get_NeedPoint2AbilityUP( BA_CHARM ) ) );
            break;
          case IID_BTN_UP_SENSE: CToolTipMgr::GetInstance().RegToolTip( ptMouse.x, ptMouse.y,
