...
CUpgradeDlg::CUpgradeDlg(int iType) {
  ...
  m_pDragItemTarget->AddTarget( iType, nullptr );
  ...
  m_pDragItemMaterial->AddTarget( iType, nullptr );
...
void CUpgradeDlg::Draw() {
  ...
  switch ( Upgrade.GetType() ) {
    case CUpgrade::TYPE_SKILL: drawFontf( g_GameDATA.m_hFONT[FONT_NORMAL], true, &rcDraw, g_dwWHITE,DT_RIGHT, "%d", Upgrade.GetRequireMp() );
      break;
    case CUpgrade::TYPE_NPC: drawFontf( g_GameDATA.m_hFONT[FONT_NORMAL], true, &rcDraw, g_dwWHITE,DT_RIGHT, "%d", Upgrade.GetRequireMoney() );
      break;
    default: break;
  }
...
void CUpgradeDlg::Update(CObservable* pObservable, CTObject* pObj) {
  ...
  switch ( pEvent->GetID() ) {
    ...
    case CTEventUpgrade::EID_SET_TARGETITEM: {
      ...
      if ( m_TargetItemSlot.GetIcon() == nullptr )
...
void CUpgradeDlg::Show() {
  ...
  switch ( CUpgrade::GetInstance().GetType() ) {
    case CUpgrade::TYPE_SKILL: HideChild( IID_TEXT_COST );
      break;
    default: break;
  }
}
