...
void CNameBox::DrawNpcName(float x, float y, float z, CObjCHAR* pCharOBJ, bool bTargeted) {
  ...
  switch ( pNpc->m_nQuestSignal ) {
    case 1: bDrawQuestEmoticon = true;
      QuestEmoticonGid         = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_UI, "QUEST_EMOTICON_POSSIBLE" );
      break;
    case 2: bDrawQuestEmoticon = true;
      QuestEmoticonGid         = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_UI, "QUEST_EMOTICON_ING" );
      break;
    case 3: bDrawQuestEmoticon = true;
      QuestEmoticonGid         = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_UI, "QUEST_EMOTICON_COMPLETE" );
      break;
    default: bDrawQuestEmoticon = false;
      break;
  }
...
void    CNameBox::DrawMyName(float x, float y, float z, const char* pName, bool bTargeted) {
  ...
  int iMaxHP = g_pAVATAR->Get_MaxHP();
  ...
  RECT rt = { 0, 0, iWidthGuage, iHeightGuage };
  //RECT rt = { iWidthGuage / 2 - size.cx / 2 - 5, -18, iWidthGuage / 2 + size.cx / 2 + 5, 0 };

  D3DXMATRIX mat;
  D3DXMatrixTranslation( &mat, vDrawPos.x + 25, vDrawPos.y + 25, 0.0f );
  setTransformSprite( mat );

  //::drawFont( g_GameDATA.m_hFONT[ FONT_NORMAL_OUTLINE ], true, &rt, dwColor, DT_CENTER , pName );
  drawFont( g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE], true, 0, 0, dwColor, pName );
  ...
  if ( g_pAVATAR->GetClanID() ) {
    ...
    D3DXMATRIX mat2;
    D3DXMatrixTranslation( &mat2, vDrawClanMark.x + 25, vDrawClanMark.y + 5, 0.0f );
    setTransformSprite( mat2 );
