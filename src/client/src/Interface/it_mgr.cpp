...
void IT_MGR::OpenDialogBySkill(int iSkillSlotIdx, int iTargetDialogID) {
  switch ( iTargetDialogID ) {
    case DLG_TYPE_MAKE: OpenMakeDlg( iSkillSlotIdx );
      break;
...
short   IT_MGR::GetItemInventoryTAB(tagITEM sItem) {
  ...
  switch ( nType ) {
    ...
    case ITEM_TYPE_JEWEL: return 0;
    case ITEM_TYPE_USE: return 1;
    case ITEM_TYPE_ETC:
    case ITEM_TYPE_NATURAL:
    case ITEM_TYPE_QUEST: return 2;
  }
  return 0;
}
...
void        IT_MGR::AppendChatMsg(const char* pszMsg, int iType, DWORD forceapply_color) {
  ...
  switch ( iType ) {
    case CHAT_TYPE_SHOUT: if ( forceapply_color )
      ...
      break;
    case CHAT_TYPE_PARTY: if ( forceapply_color )
        dwColor = forceapply_color;
      else
      ...
      break;
    case CHAT_TYPE_WHISPER: if ( forceapply_color )
        dwColor = forceapply_color;
      else
      ...
      break;
    case CHAT_TYPE_NOTICE: if ( forceapply_color )
        dwColor = forceapply_color;
      else
    ...
    case CHAT_TYPE_SYSTEM: if ( forceapply_color )
      ...
      return;
      break;
    case CHAT_TYPE_QUEST: if ( forceapply_color )
        dwColor = forceapply_color;
      else
      ...
      return;
      break;
    case CHAT_TYPE_QUESTREWARD: if ( forceapply_color )
        dwColor = forceapply_color;
      else
      ...
      return;
      break;
    case CHAT_TYPE_TRADE: if ( forceapply_color )
        dwColor = forceapply_color;
      else
        dwColor   = c_dwChatColorAll;
      iFilterType = CChatDLG::FILTER_TRADE;
      break;
    case CHAT_TYPE_CLAN: if ( forceapply_color )
        dwColor = forceapply_color;
      else
        dwColor   = c_dwChatColorClan;
      iFilterType = CChatDLG::FILTER_CLAN;
      break;
    case CHAT_TYPE_ALLIED: if ( forceapply_color )
        dwColor = forceapply_color;
      else
        dwColor   = c_dwChatColorClan;
      iFilterType = CChatDLG::FILTER_ALLIED;
      break;
    case CHAT_TYPE_FRIEND: if ( forceapply_color )
        dwColor = forceapply_color;
      else
        dwColor = c_dwChatColorWhisper;
...
void        IT_MGR::InitInterfacePos() {
  ...
  for ( iter  = m_Dlgs.begin(); iter != m_Dlgs.end(); ++iter ) {
    ...
    switch ( iDefaultX ) {
      case CTDialog::TDXP_CENTER: ptNew.x = g_pCApp->GetWIDTH() / 2 - pDlg->GetWidth() / 2;
        break;
      case CTDialog::TDXP_RIGHT: ptNew.x = g_pCApp->GetWIDTH() - pDlg->GetWidth();
        break;
    ...
    switch ( iDefaultY ) {
      case CTDialog::TDYP_CENTER: ptNew.y = g_pCApp->GetHEIGHT() / 2 - pDlg->GetHeight() / 2;
        break;
      case CTDialog::TDYP_BOTTOM: ptNew.y = g_pCApp->GetHEIGHT() - pDlg->GetHeight();
        break;
...
#endif
    OpenMsgBox( "Server Disconnected", CMsgBox::BT_OK, true, 0, pCmd, nullptr );
