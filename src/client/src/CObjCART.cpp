...
CObjCART::~CObjCART(void) {
  if ( m_pObjParent != nullptr ) {
    ...
    ((CObjAVT*)m_pObjParent)->m_pObjCART = nullptr;
    m_pObjParent                         = nullptr;
  }
...
bool CObjCART::Create(CObjCHAR* pParent, int iCartType, D3DVECTOR& Position) {
  ...
  CMODEL<CCharPART>* pPartMODEL = nullptr;
  ...
  if ( CreateCHAR( szName, &m_CharMODEL, MAX_RIDING_PART, Position ) ) {
...
bool CObjCART::Create(CObjCHAR* pTarget) {
  ...
  //회전 및 모션 설정 
  m_pRideUser->Set_ModelDIR( 0.0f, true );
  ...
  switch ( iCurrentState ) {
    case CART_STATE_STOP: {
      ...
      if ( iSoundIDX ) {
        ...
        g_pSoundLIST->IDX_PlaySound3DLoop( iSoundIDX, vecWorld, nullptr );
      }
    }
    break;
...
int CObjCART::GetANI_Attack() {
  return PAT_RELATIVE_MOTION_POS( m_nPartItemIDX[ RIDE_PART_BODY ] ) + CART_ANI_ATTACK01; /*+ RANDOM(3);*/
}
