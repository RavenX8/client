...
void CQuadPatchManager::CalculateQuadPatchCulling(short index) {
  ...
  switch ( ViewCullingFunc( &m_quadPATCH[index].aabb ) ) {
    case 0: InsertMemoQuadPatch( index );
      InsertMemoQuadExPatch( index );
      break;
    case 1: InsertMemoQuadExPatch2( index );
...
void CQuadPatchManager::GetPickingRay(float Position[3], float Direction[3]) {
  ...
  m_RayPosition[0]  = Position[0];
  m_RayPosition[1]  = Position[1];
  m_RayPosition[2]  = Position[2];
  m_RayDirection[0] = Direction[0];
  m_RayDirection[1] = Direction[1];
  m_RayDirection[2] = Direction[2];
...
bool CMAP_PATCH::UnregisterFromNZIN(void) {
  ...
  while ( pEftNode ) {
    ...
    if ( pEftNode->DATA.m_pEffect ) {
      ...
      pEftNode->DATA.m_pEffect = nullptr;
    } else
...
void                       CMAP_PATCH::PlaySOUND() {
  ...
  while ( pSndNode ) {
    ...
    if ( dwTime - pSndNode->DATA.m_dwLastPlayTIME > pSndNode->DATA.m_uiInterval ) {
      ...
      g_pSoundLIST->KEY_PlaySound3D( pSndNode->DATA.m_HashKEY, pSndNode->DATA.m_Position, iVolume, 0, nullptr, fMinDistance, fMaxDistance );
...
bool    CMAP_PATCH::MakeAABBFromObject(CGameOBJ* pObject, D3DVECTOR& Position, D3DXQUATERNION& Rotate, D3DVECTOR& Scale) {
  ...
  CMODEL<CFixedPART>* pModel = nullptr;
...
void    CMAP::MappingToZONE(short nXFrom3x3, short nYFrom3x3) {
  ...
  if ( this->IsUsing() ) {
    ...
    for ( iH   = 0; iH < m_nYPatchCnt; iH++ )
      for ( iW = 0; iW < m_nXPatchCnt; iW++ ) {
        g_pTerrain->SetPATCH( nViewPatchX + iW, nViewPatchY + iH, nullptr );
      }
  }
}
...
CMAP*   CMAP::Load(char* szFileName, short nZoneMapXIDX, short nZoneMapYIDX) {
  ...
  for ( iH                   = 0; iH < LUMP_MAP_MAX; iH++ )
    this->m_pObjectIndex[iH] = nullptr;
  ...
  for ( iH = 0; iH < iLumpCNT; iH++ ) {
    ...
    switch ( iType ) {
      case LUMP_MAPINFO: ReadMapINFO( pFileSystem, iOffset );
        break;
      case LUMP_TERRAIN_OCEAN: ReadOceanINFO( pFileSystem, iOffset );
        break;
      ...
      case LUMP_TERRAIN_WARP:
      case LUMP_TERRAIN_COLLISION:
      case LUMP_TERRAIN_EVENT_OBJECT: ReadObjINFO( pFileSystem, iOffset, iType );
      default: break;
    }
...
//-------------------------------------------------------------------------------------------------
CTERRAIN* CTERRAIN::m_pInstance = nullptr;
//-------------------------------------------------------------------------------------------------
CTERRAIN::CTERRAIN() {
  m_iObjCreateOrder = 0;
...
bool CTERRAIN::LoadZONE(short nZoneNO, bool bPlayBGM) {
  ...
  iCount = CUtil::ExtractFileName( nullptr, ZONE_FILE( m_nZoneNO ) );
  if ( iCount > 0 ) {
    char* pStr = CStr::GetString();
  ...
  for ( int iC = 0; iC < iCount; iC++ ) {
    ...
    switch ( iType ) {
      case LUMP_ZONE_INFO: ReadZoneINFO( pFileSystem, iOffset );
        break;
      case LUMP_EVENT_OBJECT: ReadEventObjINFO( pFileSystem, iOffset );
        break;
      case LUMP_ZONE_TILE: ReadTileINFO( pFileSystem, iOffset );
        break;
      case LUMP_TILE_TYPE: ReadTileTYPE( pFileSystem, iOffset );
        break;
      case LUMP_ECONOMY: ReadECONOMY( pFileSystem, iOffset );
        break;
    }
...
short CTERRAIN::Pick_OBJECT(D3DXVECTOR3& PosPICK, float& fDistance) {
  ...
  while ( pNODE ) {
    ...
    if ( g_pObjMGR->m_pOBJECTS[pNODE->DATA] != nullptr ) {
...
CMAP* CTERRAIN::GetMapPTR(short nZoneMapX, short nZoneMapY) {
  if ( nZoneMapX < 0 || nZoneMapX >= MAP_COUNT_PER_ZONE_AXIS ) return nullptr;
  if ( nZoneMapY < 0 || nZoneMapY >= MAP_COUNT_PER_ZONE_AXIS ) return nullptr;
  ...
  for ( nI       = 0; s_SubSecIdx[wUpdateFLAG][nI] >= 0; nI++ ) {
    ...
    if ( nZoneMapXIDX >= 0 && nZoneMapXIDX < MAP_COUNT_PER_ZONE_AXIS &&
      ...
      if ( pMAP ) {
        ...
        {
          tagLOAD_ONE_MAP_DATA* pOneMapData = nullptr;
      ...
      m_pMAPS[nZoneMapYIDX][nZoneMapXIDX] = nullptr;
    }
  }
}
      ...
      if ( pMap ) {
        ...
        for ( iH   = 0; iH < PATCH_COUNT_PER_MAP_AXIS; iH++ ) {
          for ( iW = 0; iW < PATCH_COUNT_PER_MAP_AXIS; iW++ ) {
            this->SetPATCH( nViewPatchX + iW, nViewPatchY + iH, nullptr );
          }
        }
      }
      this->Unlock();
...
void CTERRAIN::UpdatePatchManager(short nCenterMapXIDX, short nCenterMapYIDX) {
  ...
  for ( nI = 0; s_AddSecIdx[MOVE_UPDATE_ALL][nI] >= 0; nI++ ) {
    ...
    if ( nZoneMapXIDX >= 0 && nZoneMapXIDX < MAP_COUNT_PER_ZONE_AXIS &&
      ...
      if ( m_pMAPS[nZoneMapYIDX][nZoneMapXIDX] != nullptr ) {
        ...
        m_PatchManager.m_ppQuadPatchManager[buffer_Y + 1][buffer_X + 1] = nullptr;
...
void CTERRAIN::InsertCameraPatch(float xChar, float yChar) {
  ...
  for ( nIndex = g_pCRange->GetStartIndex( 0 ); nIndex < g_pCRange->GetStartIndex( 4 ); nIndex++ ) {
    ...
    if ( m_pMAPS[map_center[1]][map_center[0]] != nullptr && m_pMAPS[map_center[1]][map_center[0]]->IsUsing() ) {
