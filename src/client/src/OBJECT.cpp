...
bool CObjectMANAGER::Set_EmptySlot(short nSlotNO, WORD wServerObjectIndex, CGameOBJ* pObject) {
  ...
  if ( nullptr == m_pOBJECTS[nSlotNO] ) {
    ...
    switch ( pObject->Get_TYPE() ) {
      case OBJ_ITEM: pObject->m_pListNODE = m_ItemLIST.AllocNAppend( nSlotNO );
        break;
      case OBJ_MORPH:
      case OBJ_GROUND:
      case OBJ_CNST: pObject->m_pListNODE = m_CnstLIST.AllocNAppend( nSlotNO );
      ...
      case OBJ_EVENTOBJECT: CObjFixedEvent* pEventObj = (CObjFixedEvent*)pObject;
...
void CObjectMANAGER::Del_Object(CGameOBJ* pObject) {
  ...
  m_pOBJECTS[pObject->m_nIndex] = nullptr;
  m_iTotalObjCNT --;
  m_iObjectCNT[pObject->Get_TYPE()] --;
...
CObjCART* CObjectMANAGER::Add_CartCHAR(int iPetType, CObjAVT* pOwner, WORD wServerObjectIndex) {
  ...
  CObjCART* pCHAR = nullptr;

  //----------------------------------------------------------------------------------------------------
  /// Pet type 에 따른 적당한 클래스 생성.
  //----------------------------------------------------------------------------------------------------
  switch ( iPetType ) {
    case PET_TYPE_CART01: pCHAR = new CObjCART();
      break;
    case PET_TYPE_CASTLE_GEAR01: pCHAR = new CObjCastleGear();
      break;
  }
