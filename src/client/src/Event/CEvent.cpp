  ...
  */
  ...
  if ( pMenuCollMMT ) {
    ...
    for ( int i                   = 0; i < ConvHeader.m_MenuNumber; i++ ) {
      ...
      m_pScrDATA[i].m_pScrITEM    = nullptr;
...
char* CEvent::ParseMESSAGE(char* szMSG) {
  ...
  char *pBegin = nullptr, *pEnd = nullptr;
  ...
  while ( pBegin ) {
    ...
    if ( strcmp( pEnd, "WORLD_RATE" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getWorldRate() ); } else if ( strcmp( pEnd, "TOWN_RATE" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getTownRate() ); } else if ( strcmp( pEnd, "USER_MONEY" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_checkUserMoney() ); } else if ( strcmp( pEnd, "ITEM_RATE0" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 0 ) ); } else if ( strcmp( pEnd, "ITEM_RATE1" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 1 ) ); } else if ( strcmp( pEnd, "ITEM_RATE2" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 2 ) ); } else if ( strcmp( pEnd, "ITEM_RATE3" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 3 ) ); } else if ( strcmp( pEnd, "ITEM_RATE4" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 4 ) ); } else if ( strcmp( pEnd, "ITEM_RATE5" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 5 ) ); } else if ( strcmp( pEnd, "ITEM_RATE6" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 6 ) ); } else if ( strcmp( pEnd, "ITEM_RATE7" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 7 ) ); } else if ( strcmp( pEnd, "ITEM_RATE8" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 8 ) ); } else if ( strcmp( pEnd, "ITEM_RATE9" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_getItemRate( 9 ) ); } else if ( strcmp( pEnd, "NAME" ) == 0 ) { sprintf( m_TempBuffer, "%s%s", m_TempBuffer, g_pAVATAR->Get_NAME() ); } else if ( strcmp( pEnd, "SEX" ) == 0 ) { sprintf( m_TempBuffer, "%s%s", m_TempBuffer, g_pAVATAR->Get_SEX() == 1 ? "여" : "남" ); } else if ( strcmp( pEnd, "RANK" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_RANK() ); } else if ( strcmp( pEnd, "UNION" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_UNION() ); } else if ( strcmp( pEnd, "FAME" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_FAME() ); } else if ( strcmp( pEnd, "JOB" ) == 0 ) { sprintf( m_TempBuffer, "%s%s", m_TempBuffer, CStringManager::GetSingleton().GetJobName( g_pAVATAR->Get_JOB() ) ); } else if ( strcmp( pEnd, "EXP" ) == 0 ) { sprintf( m_TempBuffer, "%s%lld", m_TempBuffer, g_pAVATAR->Get_EXP() ); } else if ( strcmp( pEnd, "POINT" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_BonusPOINT() ); } else if ( strcmp( pEnd, "STR" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_STR() ); } else if ( strcmp( pEnd, "INT" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_INT() ); } else if ( strcmp( pEnd, "DEX" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_DEX() ); } else if ( strcmp( pEnd, "CON" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_CON() ); } else if ( strcmp( pEnd, "MAXHP" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_MaxHP() ); } else if ( strcmp( pEnd, "MAXWEIGHT" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_MaxWEIGHT() ); } else if ( strcmp( pEnd, "MAXMP" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->Get_MaxMP() ); } else if ( strcmp( pEnd, "MONEY" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, GF_checkUserMoney() ); } else if ( strcmp( pEnd, "LEVEL" ) == 0 ) { sprintf( m_TempBuffer, "%s%d", m_TempBuffer, g_pAVATAR->GetCur_LEVEL() ); } else if ( strcmp( pEnd, "REVIVE_ZONE" ) == 0 ) { sprintf( m_TempBuffer, "%s%s", m_TempBuffer, GF_getReviveZoneName() ); }
...
short CEvent::Conversation(int iMenuIDX) {
  ...
  short nValiedCNT = 0;
  char *fpCheck    = nullptr, *szMessage = nullptr;
    ...
    switch ( m_pScrDATA[iMenuIDX].m_pScrITEM[nI].m_iType ) {
      ...
      case SC_MSG_JUMPSELECT: {
        ...
        g_itMGR.QueryDLG_AppendExam( szMessage,           // char* szExam,
                                     ...
                                     Click_ITEM,          // void (*fpExamEvent)(int iEventID) );
                                     GetQueryType()
        );
        break;
      }
      ...
      case SC_MSG_NPCSAY:
      case SC_MSG_NEXTMSG: Del_ClickITEMS();
        ...
        g_itMGR.OpenQueryDLG( 300, 100,
                              ...
                              (short)strlen( szMessage ),
                              //m_pScrDATA[ iMenuIDX ].m_pScrITEM[ nI ].m_Message.BuffLength(),
                              (int)this,
                              Click_CLOSE, m_iOwnerObjIDX, m_iEventDlgType );
