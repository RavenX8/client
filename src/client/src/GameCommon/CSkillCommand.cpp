...
bool CBasicCommand::Execute() {
  ...
  switch ( iBasicCommandType ) {
    ...
    case COMMON_COMMAND_PICK_ITEM: {
      ...
      CObjITEM* pFoundItem    = nullptr;
      CObjITEM* pTempItem     = nullptr;
      ...
      if ( g_pAVATAR->GetPetMode() >= 0 ) {
        ...
        //				if( CCountry::GetSingleton().IsApplyNewVersion() )
        {
          switch ( ZONE_RIDING_REFUSE_FLAG( g_pTerrain->GetZoneNO() ) ) {
            case 1: if ( (ITEM_TYPE( iItemType, iItemNo ) / 100 == 5) && (ITEM_TYPE( iItemType, iItemNo ) % 10 == 1) ) {
                g_itMGR.AppendChatMsg( STR_CANT_USE_CART, IT_MGR::CHAT_TYPE_SYSTEM, D3DCOLOR_ARGB( 255, 206, 223, 136) );
                return false;
              }
              break;
            case 2: if ( (ITEM_TYPE( iItemType, iItemNo ) / 100 == 5) && (ITEM_TYPE( iItemType, iItemNo ) % 10 == 2) ) {
                g_itMGR.AppendChatMsg( STR_CANT_USE_CASTLEGEAR, IT_MGR::CHAT_TYPE_SYSTEM, D3DCOLOR_ARGB( 255, 206, 223, 136) );
                return false;
              }
              break;
            case 3: g_itMGR.AppendChatMsg( STR_CANT_USE_DRIVESKILL, IT_MGR::CHAT_TYPE_SYSTEM, D3DCOLOR_ARGB( 255, 206, 223, 136) );
              return false;
              break;
            default: break;
          }
