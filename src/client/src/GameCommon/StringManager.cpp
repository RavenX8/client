...
{
  0,
  LIST_FACEITEM_S_TB,  //ITEM_TYPE_FACE_ITEM = 1,		// 1	LIST_FACEITEM.stb	�� ���	
  ...
  LIST_JEWEL_S_TB,     //ITEM_TYPE_JEWEL,				// 7	LIST_JEWEL.stb		��ű� : ����� ����
  LIST_WEAPON_S_TB,    //ITEM_TYPE_WEAPON,				// 8	LIST_WEAPON.stb		����
  LIST_SUBWPN_S_TB,    //ITEM_TYPE_SUBWPN,				// 9	LIST_SUBWPN.stb
  LIST_USEITEM_S_TB,   //ITEM_TYPE_USE,					// 10	LIST_USEITEM.stb	�Ҹ�	
  LIST_JEMITEM_S_TB,   //ITEM_TYPE_GEM = ITEM_TYPE_ETC,	// 11	LIST_JEMITEM.stb	��Ÿ : ����
...
LANGUAGE_TYPE CStringManager::GetLanguageIDInGame(int iCharSet) {
  switch ( iCharSet ) {
    case SHIFTJIS_CHARSET: return LANGUAGE_JPN;
    case HANGUL_CHARSET: return LANGUAGE_KOR;
    case GB2312_CHARSET: return LANGUAGE_CHA_SIMPLE;

    case CHINESEBIG5_CHARSET: return LANGUAGE_CHA_TRADITIONAL;
...
char* CStringManager::GetFontNameByCharSet(int iCharSet) {
  ...
  switch ( m_iLanguageType ) {
    ...
    case LANGUAGE_JPN: {
      ...
      strcpy( (LPSTR)&lf.lfFaceName, "�l�r �o�S�V�b�N" );
      /*if (FAILED(hr))
      {
    ...
    case LANGUAGE_CHA_TRADITIONAL: return "MingLiU";
  }
  return "Arial";
}
...
//------------------------------------------------------------------------------------------
// Noraml Type Table�� Item Type Table�� ���Ǵ� Data�� �ִ�.. ���� Loading�Ұ� - 2005/1/11 - nAvy
//------------------------------------------------------------------------------------------
...
bool  CStringManager::LoadTypeTable(LANGUAGE_TYPE languageType, const char* pszFileName, int iStringType) {
  ...
  try {
    ...
    MessageBox( nullptr, "Not enough memory", "ERROR", MB_OK );
...
stItemTypeTable* CStringManager::GetItemStringData(int iType, int iItemNo) {
  ...
  if ( g_pTblSTBs[iType]->m_ppDESC[iItemNo] == nullptr )
    return nullptr;
...
int CStringManager::GetItemNOFromName(int iType, const char* cName) {
  if ( iType < 1 || iType > ITEM_TYPE_RIDE_PART )
    return -1;
  const char* cTemp = "";

  for ( int idx = 0; idx < g_pTblSTBs[iType]->m_nDataCnt; idx++ ) {
    cTemp       = GetItemName( iType, idx );
    if ( cTemp == nullptr )
      break;
    if ( _strcmpi( "", cTemp ) == 0 )
      continue;

    if ( _strcmpi( cName, cTemp ) == 0 )
      return idx;
  }
...
stItemTypeTable* CStringManager::GetSkillStringData(int iSkillNo) {
  if ( g_SkillList.m_SkillDATA.m_ppDESC[iSkillNo] == nullptr )
    return nullptr;
...
stQuestTypeTable* CStringManager::GetStatusStringData(int iStatusNo) {
  ...
  if ( STATE_STRING_ID( iStatusNo ) == nullptr )
    return nullptr;
...
stQuestTypeTable* CStringManager::GetQuestStringData(int iQuestNo) {
  if ( g_QuestList.m_STB.m_ppDESC[iQuestNo] == nullptr )
    return nullptr;
...
const char* CStringManager::GetNpcName(int iIndex) {
  if ( NPC_STRING_ID( iIndex ) == nullptr )
    return m_strNull.c_str();
...
//-----------------------------------------------------------------------------------------------
/// NormalType Table String access methods
//-----------------------------------------------------------------------------------------------
...
stItemTypeTable* CStringManager::GetZoneStringData(int iZoneNO) {
  if ( ZONE_STRING_ID( iZoneNO ) == nullptr )
    return nullptr;
...
const char* CStringManager::GetTipHeader(int iIndex) {
  ...
  if ( g_TblHELP.m_ppVALUE[iIndex][5].GetSTR() == nullptr )
    return m_strNull.c_str();
