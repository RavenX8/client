...
CObjITEM::CObjITEM() {
  ...
  m_hAnimatable    = NULL;
  //m_ITEM				= NULL;
  ZeroMemory(&m_ITEM,sizeof(tagBaseITEM));
}
...
bool CObjITEM::CreateAnimatable() {
  ...
  if ( m_hAnimatable == NULL )
    m_hAnimatable = loadAnimatable( CStr::Printf( "ItemAnimatable_%d", this->Get_INDEX() ), NULL, m_hMotion, NULL, g_GameDATA.m_hShader_nolit );
  if ( m_hAnimatable == NULL ) {
    return false;
  }
