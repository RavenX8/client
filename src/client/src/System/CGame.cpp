...
CGameState*   CGame::MakeState(int iState) {
  CGameState* pState = nullptr;
  switch ( iState ) {
    case GS_TITLE: pState = new CGameStateTitle( GS_TITLE );
      break;
    case GS_LOGIN: pState = new CGameStateLogin( GS_LOGIN );
      break;
    case GS_WARNING: pState = new CGameStateWarning( GS_WARNING );
      break;
    case GS_SELECTSVR: pState = new CGameStateSelectSvr( GS_SELECTSVR );
      break;
    case GS_PREPARESELECTAVATAR: pState = new CGameStatePrepareSelectAvatar( GS_PREPARESELECTAVATAR );
      break;
    case GS_SELECTAVATAR: pState = new CGameStateSelectAvatar( GS_SELECTAVATAR );
      break;
    case GS_LOGINVIRTUAL: pState = new CGameStateLoginVirtual( GS_LOGINVIRTUAL );
      break;
    case GS_CREATEAVATAR: pState = new CGameStateCreateAvatar( GS_CREATEAVATAR );
      break;
    case GS_PREPAREMAIN: pState = new CGameStatePrepareMain( GS_PREPAREMAIN );
      break;
    case GS_MAIN: pState = new CGameStateMain( GS_MAIN );
      break;
    case GS_WARP: pState = new CGameStateWarp( GS_WARP );
      break;
    case GS_EXITMAIN: pState = new CGameStateExitMain( GS_EXITMAIN );
      break;
    case GS_MOVEMAIN: pState = new CGameStateMoveMain( GS_MOVEMAIN );
      break;

    case GS_MOVEPLANET: pState = new CGameStateMovePlanet( GS_MOVEPLANET );
      break;

    case GS_RELOGIN: pState = new CGameStateReLogin( GS_RELOGIN );
      break;
...
void CGame::ChangeState(int newState) {
  if ( m_GameStates[newState] == nullptr )
...
bool CGame::Load_BasicDATA2() {
  ...
  ////g_TblHELMET.Load	( "3DDATA\\STB\\LIST_Cap.STB",		true, true  );=>Load_BasicDATA()�� ��,������ �ƹ�Ÿ�� ����� ���ؼ� �ʿ��� ���̺�
...
/// 2004 /3/ 29:nAvy - Client�� ����� ó�� Loading�ð��� ���̱� ���ؼ� Load_BasicDATA2�� �и�
/// CGameLoadSelectAvatarState���� �������� Loading�Ѵ�.
/// * ���� Skill, Weapon�� Loading �ð��� ����5�ʸ� �Ѿ��.
/// * 2�� �ε����� �ʰ� �����ϰų� STB�ȿ��� �̹� Load�� STB�� �ٽ� Load���ϰ� �Ѵ�.
/// * Prototype Pattern�� ����غ���??
bool CGame::Load_BasicDATA() {
  ...
  g_pEffectLIST = new CEffectLIST( "3ddata\\stb\\FILE_EFFECT.stb" ); // shader �������� �ε��ؾ� ��..

  if ( !g_DATA.Load3DDATA() ) // shader �������� �ε��ؾ� ��..
    return false;
  ...
  // *-------------------------------------------------------------------* //
  g_TblPRODUCT.Load( "3DDATA\\STB\\LIST_PRODUCT.STB", true, false ); ///2004 / 2 /4 :nAvy ���� PRODUCT.STB => LIST_PRODUCT.STB
...
bool CGame::Load_DataNotUseThread() ///�����带 ����ؼ� ������ �ȵǴ� Data�� Load�ϴ� Method: ������ ���ϸ����ؾ��Ѵ�.
{
...
void CGame::CreateSelectedAvata() {
  ...
  /// skill slot setting : UpdateAbility�ڷ� �̵�
  ...
  /// Ŭ���̾�Ʈ������ ȿ���� ��ǰ� ����ȭ ��Ű�� ���Ͽ� ���Ǵ� ������ Setting : 2004 / 2 / 23 
  /// CObjAVT::m_ShotData�� �ٸ� �ƹ�Ÿ���� ������ ��ǰ� ����ȭ ��Ű�� �ʰ�
  /// CObjUSER�� CInventory::m_ItemSHOT�� ���� 2���� ����ؼ� ������ Ŭ���̾�Ʈ�� ��ǰ� ����ȭ ��Ų��.
  ...
  //----------------------------------------------------------------------------------------------------	
  /// ���� ���� ���� + CQuickDlg �� ���� ������Ʈ( Inventory�� Skill������ ���� �޾ƾ� �Ѵ�.)
  ...
  for ( int  i     = 0; i < MAX_MAINTAIN_STATUS; ++i ) {
    if ( dwABS     = g_pAVATAR->m_GrowAbility.m_STATUS[i].m_dwExpiredSEC ) {
      if ( classTIME::GetCurrentAbsSecond() < dwABS ) {
        ...
        //g_itMGR.AppendChatMsg( CStr::Printf("%s�� %d�� %d�� %d�� %d�ñ��� ��밡���մϴ�", pszName, stCurrTime.wYear, stCurrTime.wMonth, stCurrTime.wDay, stCurrTime.wHour ), IT_MGR::CHAT_TYPE_NOTICE);
  ...
  /// ����̺� ��ų ��Ÿ�� ���� : 2005/7/27 - nAvy
  /// ��Ÿ���� ������� g_pAVATAR->m_GrowAbility.m_dwPatCoolTIME == 0
...
void CGame::ProcWndMsg(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  switch ( uiMsg ) {
    case WM_CLOSE: if ( m_pCurrState->GetStateID() == GS_MAIN )
        g_itMGR.OpenDialog( DLG_TYPE_SYSTEM );
      else
        g_pCApp->SetExitGame();
      break;
    case WM_USER_CLOSE_MSGBOX: if ( m_pCurrState->GetStateID() == GS_LOGIN || m_pCurrState->GetStateID() == GS_SELECTAVATAR )
        g_EUILobby.HideMsgBox();

      break;
    case WM_USER_SERVER_DISCONNECTED: if ( !g_pCApp->IsExitGame() )
        m_pCurrState->ServerDisconnected();
      break;
    case WM_USER_WORLDSERVER_DISCONNECTED: if ( !g_pCApp->IsExitGame() )
...
void CGame::ScreenCAPTURE() {
  ...
  if ( beginScene() ) {
    ...
    drawFontf( g_GameDATA.m_hFONT[FONT_NORMAL_OUTLINE], true, g_pCApp->GetWIDTH() - (12 * 17), g_pCApp->GetHEIGHT() - (12 * 5), D3DCOLOR_XRGB( 255, 255, 255),
            ...
            %s %d, %d \n\
            %d.%d.%d %d:%d:%d",
               this->GetServerInfo().strServerName.c_str(), // ���� �̸�

               g_pAVATAR->Get_NAME(), // �ɸ� �̸�

               ZONE_NAME( g_pTerrain->GetZoneNO() ), // ��ġ.
  ...
  if ( D3D_OK == reinterpret_cast<LPD3DDEVICE>(getDevice())->GetRenderTarget( 0, &pSUR ) ) {
    ...
    ::CreateDirectory( "capture", nullptr );
    ...
    if ( D3D_OK == D3DXSaveSurfaceToFile( pTempBMP, D3DXIFF_BMP, pSUR, nullptr, nullptr ) ) {
      LogString (LOG_NORMAL, "Capture Screen %s", pStr);
...
void        CGame::ProcInput() {
  tagWNDMSG Msg;
  while ( GetInstance().m_WndMsgQ.GetMouseMsgFromQ( Msg ) ) {
    if ( !m_pCurrState->ProcMouseInput( Msg.uiMsg, Msg.wParam, Msg.lParam ) ) { }
  }
  while ( GetInstance().m_WndMsgQ.GetKeyboardMsgFromQ( Msg ) ) {
    switch ( Msg.uiMsg ) {
      case WM_SYSKEYDOWN: {
    ...
    switch ( Msg.uiMsg ) {
      case WM_KEYUP: if ( Msg.wParam == VK_SNAPSHOT )
          //ScreenCAPTURE ();
          g_pNet->Send_cli_SCREEN_SHOT_TIME();
        break;
      case WM_SYSKEYDOWN: switch ( Msg.wParam ) {
          case '0': CCursor::GetInstance().ChangeNextState();
            //CGame::GetInstance().ToggleUserMouseFlag();
            //CGame::GetInstance().SetUserCursor( CGame::CURSOR_DEFAULT );
            break;
          case '/': g_bDontDeleteCapFile = !g_bDontDeleteCapFile;
            break;
          case VK_RETURN: {
            ChangeScreenMode();
...
WORD CGame::GetPayType() {
  ...
  if ( m_paytype & PLAY_FLAG_KOREA_DEFAULT ) {
    return PAY_PREMIUM;
  }
  //NO EXT
  switch ( m_paytype ) {
    case BILLING_MSG_PAY_FU: // FU	�α��� �� ����ڴ� ���� ���̵� ��������� ��Ÿ����.
      return PAY_FREE;
    case BILLING_MSG_PAY_FQ: // FQ	�α��� �� ����ڴ� �������� �����(�����̾�)���� ��Ÿ����.
    case BILLING_MSG_PAY_FA: // FA	�α��� �� ����ڴ� �������� �����(�����̾�)���� ��Ÿ����.
      return PAY_PREMIUM;
    case BILLING_MSG_PAY_GU:  // GU	�α��� �� ����ڴ� ���ӹ� �����(�÷�Ƽ��)���� ��Ÿ����.
    case BILLING_MSG_PAY_GQ:  // GQ	(�÷�Ƽ��)���ӹ濡�� �α����� �õ��Ͽ����� ���ӹ��� ���ݱⰣ(�ð�)�� ����Ǿ� ������������ �α��� �Ǿ����� ��Ÿ����.
    case BILLING_MSG_PAY_IQ:  // IQ	(�÷�Ƽ��)���ӹ濡�� �α����� �õ��Ͽ����� �̹� ���� IP ���ڸ� ��� ����ϰ� �־� ������������ �α��� �Ǿ����� ��Ÿ����.
