...
void CGameStateMain::Render_GameMENU() {
  ...
  if ( g_GameDATA.m_bDisplayDebugInfo ) {
    ...
    if ( g_pAVATAR->GetCur_SummonCNT() > 0 ) {
      drawFontf( g_GameDATA.m_hFONT[FONT_LARGE_BOLD],
                 false, 5, 150, g_dwYELLOW, "[ Summon Count [ %d ] , Current [%d] / Max [%d]",
    ...
    drawFontf( g_GameDATA.m_hFONT[FONT_NORMAL], false, 220, 60, g_dwRED, "( ASpeed: %d, MSpeed: %f DSpeed: %f )",
    ...
    drawFontf( g_GameDATA.m_hFONT[FONT_NORMAL], false, 220, 80, g_dwRED, "( World Time: %d, Zone Time:%d, DayNight Blend Factor:%f )",
...
int      CGameStateMain::ProcMouseInput(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  switch ( uiMsg ) {
    ...
    case WM_LBUTTONDBLCLK: On_WM_LBUTTONDBCLICK( wParam, lParam );
      break;
...
#pragma message("일본어의 경우 변환키와 중복되지 않는지 테스트해보아야 한다")
  ...
  switch ( uiMsg ) {
    ...
    case WM_CHAR: return On_WM_CHAR( wParam, lParam );

    case WM_KEYDOWN: return On_WM_KEYDOWN( wParam, lParam );
    ...
    case WM_SYSKEYDOWN: {
      ...
      switch ( vk ) {
          ...
          if ( CGame::GetInstance().GetRight() >= CHEAT_GM ) {
            ...
            if ( pChar ) {
              switch ( pChar->Get_TYPE() ) {
                case OBJ_NPC:
                case OBJ_MOB: g_pNet->Send_cli_CHAT( "/get tg" );
                  break;
                case OBJ_AVATAR: {
                ...
                break;
                case OBJ_USER: g_pNet->Send_cli_CHAT( "/get hp" );
                  break;
                default: break;
              }
            } else {
        ...
        break;
          ...
          /// 'j'
        case 0x4A: if ( CGame::GetInstance().GetRight() >= CHEAT_GM ) {
#ifdef _DEBUG
        ...
        if ( pObj ) {
          switch ( pObj->Get_TYPE() ) {
            case OBJ_MOB: if ( ((CObjCHAR*)pObj)->CanClickable() ) {
              ...
              break;
            case OBJ_NPC: if ( ((CObjCHAR*)pObj)->CanClickable() ) {
                refCursor.SetCursorType( CCursor::CURSOR_NPC );
              }
              break;

            case OBJ_AVATAR: if ( ((CObjCHAR*)pObj)->CanClickable() ) {
            ...
            case OBJ_GROUND:
            case OBJ_CNST: refCursor.SetCursorType( CCursor::CURSOR_DEFAULT );
              break;
            default: {
              refCursor.SetCursorType( CCursor::CURSOR_DEFAULT );
            }
