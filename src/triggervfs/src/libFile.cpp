...
long           __GetFileSize(const char* FileName) {
  ...
  if ( _stat( FileName, &buffer ) == 0 ) { return (long)buffer.st_size; }
...
bool     MakeFileHole(long lPos, size_t stSize, FILE* fp, bool bFlush) {
  ...
  if ( stSize < 0 || fp == nullptr || lPos < 0 ) { return false; } /// Checking
...
stFindFiles*  GetFileList(const char* pBaseDir, const char* FilterExt, const char* FilterDir) {
  ...
  if ( FilterExt != nullptr && strlen( FilterExt ) > 0 && (buff = new char [ strlen( FilterExt ) + 1 ]) ) {
  ...
  g_stFindFile.vecExcDir.push_back( ".." );
  if ( FilterDir != nullptr && strlen( FilterDir ) > 0 && (buff = new char [ strlen( FilterDir ) + 1 ]) ) {
...
const char* GetRelativePath(const char* Path1, const char* Path2) {
  ...
  if ( !sPath1 || !sPath2 ) { return nullptr; } /// 메모리할당에 실패하면 그냥 NULL을 리턴.
  strcpy( sPath1, Path1 );                      /// 대문자로 변환하기 위해
  strcpy( sPath2, Path2 );                      /// 대문자로 변환하기 위해

  if ( strlen( Path1 ) < strlen( Path2 ) ) { return nullptr; } /// 경로1이 경로2보다 작으면 NULL을 리턴
  _strupr( sPath1 );
  _strupr( sPath2 );

  for ( unsigned int i = 0; i < strlen( sPath2 ); i++ ) {
    if ( sPath1[i] != sPath2[i] ) { return nullptr; }
  }
...
bool   CFileMode::CheckMode(const char* FileName, int iChkMode, bool bForce) {
  ...
  if ( bAccess == false && bForce ) {
    switch ( iChkMode ) {
      case MODE_READ: return (_chmod( FileName, _S_IREAD ) == 0);

      case MODE_WRITE: return (_chmod( FileName, _S_IWRITE ) == 0);

      case MODE_READWRITE: return (_chmod( FileName, _S_IREAD | _S_IWRITE ) == 0);
    }
  }
