...
bool CVFS::Open(FILE* fpFAT, long lOffset, const char* VfsName, const char* Dir, const char* Mode) {
  ...
  if ( (strcmp( Mode, "mr" ) == 0) || (strcmp( Mode, "mr+" ) == 0) ) {
    m_hFile = CreateFile( VfsName, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY, nullptr );
...
short CVFS::AddFile(const char*   FileName
                    ...
                    , BYTE        btCompress
                    , bool        bUseDel) {
  FILE*      fp             = nullptr; /// 추가할 파일의 파일포인터
  BYTE*      pbtBuff        = nullptr; /// 추가할 파일에서 읽은 데이터를 여기에 저장
  BYTE*      pbtChangedData = nullptr; /// Encryption이나 Compress한 데이터는 여기로 리턴
  FileEntry* pFileEntry     = nullptr; /// 새롭게 new한 FileEntry는 여기에 저장
  /// DWORD dwCRC					= 0;	/// CRC값
...
VFileHandle*   CVFS::OpenFile(const char* FileName) {
  VFileHandle* pVFH = nullptr;
  FileEntry*   pFE  = nullptr; /// 여기 찾은 파일의 파일 엔트리를 저장한다
