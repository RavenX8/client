...
short __stdcall VAddFile(VHANDLE       hVFS
  ...
  if ( pVFS ) {
    ...
    if ( nRes == VADDFILE_CANTWRITE || nRes == VADDFILE_CANTFLUSH ) {
      DWORD dwErrCode = 0;
      switch ( nRes ) {
        case VADDFILE_CANTWRITE: dwErrCode = CErrorInfo::GetError() + VADDFILE_FIXEVALUE_CANTWRITE;
          break;

        case VADDFILE_CANTFLUSH: dwErrCode = CErrorInfo::GetError() + VADDFILE_FIXEVALUE_CANTFLUSH;
          break;
...
int __stdcall vfseek(VFileHandle* pVFH, long offset, int origin) {
  ...
  switch ( origin ) {
    ...
    case VFSEEK_CUR: lOffset                     = pVFH->lStartOff + pVFH->lCurrentOffset + offset;
      if ( lOffset < pVFH->lStartOff ) { lOffset = pVFH->lStartOff; }
      if ( lOffset > pVFH->lEndOff ) { lOffset   = pVFH->lEndOff; }
      pVFH->lCurrentOffset                       = lOffset - pVFH->lStartOff;
      break;

    case VFSEEK_END: lOffset                     = pVFH->lEndOff + offset;
      if ( lOffset < pVFH->lStartOff ) { lOffset = pVFH->lStartOff; }
      if ( lOffset > pVFH->lEndOff ) { lOffset   = pVFH->lEndOff; }
      pVFH->lCurrentOffset                       = lOffset - pVFH->lStartOff;
      break;
