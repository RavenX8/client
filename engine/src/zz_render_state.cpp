...
zz_render_state::zz_render_state(void) :
  ...
  use_polygon_sorting( true ), // for now, not work correctly, 2004/04/23
  use_render_state( true ),    // for now, not work correctly, 2004/04/23
  use_lightmap( true ),
  use_vsync( true ),
  ...
  use_multipass( false ),
  current_pass( 0 ),
  fsaa_type( ZZ_FSAA_NONE ),
  clear_color( 0, 0, 0 ),
  cull_mode( ZZ_CULLMODE_CW ),
...
void zz_render_state::set_display_quality_level(int level) {
  ...
  switch ( level ) {
    case 5: // 최하급품질
      ...
      this->mipmap_level           = -1;
      this->mipmap_filter          = zz_texture_filter_type( 2 );
      this->min_filter             = zz_texture_filter_type( 2 );
      this->mag_filter             = zz_texture_filter_type( 2 );
      this->use_glow               = false;
      ...
      znzin->set_use_time_weight( false );
    ...
    case 4: // 하급품질
      ...
      this->use_glow_ztest         = false;
      this->use_alpha_fog          = false;
      znzin->set_use_time_weight( true );
      znzin->set_use_motion_interpolation( false );
      znzin->terrain_blocks->set_num_reuse( 500 );
      break;
    case 3: // 중급 품질
    ...
    case 1: // 최상급 품질(설정파일대로)
      ...
      znzin->terrain_blocks->set_num_reuse( 500 );
      break;
    case 0: this->texture_loading_scale = initial_state.texture_loading_scale;
