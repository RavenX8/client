...
zz_scene* zz_visible::scene = nullptr;

zz_runit::zz_runit() : mesh( nullptr ), material( nullptr ), light( nullptr ),
...
void zz_visible::render(bool recursive) {
  ...
  if ( (r->get_render_where() == ZZ_RW_SHADOWMAP) && !cast_shadow && !shadow_onoff ) { } else {
...
void zz_visible::render_runit(unsigned int runit_index) {
  ...
  if ( glow_type == ZZ_GLOW_NOTSET ) {
    if ( ru.glow_type == ZZ_GLOW_NOTSET ) {
      if ( mat->get_glow_type() != ZZ_GLOW_NONE ) {
        // if material has glow type
...
void zz_visible::build_mesh_minmax(vec3& mesh_min_out, vec3& mesh_max_out) {
  ...
  for ( unsigned int i = 0; i < num_runits; ++i ) {
    ...
    if ( i == 0 ) {
      // set initial min/max as the first mesh's
      mesh_min_out = mesh_min;
      mesh_max_out = mesh_max;
    } else {
      // compare min/max and save it
...
void zz_visible::reset_bvolume(void) {
  ...
  if ( !bv ) {
    // create bounding volume, if not have one
  ...
  switch ( bv_type ) {
    case ZZ_BV_SPHERE: bv->set_sphere( .5f * (mesh_max.x - mesh_min.x) ); // not correct
      break;
    case ZZ_BV_AABB: bv->set_aabb( .5f * (mesh_max - mesh_min) );
      break;
    case ZZ_BV_OBB: bv->set_obb( .5f * (mesh_max - mesh_min) );
      break;
    default:
...
const quat& zz_visible::get_rotation_world(quat& world_rotation) {
  ...
  while ( current != get_root() ) {
    // for case current is zz_manager
...
zz_mesh* zz_visible::get_mesh(unsigned int index) {
  return (index < num_runits) ? runits[index].mesh : nullptr;
}

zz_material* zz_visible::get_material(unsigned int index) {
  return (index < num_runits) ? runits[index].material : nullptr;
}

zz_light* zz_visible::get_light(unsigned int index) {
  return (index < num_runits) ? runits[index].light : nullptr;
}
...
void zz_visible::rotate_by_axis_facing_camera(const vec3& cam_pos_world, int rotation_axis_index, int direction_axis_index) {
  ...
  if ( parent && parent->is_a( ZZ_RUNTIME_TYPE(zz_visible) ) ) {
    ...
    // do not have parent visible node
...
bool                 zz_visible::test_intersection_sphere_moving(const zz_bounding_sphere& sphere, const vec3& prev_center, vec3* closest_center, float max_distance_square) {
  ...
  if ( distance2 > max_distance_square ) {
    // too far
...
void zz_visible::scene_refresh() {
  assert(scene);
  if ( inscene ) {
    // object should be in scene
    if ( _onode ) {
      // actually  in scene
      if ( get_root() == parent_node ) {
        scene->refresh( this );
      } else {
        // no need to be actually in scene
        scene->remove( this );
      }
    } else {
      // not actually in scene
      if ( get_root() == parent_node ) {
        scene->insert( this );
      } else {
        // do nothing
    ...
    // should be not in scene
    if ( _onode ) {
      // if actually in scene
      scene->remove( this );
    }
  }
}
  ...
  if ( !bv ) {
    ...
    switch ( bv_type ) {
      ...
      case ZZ_BV_AABB: {
        ...
        break;
      case ZZ_BV_NONE:
      default: get_position_world( minmax[0] );
        minmax[1] = minmax[0];
        break;
    }
  }
