...
void zz_model::render(bool recursive) {
  ...
  for ( imesh = 0; imesh < num_runits; ++imesh ) {
...
void zz_model::attach_motion(zz_motion* motion_to_attach) {
  ...
  if ( motion_to_attach == nullptr ) return;
...
bool zz_model::set_look_at(const vec3& there, bool immediate) // internally model's y-axis is front direction
{
  ...
  if ( fabs( cos_angle * ZZ_TO_DEG ) < MIN_DEGREE ) {
    // if it is close to to_vector   //주치 범위 0 - 180 
    ...
    // 절대 나올수 없는 조건
    if ( cross_vector.z < 0 ) {
      // increase direction with counter-clock wise
...
const quat&   zz_model::get_turned_initial_rotation_incremental(quat& rotation_out, zz_time time_diff_in) {
  ...
  if ( new_angle_diff < FABS(angle_diff) ) {
    // if angle difference is too small
    if ( angle_diff > 0 ) {
      // positive direction
...
void   zz_model::set_virtual_Transform(const vec4& pos) {
  ...
  world_inverseTM._14 = inv_pos.x;
  world_inverseTM._24 = inv_pos.y;
  world_inverseTM._34 = inv_pos.z;
  world_inverseTM._44 = 1.0f;
