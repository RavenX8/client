...
int loadCharacter(ZSTRING characterFileName) {
  ...
  zch.read_string( nullptr, noname );
...
int callInt(ZSTRING funcString) {
  znzin->script->do_script( nullptr, funcString );
...
zz_node_type* getNodeType(int iType) {
  switch ( iType ) {
    case ZZ_TYPE_MORPHER: return ZZ_RUNTIME_TYPE(zz_morpher);
    case ZZ_TYPE_SKELETON: return ZZ_RUNTIME_TYPE(zz_skeleton);
    case ZZ_TYPE_MODEL: return ZZ_RUNTIME_TYPE(zz_model);
    case ZZ_TYPE_PARTICLE: return ZZ_RUNTIME_TYPE(zz_particle_emitter);
    case ZZ_TYPE_TRAIL: return ZZ_RUNTIME_TYPE(zz_trail);
    case ZZ_TYPE_TERRAIN: return ZZ_RUNTIME_TYPE(zz_terrain_block);
    case ZZ_TYPE_OCEAN: return ZZ_RUNTIME_TYPE(zz_ocean_block);
    case ZZ_TYPE_VISIBLE: return ZZ_RUNTIME_TYPE(zz_visible);
    case ZZ_TYPE_ANIMATABLE: return ZZ_RUNTIME_TYPE(zz_animatable);
  }
  return nullptr;
};
...
void swapBuffers(void) {
  ...
  znzin->renderer->swap_buffers( nullptr );
  znzin->sleep();
...
int setModelDirectionByPosition(HNODE hModel, float fX, float fY) {
  ...
  model->set_look_at( vec3( fX * ZZ_SCALE_IN, fY * ZZ_SCALE_IN, 0 ), false /* b_immediate */ );
  return 1;
}
...
HNODE loadTerrainBlock(
  ...
  first_tex  = (firstmap) ? firstmap->get_texture( 0 ) : nullptr;
  second_tex = (secondmap) ? secondmap->get_texture( 0 ) : nullptr;
  light_tex  = (lightmap) ? lightmap->get_texture( 0 ) : nullptr;

  HNODE hMat = loadTerrainMaterial(
    nullptr,
    hTerrainShader,
    reinterpret_cast<HNODE>(first_tex),
    reinterpret_cast<HNODE>((first_tex != second_tex) ? second_tex : nullptr),
...
HNODE loadTerrainBlockRough(
  ...
  zz_texture* first_tex = (zz_texture *)loadTexture( nullptr, pTexturePath, 1 /* miplevel */, 1 /* use filter */ );
...
int               intersectRay2OBB(float* pOrigin, float* pDirection, float* pCenter, float* pRotation, float xLength, float yLength, float zLength) {
  ...
  obb.center.x = pCenter[0];
  obb.center.y = pCenter[1];
  obb.center.z = pCenter[2];
  rotation.x   = pRotation[0];
  rotation.y   = pRotation[1];
  rotation.z   = pRotation[2];
  rotation.w   = pRotation[3];
  ...
  origin.y    = pOrigin[1];
  origin.z    = pOrigin[2];
  direction.x = pDirection[0];
  direction.y = pDirection[1];
  direction.z = pDirection[2];
...
HNODE pickNode(int screenX, int screenY, float* pContactPointX, float* pContactPointY, float* pContactPointZ, float* pDistance) {
  ...
  zz_visible* cur_vis = nullptr;

  // info. for closest node
  float       min_distance;
  vec3        min_contact_point;
  zz_visible* min_vis = nullptr;
...
int getRay(
  ...
  *pOriginY    = origin.y;
  *pOriginZ    = origin.z;
  *pDirectionX = direction.x;
  *pDirectionY = direction.y;
  *pDirectionZ = direction.z;
...
float getWorldHeight(HNODE hVisible, float fPointX, float fPointY) {
  ...
  if ( vis->get_intersection_ray( origin, down_dir, contact_point, contact_normal, zz_mesh_tool::ZZ_SM_NEAREST ) ) {
    return contact_point.z * ZZ_SCALE_OUT;
  }
  return -ZZ_INFINITE;
}
...
HNODE loadOceanBlock(
  ...
  zz_mesh_ocean* mesh =
      (zz_mesh_ocean*)znzin->ocean_meshes->spawn( nullptr, ZZ_RUNTIME_TYPE(zz_mesh_ocean),
                                                  false /* do_load */ );
...
HNODE loadEffect(ZSTRING pEffectName, ZSTRING pEffectFilePath) {
  ...
  for ( i = 0; i < num_particles; i++ ) {
    ...
    // load particle
    HNODE part_node;
    HNODE part = loadParticle( nullptr, particle_path );
    if ( part ) {
      controlParticle( part, 1 ); // start
      if ( use_motion && strcmp( particle_motion_name, "NULL" ) ) {
        HNODE mot = loadMotion( nullptr, particle_motion_name, 1, ZZ_INTERP_LINEAR, ZZ_INTERP_LINEAR, 1.0f, 0 );
        zz_assert(mot);
        part_node = loadAnimatable( nullptr, NULL, mot, NULL, NULL );
      } else {
        part_node = loadVisible( nullptr, NULL, NULL, NULL );
      }
  ...
  for ( i      = 0; i < num_morph; i++ ) {
    ...
    HNODE hmorph = NULL;
    hmesh        = loadMesh( nullptr, morph_mesh_path );
    hmat         = loadColormapMaterial( nullptr, shader, morph_texture_path );
    if ( morph_motion_path ) {
      hmo = loadMotion( nullptr, morph_motion_path, 1, ZZ_INTERP_NONE, ZZ_INTERP_NONE, 1, 0 );
    }
    hmorph = loadMorpher( nullptr, hmesh, hmo, hmat, light );
    if ( hmorph ) {
      HNODE htm = loadAnimatable( nullptr, NULL, NULL, NULL, NULL );
      linkNode( htm, hmorph );
      linkNode( effect, htm );
...
int setDelayedLoad(int iDelayedLoad) {
  ...
  if ( iDelayedLoad == 2 ) {
    ...
    if ( (iDelayedLoad == 0) && (state->use_delayed_loading) ) {
      // auto flush at changing state(true->false)
      znzin->flush_delayed( true, true );
    }
...
int drawFontScript(HNODE hFont, int iX, int iY, ZSTRING pMessage) {
  ...
  zz_assert(pMessage);
  zz_assert(znzin->renderer->is_a(ZZ_RUNTIME_TYPE(zz_renderer_d3d)));
  if ( ((zz_renderer_d3d*)znzin->renderer)->sprite_began() ) {
    // is in begin/end scene section
...
int getSpriteTextureColor(HNODE            hTexture,
  ...
  D3DLOCKED_RECT rect;
  d3d_tex->LockRect( 0, &rect, nullptr, D3DLOCK_READONLY );
...
int openFileSystem(ZSTRING pIndexFileName) {
  ...
  if ( zz_system::set_pkg_system( pIndexFileName ) == nullptr ) return 0;
#endif
  return 1;
}
...
float getWorldObjectHeightInScene(float fWorldX, float fWorldY, float fDefaultHeight) {
  ...
  bool                    first = true;
  zz_visible*             vis   = nullptr;
  const zz_bounding_aabb* aabb  = nullptr;
...
void                      DrawOBBBox(float* q, float* v, float xx, float yy, float zz) {
  ...
  min_vec[0] = -xx * ZZ_SCALE_IN;
  min_vec[1] = -yy * ZZ_SCALE_IN;
  min_vec[2] = -zz * ZZ_SCALE_IN;
  max_vec[0] = xx * ZZ_SCALE_IN;
  max_vec[1] = yy * ZZ_SCALE_IN;
  max_vec[2] = zz * ZZ_SCALE_IN;
...
void UserObserverCamera(int bUse) {
  ...
  switch ( bUse ) {
    case 0: if ( znzin->camera_sfx.get_play_onoff() )
        znzin->camera_sfx.play_onoff();
      break;

    case 1: if ( !(znzin->camera_sfx.get_play_onoff()) )
        znzin->camera_sfx.play_onoff();
...
void               DrawCameraMovingPath(HNODE hMotion, float vPosition[3], float OutPosition[3], float OutRotation[4]) {
  ...
  mat._33 = -direction.z;
  mat._43 = 0.0f;
  mat._14 = eye.x + position.x;
  mat._24 = eye.y + position.y;
  mat._34 = eye.z + position.z;
  mat._44 = 1.0f;
  ...
  OutPosition[0] = buffer_position.x;
  OutPosition[1] = buffer_position.y;
  OutPosition[2] = buffer_position.z;
  OutRotation[0] = buffer_rotation.x;
  OutRotation[1] = buffer_rotation.y;
  OutRotation[2] = buffer_rotation.z;
  OutRotation[3] = buffer_rotation.w;
...
void InputMovingCameraMatrix(float vPosition[3], float vRotation[4]) {
  ...
  position.x = vPosition[0];
  position.y = vPosition[1];
  position.z = vPosition[2];
