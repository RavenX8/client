...
zz_motion::zz_motion(void) : fps( 0 ), num_frames( 0 ), do_loop( true ), interp_interval( 0 ),
                             owner( nullptr ), initial_position( vec3_null ),
                             initial_rotation( quat_id ),
                             num_channels( 0 ),
                             channels( nullptr ) {}
...
bool zz_motion::load(const char* file_name, float scale_in_load) {
  ...
  zz_channel* new_channel = nullptr;
  uint32      channel_type;
  ...
  for ( channel_index = 0; channel_index < num_channels; ++channel_index ) {
    ...
    switch ( channel_type ) {
      ...
      case ZZ_CTYPE_SCALE: new_channel = zz_new zz_channel_x;
        break;
      case ZZ_CTYPE_UV0:
      case ZZ_CTYPE_UV1:
      case ZZ_CTYPE_UV2:
      case ZZ_CTYPE_UV3: new_channel = zz_new zz_channel_xy;
        break;
      case ZZ_CTYPE_POSITION:
      case ZZ_CTYPE_NORMAL: new_channel = zz_new zz_channel_position;
        break;
      case ZZ_CTYPE_ROTATION: new_channel = zz_new zz_channel_rotation;
        break;
      default:
  ...
  for ( channel_index = 0; channel_index < num_channels; ++channel_index ) {
    switch ( channels[channel_index]->get_channel_format() ) {
      case ZZ_CFMT_X: format_size += 4;
        break;
      case ZZ_CFMT_XY: format_size += 8;
        break;
      case ZZ_CFMT_XYZ: format_size += 12;
        break;
      case ZZ_CFMT_WXYZ: format_size += 16;
        break;
    }
  }
...
bool zz_motion::update_mesh(zz_mesh* mesh, int current_frame, int num_verts, float& alpha) {
  ...
  for ( unsigned int index = 0; index < num_channels; ++index ) {
    ...
    switch ( channel->channel_type ) {
      case ZZ_CTYPE_POSITION: mesh->set_pos( referid,
                                             static_cast<zz_channel_position*>(channel)->positions[current_frame] );
        break;
      case ZZ_CTYPE_NORMAL: mesh->set_normal( referid,
                                              static_cast<zz_channel_position*>(channel)->positions[current_frame] );
        break;
      case ZZ_CTYPE_ALPHA: alpha = static_cast<zz_channel_x*>(channel)->floats[current_frame];
        mesh->alpha              = alpha;
        use_alpha_animation      = true;
        break;
      case ZZ_CTYPE_UV0: mesh->set_uv( referid, 0, static_cast<zz_channel_xy*>(channel)->xys[current_frame] );
        break;
      case ZZ_CTYPE_UV1: mesh->set_uv( referid, 1, static_cast<zz_channel_xy*>(channel)->xys[current_frame] );
        break;
      case ZZ_CTYPE_UV2: mesh->set_uv( referid, 2, static_cast<zz_channel_xy*>(channel)->xys[current_frame] );
        break;
      case ZZ_CTYPE_UV3: mesh->set_uv( referid, 3, static_cast<zz_channel_xy*>(channel)->xys[current_frame] );
        break;
    }
  }
