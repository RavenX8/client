...
zz_vfs::zz_vfs() : real_filesystem_( nullptr ), data_( nullptr ), status_( ZZ_VFS_INI ) {}

zz_vfs::zz_vfs(const char* filename, const zz_vfs_mode mode) : real_filesystem_( nullptr ), data_( nullptr ), status_( ZZ_VFS_INI ) {
  if ( filename )
    open( filename, mode );
}
...
char* zz_vfs::read_string(char* string_out, uint32 size) {
  if ( 0 == size ) {
    ...
    assert(real_filesystem_);
    if ( real_filesystem_->read_( string_out, size ) == 0 ) {
      // not a explicit null-terminated string
      set_status( ZZ_VFS_EOF );
    }
  }
  return string_out;
...
char* zz_vfs::read_string_without_whitespace(char* string_out,
  ...
  while ( real_filesystem_->read_char( c ) != '\0' &&
          real_filesystem_->get_status() != ZZ_VFS_EOF ) {
    if ( c == '\"' ) {
      // if double quote
...
void zz_vfs::write_string(const char* string_in, uint32 size) {
  if ( !string_in ) return;
  if ( size == 0 ) {
    // calculate string length manually assuming null-terminated string
