...
zz_renderer*      zz_material::s_renderer    = nullptr;
zz_render_state*  zz_material::s_renderstate = nullptr;
...
bool zz_material::push_texture(const char* filename) {
  if ( !filename ) {
    _push_texture( (zz_texture*)nullptr );
    return false;
  }
...
bool zz_material::set_texture(int index, const char* filename) {
  ...
  if ( !filename ) {
    return _set_texture( index, (zz_texture*)nullptr );
  }
  zz_texture* tex = (zz_texture*)znzin->textures->find( filename );
...
bool zz_material::set_texture(int index, int width, int height) {
  ...
  zz_texture* tex = (zz_texture*)znzin->textures->spawn( nullptr, ZZ_RUNTIME_TYPE(zz_texture), false /* not to autoload */ );

  assert(tex);
  tex->set_property( nullptr, width, height, true /* dynamic */, 1 /* miplevels */, false /* use_filter */,
