...
LPDIRECT3DDEVICE9       zz_trail::device       = nullptr;
LPDIRECT3DVERTEXBUFFER9 zz_trail::vbuffer      = nullptr;
...
zz_trail::zz_trail() :
  ...
  local_ep( 0, 0, 0 ),
  world_length( 0.0f ),
  duration( 0 ), trail_color( 1, 1, 1, 1 ),
  num_segments( 1 ),
...
bool zz_trail::unload() {
  ...
  if ( texture ) {
    ...
    texture = nullptr; // invalidate texture
  }
...
bool zz_trail::invalidate_device_objects() {
  ...
  if ( vb_ref_count == 0 ) {
    ...
    device = nullptr; // invalidate device handle
  }
  res.make_uncreated();
  return true;
}
