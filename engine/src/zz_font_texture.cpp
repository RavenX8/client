...
void zz_font_texture::create(zz_font& font) {
  ...
  switch ( font.get_outline_type() ) {
    case zz_font::OUTLINE_TYPE_NONE: margin = 0;
      break;
    case zz_font::OUTLINE_TYPE_SIMPLE: margin = 1;
      break;
    case zz_font::OUTLINE_TYPE_THICKSHADOW: margin = 4;
      break;
    case zz_font::OUTLINE_TYPE_THINSHADOW: margin = 2;
      break;
  }
...
bool zz_font_texture::draw_text_texture(zz_font& font, zz_font_text& text) {
  ...
  if ( !(*it).is_render() && !(*it).is_rerender() ) {
    // not rendered
...
void   zz_font_texture::update_font_texture(zz_font& font) {
  ...
  for ( zz_element_type::iterator it( texts.begin() ), it_end( texts.end() ); it != it_end; ++it ) {
    ...
    } else if ( elm->is_render() ) {
      // still render state(it means that it is not rendered in this frame), so we should erase it
...
bool  zz_font_texture::cacheable(zz_font_text& text) {
  ...
  if ( text.format & DT_VCENTER ) {
    // if vcentor enabled, do not render to texture
    return false;
  }
  if ( text_width > tex_width ) {
    return false;
  }
