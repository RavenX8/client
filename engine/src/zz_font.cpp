  ...
  if ( dynamic ) {
    ...
    while ( it != font_texts.end() ) {
      if ( (*it).dynamic == dynamic ) {
        ...
        // for not considered node, skip it
...
bool zz_font::get_bold() const {
  return ((font_style & FONT_STYLE_BOLD) != 0);
}

bool zz_font::get_italic() const {
  return ((font_style & FONT_STYLE_ITALIC) != 0);
}
...
void zz_font::set_bold(bool bold) {
  if ( bold ) {
    font_style = zz_font_style( font_style | FONT_STYLE_BOLD );
  } else {
    font_style = zz_font_style( font_style - (font_style & FONT_STYLE_BOLD) );
  }
}

void zz_font::set_italic(bool italic) {
  if ( italic ) {
    font_style = zz_font_style( font_style | FONT_STYLE_ITALIC );
  } else {
    font_style = zz_font_style( font_style - (font_style & FONT_STYLE_ITALIC) );
  }
}
...
void zz_font::draw_text(zz_font_text& text) {
  ...
  if ( ftex && r->get_state()->use_font_texture ) {
    // if has font texture
    if ( ftex->cacheable( text ) ) {
      // if we can hold text in texture
      zz_font_text* text_found = nullptr;
      if ( ftex->find_text( text, &text_found ) ) {
        // search existing one with same text
        text.tex_rect = text_found->tex_rect; // copy tex_rect as new text does not know texture position 
        if ( ftex->draw_text_texture( *this, text ) ) {
          // draw text within texture
#if defined(_DEBUG)
  ...
  switch ( get_outline_type() ) {
    case OUTLINE_TYPE_NONE: draw_text_direct( text );
      break;
    case OUTLINE_TYPE_SIMPLE: draw_text_prim_outline_simple( text );
      break;
    case OUTLINE_TYPE_THINSHADOW: draw_text_prim_shadow_simple( text );
      break;
    default:
...
bool     zz_font::get_rect(zz_rect& rect_out, int ix, int iy, const char* msg) {
  ...
  switch ( font_outline_type ) {
    case OUTLINE_TYPE_SIMPLE: margin = 1;
      break;
    case OUTLINE_TYPE_THICKSHADOW: margin = 4;
      break;
    case OUTLINE_TYPE_THINSHADOW: margin = 2;
      break;
  }
