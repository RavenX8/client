...
void zz_bone::apply_channel_by_time(zz_time time, zz_motion* motion, int custum_fps, float blend_weight) {
  ...
  if ( rotation_channel_index >= 0 ) {
    if ( !forced_rotation_onoff ) {
      ...
      dx_quat.x = forcde_rotation_quat.x;
      dx_quat.y = forcde_rotation_quat.y;
      dx_quat.z = forcde_rotation_quat.z;
      dx_quat.w = forcde_rotation_quat.w;
      ...
      rotation.x = dx_quat.x;
      rotation.y = dx_quat.y;
      rotation.z = dx_quat.z;
      rotation.w = dx_quat.w;
...
void          zz_bone::input_forced_rotation_mode(vec4& position) {
  ...
  buffer_v.x = local_pos.x;
  buffer_v.y = local_pos.y;
  buffer_v.z = 0.0f; //local_pos.z;
