...
bool zz_texture::delete_device_objects() {
  if ( !res.is_delete_device_objects() ) {
    // skip default
    return true;
  }
...
bool zz_texture::restore_device_objects() {
  if ( !res.is_restore_device_objects() ) {
    // skip managed, systemmem, scratch
    return true;
  }
...
bool zz_texture::invalidate_device_objects() {
  if ( !res.is_invalidate_device_objects() ) {
    // skip managed, systemmem, scratch
    return true;
  }
...
bool          zz_texture::set_path(const char* path_in) {
  ...
  // not to rename old
  zz_assertf( file_name.get() == nullptr, "rename [%s] failed", file_name.get() );
