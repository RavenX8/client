...
void zz_scene_octree::render_bterrain() {
  ...
  if ( znzin->get_rs()->use_zonly_terrain ) {
    // zonly pass
...
void zz_scene_octree::render_shadowmap() {
  ...
  if ( lcam ) {
    ...
    if ( !model ) {
      // if lcam is center(no target), then update center_pos to lcam.
...
void          zz_scene_octree::update_distance(const vec3& cam_pos) {
  ...
  alpha_fog_start        = (znzin->get_rs()->alpha_fog_start > FOG_START_BACK) ? znzin->get_rs()->alpha_fog_start - FOG_START_BACK : 0; // CAUTION hard-coded 1000
  ...
  for ( zz_vislist::const_iterator it              = infrustum_nodes.begin(), it_end = infrustum_nodes.end();
    ...
    if ( node_type == ZZ_RUNTIME_TYPE(zz_terrain_block) ) {
    ...
    if ( node_type == ZZ_RUNTIME_TYPE(zz_particle_emitter) ) {
      particle_nodes.push_back( vis );
      continue;
    }
    if ( node_type == ZZ_RUNTIME_TYPE(zz_morpher) ) {
      // we assume morpher is a type of particle
      //particle_nodes.push_back(vis);
      //continue;
...
void             zz_scene_octree::update_transform(zz_time diff_time) {
  zz_visible*    vis;
  child_type*    vis_child = (child_type *)(&znzin->visibles->get_children());
...
void zz_scene_octree::update_time(zz_time diff_time) {
  ...
  // ocean_block
  vis_child = (child_type *)(&znzin->ocean_blocks->get_children());
...
void zz_scene_octree::draw_scene_obb2() {
  if ( scene_obb2_number > 0 ) {
    ...
    for ( int        i = 0; i < scene_obb2_number; i += 1 ) {
      ...
      model_m._11 = rotation_m._11;
      model_m._21 = rotation_m._21;
      model_m._31 = rotation_m._31;
      model_m._41 = 0.0f;
      model_m._12 = rotation_m._12;
      model_m._22 = rotation_m._22;
      model_m._32 = rotation_m._32;
      model_m._42 = 0.0f;
      model_m._13 = rotation_m._13;
      model_m._23 = rotation_m._23;
      model_m._33 = rotation_m._33;
      model_m._43 = 0.0f;
      model_m._14 = scene_obb2[i].center.x;
      model_m._24 = scene_obb2[i].center.y;
      model_m._34 = scene_obb2[i].center.z;
      model_m._44 = 1.0f;

      min_vec[0] = -scene_obb2[i].xlength;
      max_vec[0] = scene_obb2[i].xlength;
      min_vec[1] = -scene_obb2[i].ylength;
      max_vec[1] = scene_obb2[i].ylength;
      min_vec[2] = -scene_obb2[i].zlength;
      max_vec[2] = scene_obb2[i].zlength;
...
void           dump_view_sub(const char* category, int index, zz_visible* vis) {
  ...
  for ( runit = 0; runit < vis->get_num_runits(); ++runit ) {
    ...
    tex       = (mat) ? mat->get_texture( 0 ) : nullptr;
