...
LPDIRECT3DDEVICE9       zz_particle_emitter::m_pd3dDevice        = nullptr;
LPDIRECT3DVERTEXBUFFER9 zz_particle_emitter::m_vbParticles       = nullptr;
LPDIRECT3DVERTEXBUFFER9 zz_particle_emitter::m_vbParticles_point = nullptr;
int                     zz_particle_emitter::vb_ref_count        = 0;

zz_particle_emitter::zz_particle_emitter() : zz_animatable(), pos_channel( nullptr ) {
...
bool      zz_particle_emitter::restore_device_objects() {
  ...
  if ( m_vbParticles_point == nullptr ) {
...
void zz_particle_emitter::render(bool recursive) {
  //	zz_assert(res.get_created());             //test  particel error ��� �ּ�ó��..
  assert(m_vbParticles || m_vbParticles_point);
...
void zz_particle_emitter::update_time(bool recursive, zz_time diff_time) {
  ...
  if ( apply_motion() ) {
    // do apply motion no relation with motion_controller
