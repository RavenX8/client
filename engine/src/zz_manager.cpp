...
zz_node*   zz_manager::spawn(const char* baby_name, zz_node_type* node_type_to_spawn, bool do_load) {
  ...
  if ( num_reuse > 0 ) {
    if ( !exit_line.empty() ) {
      // if there are one more exiting items.
      new_born = exit_line.pop(); // get it!
      if ( new_born && !baby_name ) {
        // if has no name
        new_born->set_name( nullptr ); // reset as unique name to clear old name
      }
    }
  }

  if ( !new_born ) {
    // if not found existing instance, then create new instance.
  ...
  if ( baby_name ) {
    // if has specified name, rename to it.
...
void zz_manager::load(zz_node* node) {
  ...
  if ( !znzin->get_rs()->use_delayed_loading || (node->get_load_weight() == 0) ) {
    // from zz_waiting_line::flush()
    // try loading
    if ( node->load() ) {
      return;
    }
...
zz_node*   zz_manager::find_or_spawn(const char* baby_name, zz_node_type* node_type_to_spawn) {
  ...
  if ( baby_name ) {
    // omitting spawn_name is possible
...
bool zz_manager::kill(zz_node* node) {
  ...
  if ( !children.empty() ) {
    // replace _current to the first child
  ...
  if ( node_ref_count != 1 ) {
    // if still refered by someone, then just call release().
  ...
  child_type*    child = (child_type *)(&node->get_children());
  ...
  try {
    while ( node && (t > time_weight) ) {
      ...
      if ( entrance_line.flush_node( node ) ) {
        ...
        // not loaded, so reinsert to front
        entrance_line.pop();
