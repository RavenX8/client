...
zz_material_terrain::e_material_state zz_material_terrain::color_state( MS_NONE );
zz_material_terrain::e_material_state zz_material_terrain::alpha_state( MS_NONE );
  ...
  if ( !s_renderstate->use_multipass ) {
    ...
    if ( pass == 0 ) {
      ...
      if ( use_lightmap ) {
        ...
        } else if ( s_renderstate->lightmap_blend_style == ZZ_TOP_MODULATE2X ) {
          // default MODULATE2X
...
bool          zz_material_terrain::set(int pass) {
  ...
  firstmap  = (shader_format & SHADER_FORMAT_FIRSTMAP) ? textures[0] : nullptr;
  secondmap = (shader_format & SHADER_FORMAT_SECONDMAP) ? textures[1] : nullptr;
  lightmap  = (shader_format & SHADER_FORMAT_LIGHTMAP) ? textures[2] : nullptr;
