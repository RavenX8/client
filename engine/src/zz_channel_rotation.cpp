...
void update(quat& q_cur, quat& q_new, bool interpolate) {
  if ( interpolate ) {
    ...
    if ( theta_d2 > threshold ) {
      ...
      if ( theta_d2 > ZZ_PI * .5f ) {
        // do opposite direction, if exceeds pi/2
...
void    zz_channel_rotation::get_by_time(zz_time time, int fps, void* data_pointer) {
  ...
  switch ( interp_style ) {
    ...
    case ZZ_INTERP_SLERP: time_to_frame( time, start_frame, next_frame, ratio, (int)num_rotations, fps );
      new_rot = qslerp( rotations[start_frame], rotations[next_frame], ratio );
      break;
    case ZZ_INTERP_SQUAD: time_to_frame( time, start_frame, next_frame, ratio, (int)num_rotations, fps );
