...
zz_motion_mixer*   zz_animatable::blend_motion(zz_motion* motion_arg1, zz_motion* motion_arg2) {
  ...
  // create motion
  blended_motion = (zz_motion_mixer *)znzin->motions->spawn( nullptr, ZZ_RUNTIME_TYPE(zz_motion_mixer) );
...
void zz_animatable::attach_motion(zz_motion* motion_to_attach) {
  ...
  if ( !motion ) {
    ...
    motion_controller.set_motion( nullptr );
    return;
  }
...
bool zz_animatable::apply_motion(void) {
  ...
  for ( int channel_index = 0; channel_index < num_channels; channel_index++ ) {
    ...
    switch ( channel_type ) {
      case ZZ_CTYPE_POSITION: motion->get_channel_data( channel_index, time, &this->position, fps );
        to_update = true;
        break;
      case ZZ_CTYPE_ROTATION: motion->get_channel_data( channel_index, time, &this->rotation, fps );
        to_update = true;
        break;
      case ZZ_CTYPE_SCALE: motion->get_channel_data( channel_index, time, &scale_x, fps );
        scale.set( scale_x, scale_x, scale_x );
        to_update = true;
        break;
      case ZZ_CTYPE_ALPHA: motion->get_channel_data( channel_index, time, &this->visibility, fps );
...
void zz_animatable::update_animation(bool recursive, zz_time diff_time) {
  if ( motion_controller.is_playing() ) {
    ...
    if ( apply_motion() ) {
      // apply_motion is called only if PLAY state and PLAY->STOP state
