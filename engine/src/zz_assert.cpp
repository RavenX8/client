...
const char* zz_assert_shared_struct::s_current_file = nullptr;
...
const char* popup_dlg_msg = nullptr;

int zz_popup_message(const char* msg) {
  return MessageBox( nullptr, msg, "engine zz_assert failed", MB_OK | MB_TOPMOST );
}
...
int           _zz_assert(const char* msg, const char* filename, int linenum) {
  ...
  if ( name_num.find( std::string( onlyname.get() ) ) != name_num.end() ) {
    // found
    std::vector<int>& numlist = name_num[onlyname.get()];
    if ( std::find( numlist.begin(), numlist.end(), linenum ) != numlist.end() ) {
      // found
  ...
  if ( zz_popup_message( buffer ) ) {
    // user selected "ignore" button, log into history
    // insert into history
