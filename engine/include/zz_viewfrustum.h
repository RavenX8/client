...
public:
  ...
  zz_viewfrustum() : np( vec4_null ), fp( vec4_null ), lp( vec4_null ), rp( vec4_null ), tp( vec4_null ), bp( vec4_null ) {}

  zz_viewfrustum(vec4 np_in, vec4 fp_in, vec4 lp_in, vec4 rp_in, vec4 tp_in, vec4 bp_in) : np( np_in ), fp( fp_in ), lp( lp_in ), rp( rp_in ), tp( tp_in ), bp( bp_in ) {}
