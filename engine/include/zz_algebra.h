...
struct mat4 {
  ...
  mat4(const float& f0, const float&  f1, const float&  f2, const float&  f3,
       ...
       const float& f12, const float& f13, const float& f14, const float& f15) : a00( f0 ), a10( f4 ), a20( f8 ), a30( f12 ),
...
quat& axis_to_quat(quat&           q, const vec3& a, float        phi);
mat3& quat_2_mat(mat3&             M, const quat& q);
quat& mat_2_quat(quat&             q, const mat3& M);
quat& mat_2_quat(quat&             q, const mat4& M);
quat& euler_2_quat_d3d(quat&       q, float       yaw, float  pitch, float  roll); // by zho
quat& euler_2_quat_ogl(quat&       q, float       yaw, float  pitch, float  roll); // by zho
...
const float array16_scale_bias[] = {
  .5f, .0f, .0f, .0f,
  .0f, .5f, .0f, .0f,
  .0f, .0f, .5f, .0f,
  .5f, .5f, .5f, 1.0f
...
// Computes u = u * s
extern vec3& scale(vec3& u, float s);
extern vec4& scale(vec4& u, float s);
...
mat4& frustum(mat4& M, float l, float r, float b,
              float t, float n, float f);

mat4& perspective(mat4& M, float fovy, float aspect, float n, float f);

mat4& orthogonal(mat4& M, float w, float h, float zn, float zf);
...
inline float lerp(float t, float a, float b) { return a * (1.0f - t) + t * b; }
...
mat4& frustum_d3d(mat4& M, float l, float r, float b,
                  float t, float n, float f);
