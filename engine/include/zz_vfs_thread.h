...
public:
  ...
  struct zz_item {
    ...
    zz_item(const char* filename_in = nullptr) : state( STATE_FILL ), filename( filename_in ), buffer( nullptr ), filesize( 0 ), refcount( 0 ) { }
...
public:
  ...
  // Thread procedure
  unsigned thread_proc(void) override;
  ...
  void* read(zz_item_it handle, uint32* psize = nullptr, bool forcing = false);
