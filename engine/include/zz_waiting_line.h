...
public:
  ...
  zz_waiting_line(zz_usage usage_in = FOR_NONE, size_t max_size_in = 0) : _usage( usage_in ), _max_size( max_size_in ) {}
  ~zz_waiting_line() {}
  ...
  {
    typename zz_line_list::iterator it;
...
T* zz_waiting_line<T>::back() {
  if ( _line.empty() ) return nullptr;
  return _line.back();
}

template <class T>
void zz_waiting_line<T>::set(zz_usage usage_in, size_t max_size_in) {
  if ( (_usage != usage_in) && (_usage != FOR_NONE) ) {
...
bool                              zz_waiting_line<T>::remove(T* node) {
  typename zz_line_list::iterator it, it_end;
