...
protected:
  ...
  void get_local_center(vec3& local_center_out, const vec3& mesh_min, const vec3& mesh_max) override;

  void update_blink(zz_time diff_time);

  // get children scene_min/max in world coordsys recursively
  void expand_scene_minmax(vec3&        scene_min_out, vec3& scene_max_out) override;
...
public:
  ...
  const quat& get_com_rotation_world(void) override;

  const vec3& get_com_position_world(void ) override;
  ...
  bool get_intersection_ray(const vec3& ray_origin, const vec3& ray_direction, vec3& contact_point, vec3& contact_normal, zz_mesh_tool::zz_select_method select_method) override;
  ...
  void update_transform(zz_time diff_time) override;

  vec3& get_dummy_position_world(vec3& world_pos, int dummy_index);

  void set_rotation(const quat& rotation_to_set) override;

  float get_height(void) override; // get total height with children visibles
  ...
  void link_child(zz_node*   node_child) override;
  void unlink_child(zz_node* node_child) override;
  ...
  void set_seethru_recursive(float      seethru_in) override;
  void set_visibility_recursive(float   visibility_in) override;
  void set_lighting_recursive(zz_light* light) override;
  ...
  void set_glow_recursive(zz_glow_type glow_type_in, const vec3& color_in) override;
