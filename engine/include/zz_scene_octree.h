...
public:
  ...
  void clear() override;
  void build() override; // build scene array
  void insert(zz_visible*  vis) override;
  void remove(zz_visible*  vis) override;
  void refresh(zz_visible* vis) override;
  void update(zz_time      diff_time) override;
  void update_object_sort() override;
  void update_init_list() override;
  ...
  void           input_scene_sphere(float x, float y, float z, float r) override;
  void           draw_scene_sphere() override;
  void           reset_scene_sphere() override;

  int                    scene_obb_number;
  const zz_bounding_obb* scene_obb[MAX_SCENE_OBB];
  void                   input_scene_obb(const zz_bounding_obb* obb) override;
  void                   draw_scene_obb() override;
  void                   reset_scene_obb() override;

  int          scene_aabb_number;
  aabb_element scene_aabb[MAX_SCENE_AABB];
  void         input_scene_aabb(float min[3], float max[3], DWORD color) override;
  void         draw_scene_aabb() override;
  void         reset_scene_aabb() override;

  int              scene_cylinder_number;
  cylinder_element scene_cylinder[MAX_SCENE_CYLINDER];
  void             input_scene_cylinder(float x, float y, float z, float length, float r) override;
  void             draw_scene_cylinder() override;
  void             reset_scene_cylinder() override;

  int          scene_line_number;
  line_element scene_line[MAX_SCENE_LINE];
  void         input_scene_line(vec3& vec1, vec3& vec2) override;
  void         draw_scene_line() override;
  void         reset_scene_line() override;

  int          scene_obb2_number;
  obb2_element scene_obb2[MAX_SCENE_OBB2];
  void         input_scene_obb2(vec3& center, quat& rotation, float xlength, float ylength, float zlength) override;
  void         draw_scene_obb2() override;
  void         reset_scene_obb2() override;

  axis_element scene_axis[MAX_SCENE_AXIS];
  int          scene_axis_number;
  void         input_scene_axis(zz_visible* vis, float size) override;
  void         draw_scene_axis() override;
  void         reset_scene_axis() override;
  ...
  void increase_num_glows() override {
    ++num_glows;
  }

  int get_num_glows() override {
    return num_glows;
  }
