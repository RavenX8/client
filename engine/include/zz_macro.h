/** 
 * @file zz_macro.h
 * @brief #define macroes are here.
 * @author Jiho Choi(zho@popsmail.com)
 * @version 2.0
 * @date    2001.08.23-
 *
 * $Header: /engine/include/zz_macro.h 1     03-11-30 8:05p Zho $
 * $Author: Zho $
 * $Date: 03-11-30 8:05p $
 * $Revision: 1 $
 * $History: zz_macro.h $
 * 
 * *****************  Version 1  *****************
 * User: Zho          Date: 03-11-30   Time: 8:05p
 * Created in $/engine/include
 * 
 * *****************  Version 1  *****************
 * User: Zho          Date: 03-11-30   Time: 7:57p
 * Created in $/engine_1/include
 * 
 * *****************  Version 1  *****************
 * User: Zho          Date: 03-11-30   Time: 7:53p
 * Created in $/engine/engine/include
 * 
 * *****************  Version 1  *****************
 * User: Zho          Date: 03-11-30   Time: 7:47p
 * Created in $/engine/msvc71/include
 * 
 * *****************  Version 1  *****************
 * User: Zho          Date: 03-11-30   Time: 6:19p
 * Created in $/Engine/BUILD/msvc71/msvc71.root/msvc71/include
 * 
 * *****************  Version 1  *****************
 * User: Zho          Date: 03-11-30   Time: 6:08p
 * Created in $/Engine/INCLUDE
 * 
 * *****************  Version 3  *****************
 * User: Zho          Date: 03-11-28   Time: 2:55p
 * Updated in $/znzin11/engine/include
 * cleanup #include structure
 */

#ifndef __ZZ_MACRO_H__
#define __ZZ_MACRO_H__

#define ZZ_NUM_FILENAME     255 //!< maximum length of filename
#define ZZ_NUM_NODENAME     255 //!< maximum length of node name
#define ZZ_NUM_TEX_A_MESH     7 //!< number of texture per a mesh
#define ZZ_NUM_MATERIAL_MAPS 25 //!< number of maps in the material

#endif __ZZ_MACRO_H__
