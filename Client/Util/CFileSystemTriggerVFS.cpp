...
CFileSystemTriggerVFS::CFileSystemTriggerVFS(void) : m_pFileHandle( nullptr ), m_hVFile( nullptr ) {
  m_pData = nullptr;
  m_iSize = 0;
};
...
int CFileSystemTriggerVFS::Read(void* lpBuf, unsigned int nCount) {
  ...
  if ( m_pFileHandle == nullptr || lpBuf == nullptr ) {
    return 0;
  }
...
bool CFileSystemTriggerVFS::Seek(long lOff, unsigned int nFrom) {
  ...
  switch ( nFrom ) {
    case FILE_POS_SET: iResult = vfseek( m_pFileHandle, lOff, SEEK_SET );
      break;
    case FILE_POS_CUR: iResult = vfseek( m_pFileHandle, lOff, SEEK_CUR );
      break;
    case FILE_POS_END: iResult = vfseek( m_pFileHandle, lOff, SEEK_END );
      break;
  }
...
int CFileSystemTriggerVFS::GetSize() {
  if ( m_pFileHandle == nullptr || m_strFileName.empty() )
    return 0;
