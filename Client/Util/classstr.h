...
#ifdef	_DEBUG
	if ( !m_bInit ) {
		::MessageBox (NULL, (LPTSTR)"must call CStr::Init () !!!", (LPTSTR)"Error", MB_OK);
		return NULL;
	}
#endif
...
public :
  ...
  //void Set ( const char *szStrIn );
  char* Get(void       ) const { return m_pStr; }
  void  Del(void       );
  ...
  CStrVAR& operator=(const char*    rhs);
  CStrVAR& operator=(const CStrVAR& rhs);
