...
short CSkill::GetSkillIndex() {
  ...
  switch ( m_iSkillSlotType ) {
    case SKILL_SLOT_MAIN: nSkillIDX = g_pAVATAR->m_Skills.m_nSkillINDEX[m_iSkillSlot];
      break;
    case SKILL_SLOT_CLAN: nSkillIDX = m_nClanSkillIndex;
      break;
    default: break;
  }
...
void CSkill::DrawIcon(int iX, int iY) {
  ...
  if ( CCountry::GetSingleton().IsUseItemDelayNewVersion() ) {
    ...
    if ( this->m_iSkillDelayTimer > 0 ) {
      ...
      CReloadProcess* pReloadProcess = g_itMGR.GetReloadProcess();
      pReloadProcess->Draw( iX, iY, IMAGE_RES_SKILL_ICON, nIconNo, fProcessRate );
    } else { pDrawObj->Draw( iX, iY, IMAGE_RES_SKILL_ICON, nIconNo ); }
...
/*static*/
int CSkillManager::GetSkillTargetType(int iSkillIndex) {
  switch ( SKILL_TYPE( iSkillIndex ) ) {
    ...
    case SKILL_ACTION_RESURRECTION: return SKILL_TARGET_OBJECT;
...
bool CSkillManager::ActionSkill(short nSkillIdx, CObjCHAR* pSrc, CObjCHAR* pTarget, D3DXVECTOR3 PosTo, bool bTargetPos) {
  ...
  switch ( iSkillActionType ) {
    ...
    case SKILL_CREATE_WINDOW: break;

    case SKILL_ACTION_IMMEDIATE: break;
...
bool CSkillManager::CheckConditionForFireSkill(int iSkillIDX, int iCurrentTarget) {
  /// 스킬 사용 조건( 타겟팅 ) 체크..
  if ( CheckCastingTarget( iSkillIDX, g_pAVATAR, iCurrentTarget ) == false ) {
    g_itMGR.AppendChatMsg( CStr::Printf( "%s[ %d ]", STR_NOTIFY_05, GetSkillTargetType( iSkillIDX ) ), IT_MGR::CHAT_TYPE_SYSTEM );
    return false;
  }

  /// 타겟 필터링 검사	
  if ( CheckCastingTargetFilter( iSkillIDX, g_pAVATAR, iCurrentTarget ) == false ) {
    g_itMGR.AppendChatMsg( CStr::Printf( "%s[ %d ]", STR_NOTIFY_05, GetSkillTargetType( iSkillIDX ) ), IT_MGR::CHAT_TYPE_SYSTEM );
    return false;
  }

  /// 소환몹 조건 체크
  if ( CheckSummonCapacity( iSkillIDX, g_pAVATAR, iCurrentTarget ) == false ) {
    g_itMGR.AppendChatMsg( STR_CANT_SUMMON_NPC, IT_MGR::CHAT_TYPE_SYSTEM );
    return false;
  }

  /// 스킬 사용 조건 체크..
  if ( CheckCastingCondition( iSkillIDX, g_pAVATAR, iCurrentTarget ) == false ) {
    g_itMGR.AppendChatMsg( CStr::Printf( "%s", STR_NOTIFY_06 ), IT_MGR::CHAT_TYPE_SYSTEM );
    return false;
  }

  /// 중복 스킬 적용 여부 체크..
  if ( CheckDuplicatable( iSkillIDX, g_pAVATAR, iCurrentTarget ) == false ) {
...
bool CSkillManager::FireSkill(int iSkillSlotIDX, int iTargetObj, D3DXVECTOR3& PosTO) {
  ...
  int iSkillType = GetSkillActionType( pSkill->GetSkillIndex() );
  //----------------------------------------------------------------------------------------------------
  /// 아지트라면 제한된 사용..
  /// 204/12/6   
  ...
  if ( GetSkillTargetType( iSkillType ) != SKILL_TARGET_NONE ) {
    if ( iTargetObj ) {
      if ( g_pObjMGR->m_pOBJECTS[iTargetObj] == nullptr ) {
  ...
  //타켓에 대한 판별
  if ( CheckConditionForFireSkill( pSkill->GetSkillIndex(), iCurrentTarget ) == false )
    return false;
  ...
  if ( CCountry::GetSingleton().IsUseItemDelayNewVersion() ) {
    ...
    if ( CheckSkillTimer( iSkillSlotIDX ) == false ) {
      g_itMGR.AppendChatMsg( CStr::Printf( "%s", STR_NOTIFY_04 ), IT_MGR::CHAT_TYPE_SYSTEM );
      return false;
    }
...
/*static*/
void CSkillManager::UpdateUseProperty(CObjAI* pObjAI, int iSkillIdx) {
  /// 행위 방식에 따라서...
  ...
  for ( short nI        = 0; nI < SKILL_USE_PROPERTY_CNT; nI++ ) {
    ...
    switch ( SKILL_USE_PROPERTY( iSkillIdx, nI ) ) {
      case AT_HP: pObjAI->Set_HP( pObjAI->Get_HP() - SKILL_USE_VALUE( iSkillIdx, nI ) );
        break;
      case AT_MP: pObjAI->Set_MP( pObjAI->Get_MP() - SKILL_USE_VALUE( iSkillIdx, nI ) );
        break;
    }
  }
}
...
///
/* static */
bool CSkillManager::CheckCastingCondition(int iSkillIDX, CObjCHAR* pCaster, int iTargetObjIDX) {
  ...
  if ( CheckNeedWeapon( iSkillIDX, pCaster ) == false ) {
...
/// 내껏만 체크하지 남의것 해줄일없지???
/*static*/
bool          CSkillManager::CheckSkillTimer(int iActiveSkillSlot) {
  CSkillSlot* pSkillSlot = g_pAVATAR->GetSkillSlot();
...
/*static*/
bool CSkillManager::CheckCastingTarget(int iSkillIDX, CObjCHAR* pCaster, int iTargetObjIDX) {
  if ( pCaster == nullptr )
    return false;

  CObjCHAR* pTargetCHAR = g_pObjMGR->Get_CharOBJ( iTargetObjIDX, true );

  int iSkillTargetType = GetSkillTargetType( iSkillIDX );

  switch ( iSkillTargetType ) {
    case SKILL_TARGET_NONE: return true;
    ...
    case SKILL_TARGET_GROUND: return true;

    case SKILL_TARGET_PB: return true;

    case SKILL_TARGET_SCREEN: return true;

    case SKILL_TARGET_ZONE: return true;

    case SKILL_TARGET_SERVER: return true;
...
bool CSkillManager::CheckCastingTargetFilter(int iSkillIDX, CObjCHAR* pCaster, int iTargetObjIDX) {
  ...
  switch ( iTargetFilterType ) {
    case SKILL_TARGET_FILTER_SELF: return true;
...
/// 필요장비 체크..
/*static*/
bool   CSkillManager::CheckNeedWeapon(int iSkillIDX, CObjCHAR* pCaster) {
  bool bResult = true;
...
/// 중복적용 가능 체크...
/*static*/
bool CSkillManager::CheckDuplicatable(int iSkillIDX, CObjCHAR* pCaster, int iTargetObjIDX) {
  if ( pCaster == nullptr )
    return false;
...
/// 배울때 필요하다..
/*static*/
bool CSkillManager::CheckJobForStudy(int iSkillIDX) {
...
/*static*/
bool   CSkillManager::CheckProSkillForStudy(int iSkillIDX) {
  bool bResult = true;
...
/*static*/
bool   CSkillManager::CheckPropertyForStudy(int iSkillIDX) {
  bool bResult = true;
...
///
/*static*/
CObjCHAR* CSkillManager::GetSkillTarget(int iServerObjIDX, int iSkillIDX) {
  if ( iSkillIDX == 0 )
