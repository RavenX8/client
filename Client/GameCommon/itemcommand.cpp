...
bool CTCmdItemEquipInInventory::Exec(CTObject* pObj) {
  ...
  switch ( Item.GetTYPE() ) {
    case ITEM_TYPE_FACE_ITEM: nEquipIDX = EQUIP_IDX_FACE_ITEM;
      break;
    case ITEM_TYPE_HELMET: nEquipIDX = EQUIP_IDX_HELMET;
      break;
    case ITEM_TYPE_ARMOR: nEquipIDX = EQUIP_IDX_ARMOR;
      break;
    case ITEM_TYPE_KNAPSACK: nEquipIDX = EQUIP_IDX_KNAPSACK;
      break;
    case ITEM_TYPE_GAUNTLET: nEquipIDX = EQUIP_IDX_GAUNTLET;
      break;
    case ITEM_TYPE_BOOTS: nEquipIDX = EQUIP_IDX_BOOTS;
      break;
    case ITEM_TYPE_WEAPON: nEquipIDX = EQUIP_IDX_WEAPON_R;
      break;
    case ITEM_TYPE_SUBWPN: nEquipIDX = EQUIP_IDX_WEAPON_L;
      break;
...
bool CTCmdItemUseInInventory::Exec(CTObject* pObj) {
  ...
  switch ( ITEM_TYPE( Item.GetTYPE(), Item.GetItemNO() ) ) {
      ///수리용 아이템
    case USE_ITEM_REPAIR_ITEM: CGame::GetInstance().BeginRepair( CGame::REPAIR_ITEM );
  ...
  if ( CCountry::GetSingleton().IsUseItemDelayNewVersion() ) {
    ...
    if ( USEITME_STATUS_STB( Item.GetItemNO() ) ) {
      switch ( USEITME_STATUS_STB( Item.GetItemNO() ) ) {
        case 1:
        case 2:
        case 3: if ( g_UseItemDelay.GetUseItemDelay( USE_ITEM_HP ) ) {
            ///Use item delay 가 설정되어 있다면 패스..
            g_itMGR.AppendChatMsg( STR_CANT_DOUBLE_USE, IT_MGR::CHAT_TYPE_SYSTEM );
            return false;
          }
        ...
        case 4:
        case 5:
        case 6: if ( g_UseItemDelay.GetUseItemDelay( USE_ITEM_MP ) ) {
            ///Use item delay 가 설정되어 있다면 패스..
            g_itMGR.AppendChatMsg( STR_CANT_DOUBLE_USE, IT_MGR::CHAT_TYPE_SYSTEM );
            return false;
          }
