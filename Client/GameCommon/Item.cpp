...
void    CItem::AddItemEquipRequireCondition(tagITEM& sItem, CInfo& Info) {
  ...
  switch ( sItem.GetTYPE() ) {
    case ITEM_TYPE_USE: break;
    case ITEM_TYPE_RIDE_PART: if ( PAT_ITEM_NEED_ABILITY_IDX( sItem.GetItemNO()) ) {
...
void    CItem::AddItemMoveSpeed(tagITEM& sItem, CInfo& Info) {
  ...
  switch ( nItemType ) {
    ...
    case ITEM_TYPE_BOOTS: pszBuf = CStr::Printf( "[%s %d]",STR_MOVE_SPEED,BOOTS_MOVE_SPEED( nItemIdx ) - iDefaultMoveSpeed );
      Info.AddString( pszBuf );
      break;

    case ITEM_TYPE_RIDE_PART: if ( PAT_ITEM_MOV_SPD( sItem.GetItemNO() ) > 0 ) {
        Info.AddString( CStr::Printf( "%s:%d%%",
                                      CStringManager::GetSingleton().GetAbility( AT_SPEED ), PAT_ITEM_MOV_SPD( sItem.GetItemNO() ) ) );
      }
      break;

    default: Info.AddString( "INVALID ITEM TYPE:MOVE SPEED" );
      break;
  }
}
...
void        CItem::AddItemPrice(tagITEM& sItem, DWORD dwDlgType, DWORD dwType, CInfo& Info) {
  ...
  switch ( dwDlgType ) {
    case DLG_TYPE_ITEM: color = g_dwVIOLET;
      break;
    case DLG_TYPE_STORE: color = g_dwYELLOW;
      break;
    case DLG_TYPE_AVATARSTORE:
    case DLG_TYPE_PRIVATESTORE: color = g_dwYELLOW;
    default: break;
  }
...
int CItem::GetItemRareType(int type, int no) {
  switch ( type ) {
    ...
    case ITEM_TYPE_HELMET:
    case ITEM_TYPE_GAUNTLET:
    case ITEM_TYPE_BOOTS: return (g_pTblSTBs[type]->m_ppDATA[no][g_pTblSTBs[type]->m_nColCnt - 1]);
    default: return 0;
  }
  return 0;
}
