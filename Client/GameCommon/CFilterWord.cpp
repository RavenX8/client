...
bool CFilterWord::CheckString(char* pszString) {
  if ( pszString == nullptr ) return false;
...
bool CFilterWord::IsValidName(char* szName) {
  ...
  if ( /*Filter.*/MulityByte2WideString( szName, wstrName ) ) {
    ...
    switch ( wPrimaryLang ) {
      case LANG_KOREAN: return CheckKorean( wstrName );
      case LANG_JAPANESE: return CheckJapanese( wstrName );
      case LANG_CHINESE: return CheckChinese( wstrName );
      default: {
        return CheckEnglish( wstrName );
      }
    }
...
bool                     CFilterWord::CheckKorean(std::wstring& wstrName) {
  CFilterWord&           Filter = GetInstance();
