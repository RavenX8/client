...
enum enumSKILL_ACTION_TYPE {
  SKILL_BASE_ACTION = 1,
  ///< 기본 명령( 앉기, 서기, 줍기... ), 감정표현
  SKILL_CREATE_WINDOW = 2,
  ///< 창생성( 제조, 파티, 개인상점... )	
  SKILL_ACTION_IMMEDIATE = 3,
  ///< 근접 즉시 발동.( 즉시 모션 교체 )
  SKILL_ACTION_ENFORCE_WEAPON = 4,
  ///< 무기상태 변경( 강화, 효과 연출( 정령탄? ) )
  SKILL_ACTION_ENFORCE_BULLET = 5,
  ///< 강화총알 변경 발사. ( 아이스 애로우.. 실제 화살이 변하는.. )
  SKILL_ACTION_FIRE_BULLET = 6,
  ///< 발사.( 파이어볼 )
  SKILL_ACTION_AREA_TARGET = 7,
  ///< 지역 공격마법( 당근 범위.. )
  SKILL_ACTION_SELF_BOUND_DURATION = 8,
  ///< 자신에게 발동 지속 마법.( 캐스팅 유 ) 능력치 
  SKILL_ACTION_TARGET_BOUND_DURATION = 9,
  ///< 타겟에게 발동 지속 마법.( 캐스팅 유 ) 능력치
  SKILL_ACTION_SELF_BOUND = 10,
  ///< 자신에게 발동 바로 업 마법.( 캐스팅 유 ) 능력치
  SKILL_ACTION_TARGET_BOUND = 11,
  ///< 타겟에게 발동 바로 업 마법.( 캐스팅 유 ) 능력치
  SKILL_ACTION_SELF_STATE_DURATION = 12,
  ///< 자신에게 발동 지속 마법.( 캐스팅 유 ) 상태관련
  SKILL_ACTION_TARGET_STATE_DURATION = 13,
  ///< 상대에게 발동 지속 마법.( 캐스팅 유 ) 상태관련
  SKILL_ACTION_SUMMON_PET = 14,
  ///< 팻 소환 스킬
  SKILL_ACTION_PASSIVE = 15,
  ///< 패시브..
  SKILL_EMOTION_ACTION = 16,
  ///< 감정 표현
  SKILL_ACTION_SELF_DAMAGE = 17,
  ///< 자신주위로 데미지를 주는 스킬..( 이건 셀프로 안되나? ) 물론 범위..
  SKILL_ACTION_WARP = 18,
  ///< 워프 스킬..( 서버에서 사용 )
  SKILL_ACTION_SELF_AND_TARGET = 19,
  ///< 타겟에게 영향을 주는동시에 나에게 영향을 줌
  SKILL_ACTION_RESURRECTION = 20,
  ///< 부활..
};
...
public:
  ...
  //----------------------------------------------------------------------------------------------------
  virtual void Action(CObjCHAR* pAttacker, CObjCHAR* pTarget) = 0 { *(int*)nullptr = 10; };
  virtual void DrawIcon(int     iX, int              iY);
  virtual int  Process();
  virtual bool Execute();
  virtual int  SendSkillActionReq(int iSkillSlot, int iTarget, D3DXVECTOR3& PosTO = D3DXVECTOR3( 0.0f, 0.0f, 0.0f )) = 0 { *(int*)nullptr = 10; };
...
public:
  ...
  */
  void Action(CObjCHAR* pAttacker, CObjCHAR* pTarget) override;
  void DrawIcon(int     iX, int              iY) override;
  int  Process() override;
  int  SendSkillActionReq(int iSkillSlot, int iTarget, D3DXVECTOR3& PosTO = D3DXVECTOR3( 0.0f, 0.0f, 0.0f )) override;
};
