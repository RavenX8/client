...
CAI_OBJ* CObjTARGET::Get_TargetOBJ() {
  return (m_iServerTarget) ? g_pObjMGR->Get_ClientCharOBJ( m_iServerTarget, true ) : nullptr;
}
...
int CObjAI::Cal_AtkAniSPEED(short nRightWeaponItemNO) {
  if ( CCountry::GetSingleton().IsApplyNewVersion() ) {
    /// 100을 기준으로 계산됨.
    float fWeaponSpeed = (1500.f / (WEAPON_ATTACK_SPEED( nRightWeaponItemNO ) + 5));
  ...
  if ( Attack_START( pTarget ) ) {
    ...
    this->Set_MOTION( this->GetANI_Attack(), 0, this->Get_fAttackSPEED(), true );
			...
			if ( m_pCurMOTION->m_nActionPointCNT <= 0 ) {
				char *szMsg = CStr::Printf("%s Attack action frame not set!!!", Get_NAME());
...
char CObjAI::Do_SKILL(int iServerTarget, CObjCHAR* pTarget) {
  ...
  switch ( m_SkillActionState ) {
    ...
    case SKILL_CASTING_LOOP_STATE: if ( ProcSkillCastingLoop( pTarget ) == 5 ) {
        m_SkillActionState = SKILL_ACTION_STATE;
      }
      return 1;

    case SKILL_ACTION_STATE: ProcSkillAction( pTarget );
...
bool CObjAI::SetCMD_TOGGLE(BYTE btTYPE) {
  ...
  if ( btTYPE >= TOGGLE_TYPE_DRIVE ) {
    ...
    switch ( btTYPE ) {
      ...
      case TOGGLE_TYPE_SIT: if ( !this->ToggleSitMODE() ) {
          return false;
        }
        break;
...
void CObjAI::SetCMD_Skill2SELF(short nSkillIDX) {
  ...
  switch ( m_wCommand ) {
    case CMD_SKILL2SELF:
    case CMD_SKILL2OBJ:
    case CMD_SKILL2POS: m_wBeforeCMD = CMD_STOP;
      break;
    default: m_wBeforeCMD = m_wCommand;
      break;
  }
  ...
  if ( pTarget ) {
    ...
    if ( pTarget->IsA( OBJ_NPC ) ) {
      ...
      if ( Goto_TARGET( pTarget, NPC_CLICK_EVENT_RANGE ) ) {
        // 도착했다... 대화 !
        //				g_pNet->Send_cli_CHAR_CLICK(pTarget->Get_INDEX());
