...
enum t_eItemCLASS {
  ...
  // SHOT_TYPE_ARROW 소모하는 무기
  WEAPON_ITEM_USE_ARROW2 = 271,
  // SHOT_TYPE_ARROW 소모하는 무기(한손원거리무기)
  WEAPON_ITEM_USE_BULLET = 232,
  // SHOT_TYPE_BULLET 소모하는 무기
  WEAPON_ITEM_USE_THROW = 233,
  // SHOT_TYPE_THROW 소모하는 무기
...
enum t_eITEM {
  ...
  // 6	LIST_BACK.stb
  ITEM_TYPE_JEWEL,
  // 7	LIST_JEWEL.stb		장신구 : 목걸이 반지

  ITEM_TYPE_WEAPON,
  // 8	LIST_WEAPON.stb		무기
  ITEM_TYPE_SUBWPN,
  // 9	LIST_SUBWPN.stb

  ITEM_TYPE_USE,
  // 10	LIST_USEITEM.stb	소모

  ITEM_TYPE_ETC,
  // 11
  ITEM_TYPE_GEM = ITEM_TYPE_ETC,
  // 11	LIST_JEMITEM.stb	기타 : 보석
  ITEM_TYPE_NATURAL,
  // 12	LIST_NATURAL.stb
  ITEM_TYPE_QUEST,
...
enum t_AbilityINDEX {
  ...
  // 공격력 : 한손	<< AT_PSV_ATK_POW_SWORD 검 에서 한손으로 변경 2004. 2. 26 by icarus
  AT_PSV_ATK_POW_2HAND_WPN = 43,
  //			양손	<< AT_PSV_ATK_POW_AXE	둔기에서 양손을 변경
...
enum eING_TYPE {
  ...
  ING_DEC_AVOID,
  ING_DUMB=22,
  // 유지 : 벙어리 : skill stb 5번 컬럼이 7~14인 스킬 사용 못함.
  ING_SLEEP,
  // 유지 : 수면 : 명령 불능 상태로.. 공격 받으면 풀림
  ING_FAINTING,
  // 유지 : 기절 : 수면과 같으나 공격 받아도 안풀림
  ING_DISGUISE,
  // 유지 : 위장 : 
  ING_TRANSPARENT,
  // 유지 : 투명 :
  ...
  // 즉각 : 모든 상태 해지
  ING_CLEAR_INVISIBLE,
  // 즉각 : 투명상태 해지(ING_DISGUISE,ING_TRANSPARENT) 
