...
CRecvPACKET::~CRecvPACKET() { SAFE_DELETE_ARRAY(m_pRecvPacket); }
...
int CRecvPACKET::Recv_srv_JOIN_SERVER_REPLY() {
  char *szResult[] = {"RESULT_JOIN_SERVER_OK", "RESULT_JOIN_SERVER_FAILED",
...
void CRecvPACKET::Recv_gsv_SET_VAR_REPLY() {
  ...
  case SV_SEX:
    return g_pAVATAR->Set_SEX(iValue);
  case SV_CLASS:
    return g_pAVATAR->Set_JOB(iValue);
  case SV_UNION:
    return g_pAVATAR->Set_UNION(iValue);
  case SV_RANK:
    return g_pAVATAR->Set_RANK(iValue);
  case SV_FAME:
    return g_pAVATAR->Set_FAME(iValue);

  case SV_STR:
    return g_pAVATAR->Set_STR(iValue);
  case SV_DEX:
    return g_pAVATAR->Set_DEX(iValue);
  case SV_INT:
    return g_pAVATAR->Set_INT(iValue);
  case SV_CON:
    return g_pAVATAR->Set_CON(iValue);
  case SV_CHA:
    return g_pAVATAR->Set_CHARM(iValue);
  case SV_SEN:
    return g_pAVATAR->Set_SENSE(iValue);

  case SV_LEVEL:
    return g_pAVATAR->Set_LEVEL(iValue);
  case SV_EXP:
    return g_pAVATAR->Set_EXP(iValue);
  }
}
...
void CRecvPACKET::Recv_gsv_ATTACK_START() { _ASSERT(0); }
      ...
      for (int i = 0; i < 2; i++) {
        if ((0x01 << i) &
          ...
          SKILL_TYPE( iSkillIDX ) == SKILL_ACTION_TARGET_STATE_DURATION )*/
          {
            ...
            default:
              g_itMGR.AppendChatMsg("Unknown property..",
                                    IT_MGR::CHAT_TYPE_SYSTEM);
              break;
            }
          }
...
void CRecvPACKET::Recv_gsv_PARTY_REPLY() {
  ...
  case PARTY_REPLY_ACCEPT_MAKE: ///자신이 파티결성을 요청해서 상대방이
                                ///응답했을경우
  {
    if (g_pAVATAR) {
...
void CRecvPACKET::Set_BILLING_MESSAGE_EXT_KOR_And_PH() {
  ...
  case PAY_FLAG_KR_FAP:
    g_itMGR.AppendChatMsg(STR_PAID_DAYBASED_PLAN_PLATINUM,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    CGame::GetInstance().SetPayType(
        PLAY_FLAG_KOREA_DEFAULT | PLAY_FLAG_EXTRA_CHAR | PLAY_FLAG_EXTRA_STOCK);
    break;
  ...
  case PAY_FLAG_KR_GU:
    ...
    break;
  case PAY_FLAG_KR_GQ:
    g_itMGR.AppendChatMsg(STR_PAID_USER_PLAN_IC_TIME_END,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    CGame::GetInstance().SetPayType(PLAY_FLAG_KOREA_DEFAULT);
    break;
  case PAY_FLAG_KR_IQ:
    g_itMGR.AppendChatMsg(STR_PAID_USER_PLAN_IC_IP_FULL,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    CGame::GetInstance().SetPayType(PLAY_FLAG_KOREA_DEFAULT);
    break;
  }
...
void CRecvPACKET::Set_BILLING_MESSAGE_EXT_Other() {
  ...
  case BILLING_MSG_KICKOUT_CHEAT_DETECT:
    g_itMGR.AppendChatMsg(STR_BILLING_MSG_KICKOUT_CHEAT_DETECT,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    break;
  case BILLING_MSG_KICKOUT_ALREADY_LOGIN:
    g_itMGR.AppendChatMsg(STR_BILLING_MSG_KICKOUT_ALREADY_LOGIN,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    break;
  case BILLING_MSG_KICKOUT_TIME_EXPIRED:
    g_itMGR.AppendChatMsg(STR_BILLING_MSG_KICKOUT_TIME_EXPIRED,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    break;
  case BILLING_MSG_KICKOUT_DUP_LOGIN:
    g_itMGR.AppendChatMsg(STR_BILLING_MSG_KICKOUT_DUP_LOGIN,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    break;
  case BILLING_MSG_KICKOUT_NO_RIGHT:
    g_itMGR.AppendChatMsg(STR_BILLING_MSG_KICKOUT_NO_RIGHT,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    break;
  case BILLING_MSG_KICKOUT_OUT_OF_IP:
    g_itMGR.AppendChatMsg(STR_BILLING_MSG_KICKOUT_OUT_OF_IP,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    break;
  case BILLING_MSG_KICKOUT_EXIST_ACCOUNT:
    g_itMGR.AppendChatMsg(STR_BILLING_MSG_KICKOUT_EXIST_ACCOUNT,
                          IT_MGR::CHAT_TYPE_SYSTEM);
    break;
    ...
    if (m_pRecvPacket->m_gsv_BILLING_MESSAGE_EXT.m_dwPayType == 0 &&
      ...
      if (char *pszRemainTime = Packet_GetStringPtr(m_pRecvPacket, nOffset)) {
        ...
        while (p != NULL) {
          ...
          switch (iType - 1) {
          case BILL_FLAG_JP_BATTLE:
            g_itMGR.AppendChatMsg(CStr::Printf(STR_JP_PAY_REMAIN_TIME_MESSAGE,
                                               STR_JP_PAY_ENTRY,
                                               strTime.c_str()),
                                  IT_MGR::CHAT_TYPE_SYSTEM);
            break;
...
void CRecvPACKET::Recv_gsv_BILLING_MESSAGE() {
  ...
  case BILLING_MSG_PAY_FAP:
    g_itMGR.AppendChatMsg("개인정액(플레티넘) 사용자입니다",
                          IT_MGR::CHAT_TYPE_SYSTEM);
    CGame::GetInstance().SetPayType(
        m_pRecvPacket->m_gsv_BILLING_MESSAGE.m_btTYPE);
    break;
  ...
  case BILLING_MSG_PAY_GU:
    g_itMGR.AppendChatMsg("게임방 사용자로 접속되었습니다",
                          IT_MGR::CHAT_TYPE_SYSTEM);
    CGame::GetInstance().SetPayType(
        m_pRecvPacket->m_gsv_BILLING_MESSAGE.m_btTYPE);
    break;
