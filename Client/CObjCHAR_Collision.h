...
public:
  ...
  enum CollisionType {
    ...
    CT_AVATAR_CART,
    CT_AVATAR_CGEAR,
    // castle_gear for main avatar
    CT_OTHER_CART,
    CT_OTHER_CGEAR,
    // castle_gear for other character
  };

public:
  enum CollisionState {
    CS_NONE = 0,
    // not collided except the terrain
    CS_BODY_FRONT = 1,
    // body front collided
    CS_FOOT_FRONT = 1 << 1,
    // foot front collided
  ...
  struct CollisionSphere {
    ...
    CollisionSphere(CollisionState state_in, D3DXVECTOR3 center_in, float radius_in) : center( center_in ), radius( radius_in ), state( state_in ) {}
  };
