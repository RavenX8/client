...
void CSelectServer::RecvSelectServer(t_PACKET* recvPacket) {
  if ( recvPacket->m_lsv_SELECT_SERVER.m_btResult != RESULT_SELECT_SERVER_OK ) {
    switch ( recvPacket->m_lsv_SELECT_SERVER.m_btResult ) {
      case RESULT_SELECT_SERVER_FAILED: g_EUILobby.ShowMsgBox( STR_RESULT_SELECT_SERVER_FAILED, CTMsgBox::BT_OK, true, GetDialogType() );
        break;
      case RESULT_SELECT_SERVER_FULL: g_EUILobby.ShowMsgBox( STR_RESULT_SELECT_SERVER_FULL, CTMsgBox::BT_OK, true, GetDialogType() );
        break;
      case RESULT_SELECT_SERVER_INVALID_CHANNEL: g_EUILobby.ShowMsgBox( STR_RESULT_SELECT_SERVER_INVALID_CHANNEL, CTMsgBox::BT_OK, true, GetDialogType() );
        break;
      ...
      case RESUTL_SELECT_SERVER_INVALID_AGE: //		0x05	// 연령이 맞지 않는다..
        ...
        break;
      case RESULT_SELECT_SERVER_NEED_CHARGE: g_EUILobby.ShowMsgBox( STR_RESULT_SELECT_SERVER_NEED_CHARGE, CTMsgBox::BT_OK, true, GetDialogType() );
        break;
      default: break;
    }
    m_bWaitServerReply = false;
...
void CSelectServer::OnLButtonUp(unsigned iProcID) {
  switch ( iProcID ) {
    case IID_BTN_OK: SendSelectChannel();
      break;
    case IID_BTN_CANCEL: g_pCApp->SetExitGame();
      break;
    default: break;
  }
}
...
void CSelectServer::MsgBoxProc(unsigned int iMsgType, short int iButton) {
  switch ( iMsgType ) {
    case MSG_TYPE_EXIT: g_pCApp->SetExitGame();
      break;
    case MSG_TYPE_NORMAL: g_EUILobby.HideMsgBox();
      break;
    default: break;
  }
}
...
void        CSelectServer::SendSelectChannel() {
  ...
  if ( pCtrl && pCtrl->GetControlType() == CTRL_LISTBOX ) {
    ...
    if ( iChannelNo ) {
      ...
      g_EUILobby.ShowMsgBox( STR_RECEIVE_CHARACTER_LIST, CTMsgBox::BT_CANCEL, true, GetDialogType(), nullptr, pCmd );
...
void CSelectServer::Update(CObservable* pObservable, CTObject* pObj) {
  ...
  if ( pObj == nullptr || strcmp( pObj->toString(), "SVR_LIST" ) ) {
