...
UINT   CLogin::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if ( uiMsg == WM_KEYDOWN ) {
    switch ( wParam ) {
      case VK_TAB: {
        CTEditBox* pEditBoxID  = nullptr;
        CTEditBox* pEditBoxPwd = nullptr;
...
void CLogin::OnLButtonUp(unsigned iProcID) {
  switch ( iProcID ) {
    case IID_BTN_OK: ConnectLoginServer();
      break;
    case IID_BTN_CANCEL: g_EUILobby.m_bLoginSuccess = false;
      g_pCApp->SetExitGame();
      break;
    default: break;
  }
...
void CLogin::OnLButtonDown(unsigned iProcID) {
  switch ( iProcID ) {
    case IID_CHECKBOX_SAVE_LASTCONECTID: if ( CWinCtrl* pCtrl = Find( IID_CHECKBOX_SAVE_LASTCONECTID ) ) {
        if ( pCtrl && pCtrl->GetControlType() == CTRL_CHECKBOX ) {
          CTCheckBox* p = (CTCheckBox*)pCtrl;
          if ( p->IsCheck() ) {
