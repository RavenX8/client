...
bool CCreateAvata::RecvCreateAvata(t_PACKET* recvPacket) {
  switch ( recvPacket->m_wsv_CREATE_CHAR.m_btResult ) {
    case RESULT_CREATE_CHAR_OK: CGame::GetInstance().ChangeState( CGame::GS_SELECTAVATAR );
      return true;
    case RESULT_CREATE_CHAR_FAILED: g_EUILobby.ShowMsgBox( CStr::Printf( "%s(%d)",STR_FAIL_MAKE_NEW_CHARACTER, 1 ), CMsgBox::BT_OK, true, 0 );
      break;
    case RESULT_CREATE_CHAR_DUP_NAME: g_EUILobby.ShowMsgBox( STR_RESULT_CREATE_CHAR_DUP_NAME, CMsgBox::BT_OK, true, 0 );
      break;
    case RESULT_CREATE_CHAR_INVALID_VALUE: g_EUILobby.ShowMsgBox( CStr::Printf( "%s(%d)",STR_FAIL_MAKE_NEW_CHARACTER, 2 ), CMsgBox::BT_OK, true, 0 );
      break;
    case RESULT_CREATE_CHAR_NO_MORE_SLOT: g_EUILobby.ShowMsgBox( STR_CANT_CREATE_MORE_CHARACTER, CMsgBox::BT_OK, true, 0 );
      break;
    case RESULT_CREATE_CHAR_BLOCKED: g_EUILobby.ShowMsgBox( "현재 서버에서는 캐릭터 생성이 불가능합니다. 자세한 내용은 홈페이지를 참고해주세요.", CMsgBox::BT_OK, true, 0 );
      break;
    default: g_EUILobby.ShowMsgBox( STR_FAIL_MAKE_NEW_CHARACTER, CMsgBox::BT_OK, true, 0 );
      break;
  }
  return false;
}
...
void CCreateAvata::OnLButtonUp(unsigned iProcID) {
  switch ( iProcID ) {
    case IID_BTN_OK: {
      ...
      if ( pCtrl == nullptr || pCtrl->GetControlType() != CTRL_EDITBOX ) {
    ...
    break;
    case IID_BTN_CANCEL: CGame::GetInstance().ChangeState( CGame::GS_SELECTAVATAR );

      //		g_EUILobby.CreateServerListDlg();
      break;
    case IID_BTN_LEFT_SEX: --m_iSelectedSex;
      if ( m_iSelectedSex < 0 )
      ...
      break;
    case IID_BTN_LEFT_FACE: --m_iSelectedFace;
      ...
      break;
    case IID_BTN_LEFT_HAIR: --m_iSelectedHair;
      ...
      break;
    case IID_BTN_LEFT_STARTPOS: --m_iSelectedStartPos;
      if ( m_iSelectedStartPos < 0 )
        m_iSelectedStartPos = m_mapStartPos.size() - 1;
      break;
    case IID_BTN_LEFT_BIRTHSTONE: --m_iSelectedBirthStone;
...
void CCreateAvata::CreateAvata(char* strName, int iRace, int iSex, int iBoneStone, int iHair, int iFace, int iStartPos) {
  if ( strName == nullptr || strcmp( strName, "" ) == 0 )
    return;
...
void CCreateAvata::ClearSelectItem(int iType) {
  switch ( iType ) {
    case 0: m_mapSex.clear();
      break;
    case 1: m_mapFace.clear();
      break;
    case 2: m_mapHair.clear();
      break;
    case 3: m_mapStartPos.clear();
      break;
    case 4: m_mapBirthStone.clear();
      break;
    default: break;
  }
}
...
void CCreateAvata::AddSelectItem(int iType, const char* pszIdentify, const char* pszValue) {
  ...
  if ( pszIdentify == nullptr || pszValue == nullptr ) return;
  ...
  switch ( iType ) {
    case 0: m_mapSex.push_back( Item );
      break;
    case 1: m_mapFace.push_back( Item );
      break;
    case 2: m_mapHair.push_back( Item );
      break;
    case 3: m_mapStartPos.push_back( Item );
      break;
    case 4: m_mapBirthStone.push_back( Item );
      break;
    default: break;
  }
}
