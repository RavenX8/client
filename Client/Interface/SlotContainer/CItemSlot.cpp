...
CItem*   CItemSlot::CreateItem(int iInvenIdx, tagITEM& Item) {
  ...
  if ( iInvenIdx >= 0 && iInvenIdx < MAX_EQUIP_IDX ) {
    ...
    switch ( Item.GetTYPE() ) {
      ...
      case ITEM_TYPE_SUBWPN: pItem->SetCommand( m_pCmdItemEquipInInventory );
        break;
      case ITEM_TYPE_USE: pItem->SetCommand( m_pCmdItemUseInInventory );
        break;
      case ITEM_TYPE_ETC:
      case ITEM_TYPE_NATURAL: pItem->SetCommand( m_pCmdItemEtcInInventory );
        break;
      case ITEM_TYPE_RIDE_PART: pItem->SetCommand( m_pCmdItemPatInInventory );
        break;
      default:
...
CItem*   CItemSlot::AddItem(int iInvenIdx, tagITEM& Item) {
  ...
  if ( m_listItems[iInvenIdx] == nullptr ) {
