...
CEnduranceProperty* CEndurancePack::GetEntity(int Index) {
  if ( Index >= ING_MAX_STATUS || m_EntityList == nullptr )
    return nullptr;
...
bool CEndurancePack::AddEntity(int iEntityIDX, int iStatusSTBNO, int iEnduranceTime, int iEntityType) {
  if ( m_pObjCHAR == nullptr || m_pObjCHAR->Get_HP() <= 0 ) {
  ...
  CEnduranceProperty* pEnduranceProperty = nullptr;

  switch ( iEntityType ) {
    case ENDURANCE_TYPE_SKILL: pEnduranceProperty = new CEnduranceSkill();
      if ( pEnduranceProperty == nullptr )
        return false;
    ...
    case ENDURANCE_TYPE_USEITEM: pEnduranceProperty = new CEnduranceItem();
      if ( pEnduranceProperty == nullptr )
        return false;
  ...
  if ( iStatusSTBNO != 0 ) {
    if ( ING_MAX_STATUS > STATE_TYPE( iStatusSTBNO ) ) {
      ...
      if ( m_EntityList[STATE_TYPE( iStatusSTBNO )] ) {
        ...
        m_EntityList[STATE_TYPE( iStatusSTBNO )] = nullptr;
      } else {
...
void CEndurancePack::Update() {
  ...
  CEnduranceProperty* pEntity = nullptr;
  for ( int           i       = 0; i < ING_MAX_STATUS; i++ ) {
...
void CEndurancePack::SetStateValue(int iType, int iValue) {
  switch ( iType ) {
    ...
    default: m_CurrentStateValue[iType] = iValue;
      break;
  }
}
...
void CEndurancePack::Draw() {
  ...
  for ( int           i        = 0; i < ING_MAX_STATUS; i++ ) {
    ...
    if ( pEntity ) {
      ...
      if ( m_pObjCHAR->IsA( OBJ_USER ) ) {
        ...
        rcIcon.left   = /*ENDU_ICON_X*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().x + (m_iDrawIconCnt * (ENDU_ICON_WIDTH + 2));
        rcIcon.top    = /*ENDU_ICON_Y*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().y + g_itMGR.FindDlg( DLG_TYPE_INFO )->GetHeight();
        ...
        if ( PtInRect( &rcIcon, ptMouse ) ) {
          ...
          switch ( iMinutes ) {
            case 0: break;
            case 1: cTimeRemaining = CStr::Printf( "%i minute and ", iMinutes );
              break;
            default: cTimeRemaining = CStr::Printf( "%i minutes and ", iMinutes );
              break;
          }

          switch ( iSeconds ) {
            case 1: cTimeRemaining = CStr::Printf( "%s%i second remaining.", cTimeRemaining, iSeconds );
              break;
            default: cTimeRemaining = CStr::Printf( "%s%i seconds remaining.", cTimeRemaining, iSeconds );
              break;
          }
          /*if(iMinutes == 0)
            cTimeRemaining = CStr::Printf("%i seconds", iSeconds);
          else
            cTimeRemaining = CStr::Printf("%i minutes, %i seconds", iMinutes, iSeconds);*/
          Tooltip.Clear();
  ...
  if ( m_pObjCHAR->IsA( OBJ_USER ) ) {
    ...
    POINT ptDraw    = { /*ENDU_ICON_X*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().x, /*ENDU_ICON_Y*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().y + g_itMGR.FindDlg( DLG_TYPE_INFO )->GetHeight() + 65 };
    RECT  rcHittest = { /*ENDU_ICON_X*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().x, /*ENDU_ICON_Y*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().y + g_itMGR.FindDlg( DLG_TYPE_INFO )->GetHeight() + 65, /*ENDU_ICON_X*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().x + 100, /*ENDU_ICON_Y*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().y + g_itMGR.FindDlg( DLG_TYPE_INFO )->GetHeight() + 65 + 20 };
    ...
    if ( iMax > 0 && iValue > 0 ) ///Summon Guage
    {
      ...
      POINT ptMouse;
      CGame::GetInstance().Get_MousePos( ptMouse );
      if ( PtInRect( &rcHittest, ptMouse ) && CTDialog::GetProcessMouseOverDialog() == nullptr )
    ...
    ptDraw.x             = /*ENDU_ICON_X*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().x;
    ptDraw.y             = /*ENDU_ICON_Y*/g_itMGR.FindDlg( DLG_TYPE_INFO )->GetPosition().y + g_itMGR.FindDlg( DLG_TYPE_INFO )->GetHeight() + 25;
    CItemSlot* pItemSlot = g_pAVATAR->GetItemSlot();
    assert( pItemSlot );
    CItem* pItem = nullptr;
...
void CEndurancePack::DeleteEntityByStateType(int iStateType) {
  if ( m_EntityList[iStateType] ) {
    ...
    m_EntityList[iStateType] = nullptr;
    ...
    //if( std::find( m_CurrentStateValue.begin(), m_CurrentStateValue.end(), iStateType ) == m_CurrentStateValue.end() )
    {
      switch ( iStateType ) {
        ...
        break;
        default: m_CurrentStateValue[iStateType] = 0;
          break;
      }
    }
  }
...
void    CEndurancePack::DeleteEnduranceState(DWORD dwNewState) {
  ...
  for ( int i = 0; i < sizeof( DWORD ) * 8; i++ ) {
    ...
    if ( m_StateFlagTable.find( (dwTemp << i) ) != m_StateFlagTable.end() ) {
      iStateType = m_StateFlagTable[(dwTemp << i)];
...
bool CEnduranceSkill::Proc(CObjCHAR* pObjCHAR) {
  ...
  ///*
  if ( (m_pObjCHAR && m_pObjCHAR->IsA( OBJ_AVATAR ) && (((CObjAVT*)m_pObjCHAR)->GetSpecialStateFLAG() & FLAG_SUB_HIDE)) || !g_ClientStorage.GetDisplyEffects() ) {
    //ClearEffectResource();
    DeleteEffect();
  } else {
    if ( !m_pEffect ) {
      int iEnduranceEffect = STATE_STEP_EFFECT( this->GetStateSTBNO() );
      CreateEffect( m_pObjCHAR, iEnduranceEffect );
    }
  }
  //*/
...
void CEnduranceItem::ClearEffectResource() {
  if ( m_pEffect ) {
    ///g_pEffectLIST->Del_EFFECT( m_pEffect, true );
    delete m_pEffect;
    m_pEffect = nullptr;
...
bool  CEnduranceItem::Proc(CObjCHAR* pObjCHAR) {
  ...
  if ( (iCurrentTime - m_iStartTime) >= m_iEnduranceTime ) {
    for ( int i = 0; i < STATE_APPLY_ABILITY_CNT; i++ ) {
      ...
      switch ( STATE_TYPE( iSTBLineNO ) ) {
          /// hp
        case ING_INC_HP: pObjCHAR->Add_HP( iApplyProperty );
          ...
          /// mp
        case ING_INC_MP: pObjCHAR->Add_MP( iApplyProperty );
          ...
          /// poison
        case ING_POISONED: pObjCHAR->Add_HP( iApplyProperty );
          break;
      }
  ...
  for ( int i      = 0; i < STATE_APPLY_ABILITY_CNT; i++ ) {
    ...
    switch ( STATE_TYPE( iSTBLineNO ) ) {
        /// hp
      case ING_INC_HP: pObjCHAR->Add_HP( iApplyProperty - m_iTotalAppliedProperty[i] );
        if ( pObjCHAR->Get_HP() >= pObjCHAR->Get_MaxHP() ) {
          pObjCHAR->Set_HP( pObjCHAR->Get_MaxHP() );
          //m_bExpired = true;
          //return false;
        ...
        /// mp
      case ING_INC_MP: pObjCHAR->Add_MP( iApplyProperty - m_iTotalAppliedProperty[i] );
        if ( pObjCHAR->Get_MP() >= pObjCHAR->Get_MaxMP() ) {
          pObjCHAR->Set_MP( pObjCHAR->Get_MaxMP() );
          //m_bExpired = true;
          //return false;
        ...
        /// poison
      case ING_POISONED: pObjCHAR->Add_HP( iApplyProperty - m_iTotalAppliedProperty[i] );
