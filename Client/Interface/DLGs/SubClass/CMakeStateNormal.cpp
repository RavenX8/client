...
void CMakeStateNormal::OnLButtonUp(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
  switch ( iProcID ) {
    case CMakeDLG::IID_BTN_CLOSE: m_pParent->Hide();
      return;
    case CMakeDLG::IID_BTN_START: {
      if ( int iRet = CManufacture::GetInstance().IsValidSendMakeItemReq() ) {
        if ( iRet ) {
          switch ( iRet ) {
            case 1: g_itMGR.OpenMsgBox( STR_NOTSELECT_MAKEITEM );
              break;
            case 2: g_itMGR.OpenMsgBox( STR_NOT_ENOUGH_INVENTORY_SPACE );
              break;
            case 3: g_itMGR.OpenMsgBox( STR_NOT_ENOUGH_MANA );
              break;
            case 4: g_itMGR.OpenMsgBox( STR_NOT_EXIST_MATERIAL );
              break;
            case 5: g_itMGR.OpenMsgBox( STR_NOT_ENOUGH_MATERIAL );
              break;
            default: break;
          }
        }
