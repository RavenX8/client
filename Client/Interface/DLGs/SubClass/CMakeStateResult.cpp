    ...
    if ( m_listGuage[i]->Update( ptMouse, m_dwPrevTime, dwCurrTime ) == CGuage::UPDATE_END ) {
      if ( m_pParent->m_btRESULT == RESULT_CREATE_ITEM_SUCCESS ) {
        if ( i == CManufacture::GetInstance().GetMaterialCount() - 1 ) ///맨밑의 게이지 갱신이 끝까지 갔다
        {
          ...
          //}
          g_itMGR.OpenMsgBox( STR_SUCCESS_MAKE_ITEM, CMsgBox::BT_OK, true, m_pParent->GetDialogType(), pCmd, nullptr );
        ...
        if ( i == m_pParent->m_nStepORInvIDX ) {
          ...
          CTCmdChangeState* pCmd = new CTCmdChangeState( CMakeDLG::STATE_NORMAL );
          g_itMGR.OpenMsgBox( STR_FAIL_MAKE_ITEM, CMsgBox::BT_OK, true, m_pParent->GetDialogType(), pCmd, nullptr );
  ...
  */
  ...
  switch ( m_pParent->m_btRESULT ) {
    ...
    case RESULT_CREATE_ITEM_INVALID_CONDITION: // 마나가 부족하다.
    {
      m_bWaitState           = true;
      CTCmdChangeState* pCmd = new CTCmdChangeState( CMakeDLG::STATE_NORMAL );
      g_itMGR.OpenMsgBox( STR_NOT_ENOUGH_MANA, CMsgBox::BT_OK, true, m_pParent->GetDialogType(), pCmd, nullptr );
      break;
    }
    case RESULT_CREATE_ITEM_NEED_ITEM: // 소모될 아이템이 부족하다
    {
      m_bWaitState           = true;
      CTCmdChangeState* pCmd = new CTCmdChangeState( CMakeDLG::STATE_NORMAL );
      g_itMGR.OpenMsgBox( STR_NOT_ENOUGH_MATERIAL, CMsgBox::BT_OK, true, m_pParent->GetDialogType(), pCmd, nullptr );
    ...
    case RESULT_CREATE_ITEM_INVALID_ITEM: // 소모될 아이템이 부적합하다
    {
      m_bWaitState           = true;
      CTCmdChangeState* pCmd = new CTCmdChangeState( CMakeDLG::STATE_NORMAL );
      g_itMGR.OpenMsgBox( STR_NOT_EXIST_MATERIAL, CMsgBox::BT_OK, true, m_pParent->GetDialogType(), pCmd, nullptr );
      CManufacture::GetInstance().SubItemsAfterRecvResult( m_pParent->m_nStepORInvIDX );
      break;
    }
    case RESULT_CREATE_ITEM_NEED_SKILL_LEV: // 제조 스킬 레벨이 부족하다
    {
      m_bWaitState           = true;
      CTCmdChangeState* pCmd = new CTCmdChangeState( CMakeDLG::STATE_NORMAL );
      g_itMGR.OpenMsgBox( STR_NOT_ENOUGH_MAKE_SKILL_LEVEL, CMsgBox::BT_OK, true, m_pParent->GetDialogType(), pCmd, nullptr );
      break;
    }
    default: {
      m_bWaitState           = true;
