...
unsigned CFriendListItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  switch ( uiMsg ) {
    case WM_LBUTTONDOWN: SetSelected();
      return GetControlID();
    case WM_LBUTTONDBLCLK: {
      switch ( m_btStatus ) {
        case FRIEND_STATUS_DELETED: g_itMGR.OpenMsgBox( CStr::Printf( F_STR_FRIEND_DELETED, m_strName.c_str() ) );
          g_itMGR.OpenDialog( DLG_TYPE_MEMO, false );
          break;
        case FRIEND_STATUS_REFUSED: g_itMGR.OpenMsgBox( CStr::Printf( F_STR_MESSANGER_BLOCKED, m_strName.c_str() ) );
          break;
        default: {
