...
void CUpgradeDlgStateResult::Leave() {
  ...
  switch ( btResult ) {
    case CRAFT_UPGRADE_SUCCESS: CUpgrade::GetInstance().RemoveTargetItem();
      CUpgrade::GetInstance().ApplyResultItemSet();
      //g_itMGR.OpenMsgBox(STR_CRAFT_UPGRADE_SUCCESS);///해당 아이템을 보여주자.
      break;
    case CRAFT_UPGRADE_FAILED: CUpgrade::GetInstance().RemoveTargetItem();
      CUpgrade::GetInstance().ApplyResultItemSet();
      //g_itMGR.OpenMsgBox(STR_CRAFT_UPGRADE_FAILED);
      break;
    case CRAFT_UPGRADE_INVALID_MAT: g_itMGR.OpenMsgBox( STR_CRAFT_UPGRADE_INVALID_MAT );
      break;
    default: break;
  }
}
...
void CUpgradeDlgStateResult::Enter() {
  ...
  switch ( btResult ) {
    ...
    case CRAFT_UPGRADE_INVALID_MAT: m_pParent->ChangeState( CUpgradeDlg::STATE_NORMAL );
      break;
    default: break;
  }
...
void CUpgradeDlgStateResult::Update(POINT ptMouse) {
  ...
  if ( m_pResultGuage->Update( ptMouse, m_dwPrevTime, dwCurrTime ) == CGuage::UPDATE_END ) {
    ...
    switch ( btResult ) {
      ...
      default: m_pParent->ChangeState( CUpgradeDlg::STATE_NORMAL );
        break;
    }
  }
