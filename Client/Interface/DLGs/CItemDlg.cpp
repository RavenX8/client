...
CItemDlg::CItemDlg(int iType) {
  ...
  m_pEquipDragItem->AddTarget( CDragItem::TARGET_ALL, nullptr ); ///
...
void CItemDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  switch ( uiProcID ) {
    case IID_BTN_MAXIMIZE: Maximize();
      break;
    case IID_BTN_MINIMIZE: Minimize();
      break;
...
void CItemDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  switch ( uiProcID ) {
    ...
    case IID_BTN_INVEN_EQUIP: m_iInventoryTab = 0;
      for ( int iSlot                         = 0; iSlot < INVENTORY_PAGE_SIZE; ++iSlot )
        m_ItemSlots[m_iInventoryTab][iSlot].MoveWindow( m_sPosition );

      break;
    case IID_BTN_INVEN_USE: m_iInventoryTab = 1;
      for ( int iSlot                       = 0; iSlot < INVENTORY_PAGE_SIZE; ++iSlot )
        m_ItemSlots[m_iInventoryTab][iSlot].MoveWindow( m_sPosition );

      break;
    case IID_BTN_INVEN_ETC: m_iInventoryTab = 2;
      for ( int iSlot                       = 0; iSlot < INVENTORY_PAGE_SIZE; ++iSlot )
        m_ItemSlots[m_iInventoryTab][iSlot].MoveWindow( m_sPosition );
  ...
  switch ( pEvent->GetID() ) {
    case CTEventItem::EID_ADD_ITEM: {
      ...
      if ( iIndex >= 1 && iIndex < MAX_EQUIP_IDX )
        ...
        for ( int iSlot           = 0; iSlot < INVENTORY_PAGE_SIZE; ++iSlot ) {
          if ( m_ItemSlots[iType][iSlot].GetIcon() == nullptr ) {
