...
unsigned int CBankDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if ( iProcID      = CTDialog::Process( uiMsg, wParam, lParam ) ) {
    ...
    switch ( uiMsg ) {
      case WM_LBUTTONUP: OnLButtonUp( iProcID, wParam, lParam );
        break;
      case WM_LBUTTONDOWN: OnLButtonDown( iProcID, wParam, lParam );
        break;
      default: break;
    }
    return uiMsg;
...
void CBankDlg::OnLButtonDown(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
  switch ( iProcID ) {
    case IID_BTN_TAB1: m_iCurrBankPage = 0;
      break;
    case IID_BTN_TAB2: m_iCurrBankPage = 1;
      break;
    case IID_BTN_TAB3: m_iCurrBankPage = 2;
      break;
    case IID_BTN_TAB4: m_iCurrBankPage = 3;
      break;
    default: break;
  }
}
...
void CBankDlg::OnLButtonUp(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
  switch ( iProcID ) {
    case IID_BTN_SAVE: g_itMGR.PostMsg2Dlg( DLG_TYPE_BANKWINDOW, CBankWindowDlg::MSG_SETYPE, CBankWindowDlg::TYPE_SAVE, 0 );
      g_itMGR.OpenDialog( DLG_TYPE_BANKWINDOW );
      break;
    case IID_BTN_WITHDRAW: g_itMGR.PostMsg2Dlg( DLG_TYPE_BANKWINDOW, CBankWindowDlg::MSG_SETYPE, CBankWindowDlg::TYPE_WITHDRAW, 0 );
      g_itMGR.OpenDialog( DLG_TYPE_BANKWINDOW );
      break;
...
void CBankDlg::Update(CObservable* pObservable, CTObject* pObj) {
  ...
  if ( Item.IsEmpty() ) ///삭제
  {
    if ( m_Slots[iPage][iSlot].GetIcon() == nullptr ) {
...
bool        CBankDlg::IsFullFirstTab() {
  for ( int i = 0; i < g_iSlotCountPerPage; ++i ) {
    if ( nullptr == m_Slots[0][i].GetIcon() )
      return false;
  }
  return true;
}
