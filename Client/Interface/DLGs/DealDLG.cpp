...
CDealDLG::CDealDLG(int iType) {
  m_pCmdRemoveItemFromBuyList  = nullptr;
  m_pCmdRemoveItemFromSellList = nullptr;
  ...
  if ( pObj && strcmp( pObj->toString(), "CTEventDeal" ) == 0 ) {
    ...
    switch ( pEvent->GetID() ) {
      ...
      case CTEventDeal::EID_ADD_SELLITEM: {
        ...
        for ( int iSlot = 0; iSlot < TOTAL_DEAL_INVENTORY; ++iSlot ) {
          if ( m_Slots[DEAL_SELL][iSlot].GetIcon() == nullptr ) {
            pIcon = pFragmentItem->CreateItemIcon();
...
bool CDealDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
  switch ( iProcID ) {
    case DEAL_BTN_CLOSE: g_itMGR.Close_store();
      return true;
    case DEAL_BTN_OK: CDealData::GetInstance().SendTradeReq();
      return true;
    default: break;
  }
