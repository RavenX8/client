...
CAvatarStoreDlg::CAvatarStoreDlg(int iDlgType) {
  ...
  for ( int i      = 0; i < c_iAvatarStoreMaxSlotCount; ++i ) {
    m_SellItems[i] = nullptr;
    m_BuyItems[i]  = nullptr;
  }
...
void CAvatarStoreDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  switch ( uiProcID ) {
    case IID_BTN_SELL: m_iTab = 0;
      break;
    case IID_BTN_BUY: m_iTab = 1;
      break;
    default: break;
  }
}
...
void CAvatarStoreDlg::Update(POINT ptMouse) {
  ...
  if ( CTDialog* pDlg = GetProcessMouseOverDialog() ) {
...
int CAvatarStoreDlg::IsBuyItem(tagITEM& Item) {
  ...
  CItem* pItem;
  m_BuyItems[c_iAvatarStoreMaxSlotCount];
  for ( int iSlot = 0; iSlot < c_iAvatarStoreMaxSlotCount; ++iSlot ) {
    pItem         = m_BuyItems[iSlot];
  ...
  {
    switch ( iErrCode ) {
      case ERR_OPTION: g_itMGR.OpenMsgBox( STR_AVATARSTORE_CANT_SELL_OPTION );
        break;
      case ERR_UPGRADE: g_itMGR.OpenMsgBox( STR_AVATARSTORE_CANT_SELL_UPGRADE );
        break;
      case ERR_DURABILITY: g_itMGR.OpenMsgBox( STR_AVATARSTORE_CANT_SELL_DURABILITY );
        break;
      case ERR_SOCKET: g_itMGR.OpenMsgBox( STR_AVATARSTORE_CANT_SELL_SOCKET );
        break;
      default: break;
    }
  }
