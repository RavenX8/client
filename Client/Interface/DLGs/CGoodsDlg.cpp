        ...
        case IID_BTN_CONFIRM: {
          ...
          if ( iUnitPrice > 0 && iQuantity > 0 && pIcon ) {
            ...
            switch ( m_iType ) {
              ...
              case ADD_BUYLIST: CPrivateStore::GetInstance().AddItemBuyList( pItemIcon->GetIndex(), iUnitPrice, iQuantity );
                break;
              default: break;
            }
          }
...
void CGoodsDlg::Show() {
  ...
  if ( pCtrl && pCtrl->GetControlType() == CTRL_EDITBOX ) {
    ...
    if ( pIcon != nullptr && m_iType == ADD_SELLLIST ) {
      CIconItem* pItemIcon = (CIconItem*)pIcon;
      tagITEM&   Item      = pItemIcon->GetItem();
  ...
  switch ( m_iType ) {
    case ADD_SELLLIST: ShowChild( IID_TEXT_SELL );
      HideChild( IID_TEXT_BUY );
      break;
    case ADD_BUYLIST: ShowChild( IID_TEXT_BUY );
