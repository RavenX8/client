...
void CBankWindowDlg::PostMsg(unsigned msg, unsigned param1, unsigned param2) {
  switch ( msg ) {
    case MSG_SETYPE: {
      ...
      //m_Title.set
      switch ( m_Type ) {
        case TYPE_SAVE: m_Title.set_string( STR_QUERY_BANK_SAVE, rc, 0 );
          break;
        case TYPE_WITHDRAW: m_Title.set_string( STR_QUERY_BANK_WITHDRAW, rc, 0 );
          break;
        default: break;
      }
    }
          ...
          if ( zuly > 0 ) {
            switch ( m_Type ) {
              case TYPE_SAVE: if ( zuly <= g_pAVATAR->Get_MONEY() )
                ...
                break;
              case TYPE_WITHDRAW: if ( zuly <= CBank::GetInstance().GetMoney() )
