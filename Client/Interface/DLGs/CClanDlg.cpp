...
void CClanDlg::Draw() {
  ...
  switch ( m_iTab ) {
    case TAB_INFO: CClanMarkView::Draw( g_pAVATAR, vCharPos );
      ...
      break;
    case TAB_MEMBER: CClanMarkView::Draw( g_pAVATAR, vCharPos );
      DrawMembers();
      break;
    case TAB_SKILL: if ( !CCountry::GetSingleton().IsApplyNewVersion() ) {
        CClanMarkView::Draw( g_pAVATAR, vCharPos );
        DrawSkills();
      }
      break;
    case TAB_NOTICE: CClanMarkView::Draw( g_pAVATAR, vCharPos );
      DrawNotice();
      break;
    default: break;
  }
...
void CClanDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  switch ( uiProcID ) {
    ...
    case IID_BTN_TAB_INFO: m_iTab = TAB_INFO;
      break;
    case IID_BTN_TAB_MEMBER: m_iTab = TAB_MEMBER;
      break;
    case IID_BTN_TAB_SKILL: m_iTab = TAB_SKILL;
      break;
    case IID_BTN_TAB_NOTICE: m_iTab = TAB_NOTICE;
      break;
    default: break;
  }
}
...
void CClanDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  switch ( uiProcID ) {
    case IID_BTN_PREVIEW: {
      ...
      HANDLE hBmpFile = CreateFile( CClanMarkUserDefined::NewClanMarkFileName.c_str(), GENERIC_READ,FILE_SHARE_READ, nullptr,OPEN_EXISTING,FILE_ATTRIBUTE_READONLY, nullptr );
    ...
    break;
    case IID_BTN_REGIST_CLANMARK: if ( CClan::GetInstance().GetClanNo() ) {
        if ( CClan::GetInstance().GetClass() >= CClan::CLAN_MASTER ) {
          ...
          g_itMGR.OpenMsgBox( CStr::Printf( "%s %s",STR_QUERY_CHANGE_CLANMARK,STR_CLANMARK_UPDATE_ERROR ), CMsgBox::BT_OK | CMsgBox::BT_CANCEL, true, 0, pCmdOk, nullptr );
        } else
    ...
    case IID_BTN_WITHDRAWAL: {
      ...
      if ( Clan.GetClanNo() && !g_pTerrain->IsPVPZone() ) {
        if ( CClan::GetInstance().GetClass() < CClan::CLAN_MASTER ) {
          ///클랜 마스터가 아닐경우에만
          g_pNet->Send_cli_CLAN_COMMAND( GCMD_QUIT, nullptr );
...
void CClanDlg::Update(CObservable* pObservable, CTObject* pObj) {
  ...
  if ( pObservable == nullptr || pObj == nullptr ) return;
...
void                  CClanDlg::RemoveClanSkillListItem(CZListBox* pListBox, int iSkillSlotIndex) {
  CClanSkillListItem* pItem      = nullptr;
  CWinCtrl*           pCtrl      = nullptr;
  CIconSkillClan*     pSkillIcon = nullptr;
...
void                                                     CClanDlg::SortMemberlistByClass() {
  ...
  CClanMemberItem* pItem    = nullptr;
  CClanMemberItem* pNewItem = nullptr;
  CWinCtrl*        pCtrl    = Find( IID_TABBEDPANE );
  if ( pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE ) {
    CTabbedPane* pPane = (CTabbedPane*)pCtrl;
    pCtrl              = FindCtrlInTabbedPane( pPane, TAB_MEMBER, IID_ZLIST_MEMBER );
