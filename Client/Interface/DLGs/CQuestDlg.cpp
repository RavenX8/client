...
unsigned int CQuestDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if ( iProcID     = CTDialog::Process( uiMsg, wParam, lParam ) ) {
    switch ( uiMsg ) {
      case WM_LBUTTONUP: if ( On_LButtonUP( iProcID ) )
          return uiMsg;
        break;
      case WM_LBUTTONDOWN: if ( On_LButtonDN( iProcID ) )
          return uiMsg;
        break;
    }
    return uiMsg;
...
void CQuestDlg::Update(POINT ptMouse) {
  ...
  ///이하 툴팁 표시
  CTDialog* pTDlg = GetProcessMouseOverDialog();
...
void CQuestDlg::UpdateQuestDescription() {
  ...
  assert( pAbandonBtn );
  if ( pAbandonBtn == nullptr ) return;

  CWinCtrl* pCtrl = FindChildInPane( IID_PANE_QUESTINFO, IID_LIST_QUESTINFO );
  if ( pCtrl == nullptr || pCtrl->GetControlType() != CTRL_LISTBOX ) {
