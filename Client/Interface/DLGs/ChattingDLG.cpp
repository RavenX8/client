...
unsigned int CChatDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if ( iProcID     = CTDialog::Process( uiMsg, wParam, lParam ) ) {
    switch ( uiMsg ) {
      case WM_MOUSEWHEEL: return uiMsg;
        break;
    ...
    case WM_KEYDOWN: switch ( wParam ) {
        case VK_SPACE: {
          ...
          if ( CTControlMgr::GetInstance()->GetKeyboardInputType() == CTControlMgr::INPUTTYPE_NORMAL
               && nullptr == CTEditBox::s_pFocusEdit
...
bool CChatDLG::On_LButtonUP(unsigned iProcID) {
  switch ( iProcID ) {
    ...
    break;
    case IID_BTN_FILTER: g_itMGR.OpenDialog( DLG_TYPE_CHATFILTER );
      break;
...
void CChatDLG::SendChatMsg(char* szMsg) {
  ...
  switch ( iChatType ) {
    case CHAT_ALLIED: if ( g_pAVATAR && g_pAVATAR->Get_TeamNO() != 2 ) ///Defalt Team No일경우에는 동맹쳇을 보내지 않는다
        g_pNet->Send_cli_ALLIED_SHOUT( (char*)stRealMsg.c_str() );
      break;
    case CHAT_OPEN_HELP: g_itMGR.OpenDialog( DLG_TYPE_HELP );
      break;
    case CHAT_CLAN: {
    ...
    break;
    case CHAT_NORMAL: g_pNet->Send_cli_CHAT( (char*)stRealMsg.c_str() );
      break;
    case CHAT_SHOUT: if ( m_bCheckShoutRestrict && (dwCurrentTime - m_dwPrevUseShoutTime) < m_dwShoutRestrictTime ) {
    ...
    default: g_pNet->Send_cli_CHAT( (char*)stRealMsg.c_str() );
      break;
  }
}
...
int CChatDLG::GetActiveListBox() {
  switch ( m_iSelectedTab ) {
    case TAB_ALL: return IID_LISTBOX_ALL;
    case TAB_WHISPER: return IID_LISTBOX_WHISPER;
    case TAB_TRADE: return IID_LISTBOX_TRADE;
    case TAB_PARTY: return IID_LISTBOX_PARTY;
    case TAB_CLAN: return IID_LISTBOX_CLAN;
    case TAB_ALLIED: return IID_LISTBOX_ALLIED;
    default: break;
  }
  return 0;
}
...
int CChatDLG::GetActiveScrollBar() {
  switch ( m_iSelectedTab ) {
    case TAB_ALL: return IID_SCROLLBAR_ALL;
    case TAB_WHISPER: return IID_SCROLLBAR_WHISPER;
    case TAB_TRADE: return IID_SCROLLBAR_TRADE;
    case TAB_PARTY: return IID_SCROLLBAR_PARTY;
    case TAB_CLAN: return IID_SCROLLBAR_CLAN;
    case TAB_ALLIED: return IID_SCROLLBAR_ALLIED;
    default: break;
  }
  return 0;
}
...
void CChatDLG::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
  switch ( uiProcID ) {
    case IID_BTN_ALL: TabSelected( TAB_ALL );
      if ( CWinCtrl* pCtrl = Find( IID_EDITBOX ) ) {
        CTEditBox*   pBox  = (CTEditBox*)pCtrl;
        pBox->clear_text();
      }
      ActiveListBoxMoveEnd();
      break;
    case IID_BTN_WHISPER: TabSelected( TAB_WHISPER );
      if ( CWinCtrl* pCtrl = Find( IID_EDITBOX ) ) {
        CTEditBox*   pBox  = (CTEditBox*)pCtrl;
        pBox->clear_text();
        pBox->AppendText( "@" );
      ...
      ActiveListBoxMoveEnd();
      break;
    case IID_BTN_TRADE: TabSelected( TAB_TRADE );
      if ( CWinCtrl* pCtrl = Find( IID_EDITBOX ) ) {
        CTEditBox*   pBox  = (CTEditBox*)pCtrl;
        pBox->clear_text();
        pBox->AppendText( "$" );
      ...
      ActiveListBoxMoveEnd();
      break;
    case IID_BTN_PARTY: TabSelected( TAB_PARTY );
      if ( CWinCtrl* pCtrl = Find( IID_EDITBOX ) ) {
        CTEditBox*   pBox  = (CTEditBox*)pCtrl;
        pBox->clear_text();
        pBox->AppendText( "#" );
      ...
      ActiveListBoxMoveEnd();
      break;
    case IID_BTN_CLAN: TabSelected( TAB_CLAN );
      if ( CWinCtrl* pCtrl = Find( IID_EDITBOX ) ) {
        CTEditBox*   pBox  = (CTEditBox*)pCtrl;
        pBox->clear_text();
        pBox->AppendText( "&" );
      ...
      if ( e->GetWParam() == VK_RETURN ) {
        ...
        if ( pCtrl && pCtrl->GetControlType() == CTRL_EDITBOX ) {
          ...
          if ( CTEditBox::s_pFocusEdit && CTEditBox::s_pFocusEdit == pEditBox && pEditBox->get_text() ) {
            ...
            switch ( m_iSelectedTab ) {
              case TAB_WHISPER: if ( pEditBox->get_text() == nullptr )
                  pEditBox->AppendText( "@" );
                else if ( strlen( pEditBox->get_text() ) < 1 )
                  pEditBox->AppendText( "@" );
                break;
              case TAB_TRADE: if ( pEditBox->get_text() == nullptr )
                  pEditBox->AppendText( "$" );
                else if ( strlen( pEditBox->get_text() ) < 1 )
                  pEditBox->AppendText( "$" );
                break;
              case TAB_PARTY: if ( pEditBox->get_text() == nullptr )
                  pEditBox->AppendText( "#" );
                else if ( strlen( pEditBox->get_text() ) < 1 )
                  pEditBox->AppendText( "#" );
                break;
              case TAB_CLAN: if ( pEditBox->get_text() == nullptr )
                  pEditBox->AppendText( "&" );
                else if ( strlen( pEditBox->get_text() ) < 1 )
                  pEditBox->AppendText( "&" );
                break;
              case TAB_ALLIED: if ( pEditBox->get_text() == nullptr )
                  pEditBox->AppendText( "~" );
                else if ( strlen( pEditBox->get_text() ) < 1 )
                  pEditBox->AppendText( "~" );
                break;
