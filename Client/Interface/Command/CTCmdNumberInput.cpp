...
bool CTCmdDropItem::Exec(CTObject* pObj) {
  ...
  if ( lPrice >= 1000 ) {
    ...
    g_itMGR.OpenMsgBox(
      ...
      CMsgBox::BT_OK | CMsgBox::BT_CANCEL, true, 0, pCmd, nullptr, 0
    );
  } else {
...
bool CTCmdBuyItem::Exec(CTObject* pObj) {
  ...
  g_pNet->Send_cli_STORE_TRADE_REQ( g_pObjMGR->Get_ServerObjectIndex( CStore::GetInstance().GetNpcObjIndex() ), 1, 0, &BuyItem, nullptr );
  return true;
}
...
bool CTCmdSellItem::Exec(CTObject* pObj) {
  ...
  if ( strcmp( pObj->toString(), "CIcon" ) == 0 ) {
    ...
    tagITEM& Item = ((CIconItem*)pObj)->GetItem();
    if ( Item.IsEnableSELL() )
      g_pNet->Send_cli_STORE_TRADE_REQ( g_pObjMGR->Get_ServerObjectIndex( CStore::GetInstance().GetNpcObjIndex() ), 0, 1, nullptr, &SellItem );
    else
      g_itMGR.AppendChatMsg( STR_DONT_SELL_ITEM, IT_MGR::CHAT_TYPE_SYSTEM );
