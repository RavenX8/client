...
public:
  ...
  enum {
    /// 채팅 입력 방식 타입
    INPUTTYPE_NORMAL,
    /// 채팅시마다 Enter를 입력하고 다시 엔터를 입력시 채팅모드는 종료된다.
    INPUTTYPE_AUTOENTER,
    /// Show시에 해당 Dialog에 EditBox가 있다면 자동 입력된다.Enter후 자동으로 다시 Focus를 가진다.
  };
  ...
  enum {
    ...
    /// 아이템 사용시 사용되는 대화창
    QUERYTYPE_SELECT,
    /// 선택문만이 나열되어 유저가 선택하는 대화창( 루나마을의 워프게이트 )
  };
  ...
  void OpenMsgBox(const char*  szMsg,
                  ...
                  unsigned int iInvokerDlgID = 0, CTCommand*        pCmdOk = nullptr, CTCommand* pCmdCancel = nullptr,
                  int          iMsgType      = 0
  );
