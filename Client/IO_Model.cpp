...
bool   CPointPART::Load(CFileSystem* pFileSystem) {
  ...
  while ( btTAG ) {
    ...
    switch ( btTAG ) {
      ...
      case SWITCH_POS: pFileSystem->Read( &m_Transform, sizeof( D3DVECTOR ) );
        break;

      case SWITCH_ROT: pFileSystem->ReadFloat4( &m_Rotate.w, &m_Rotate.x, &m_Rotate.y, &m_Rotate.z );
        break;

      case SWITCH_SCALE: pFileSystem->Read( &m_Scale, sizeof( D3DVECTOR ) );
        break;

      default: pFileSystem->Seek( btLen, FILE_POS_CUR );
        break;
    }
...
bool   CCharPART::Load(CFileSystem* pFileSystem, short nLinkBoneNo, short nLinkDummyNo) {
  ...
  while ( btTAG ) {
    ...
    switch ( btTAG ) {
      case SWITCH_BONEIDX: pFileSystem->ReadInt16( &m_nLinkBoneIDX );
        break;

      case SWITCH_DUMMYIDX: pFileSystem->ReadInt16( &m_nLinkDummyIDX );
        break;

      default: if ( btTAG >= SWITCH_ANI && btTAG < SWITCH_ANI + MAX_MESH_ANI_TYPE ) {
...
bool   CFixedPART::Load(CFileSystem* pFileSystem, short nLinkBoneNo, short nLinkDummyNo) {
  ...
  while ( btTAG ) {
    ...
    switch ( btTAG ) {
      ...
      case SWITCH_ROTAXIS: m_wAniTYPE |= BIT_ROTAXIS;
