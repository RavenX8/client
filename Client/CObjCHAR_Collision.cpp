...
//--------------------------------------------------------------------------------
{
  ...
  if ( pObj->GetPetMode() >= 0 ) {
    // if is on the PET, then ignore UpdateHeight
    return;
  }
  ...
  switch ( iObjTYPE ) {
    ...
    case OBJ_MOB: AdjustHeight_Monster();
      break;
    case OBJ_AVATAR: AdjustHeight_Other();
      break;
    case OBJ_USER: if ( g_GameDATA.m_bFilmingMode )
...
//--------------------------------------------------------------------------------
{
  ...
  switch ( m_State ) {
    case CS_BODY_FRONT:
    case CS_FOOT_FRONT: CollisionResponseBody_Cart();
      setModelNormal( m_hNodeModel, D3DXVECTOR3( 0, 0, 1 ) );
      break;
    case CS_FOOT: CollisionResponseFoot_Cart();
      setModelNormal( m_hNodeModel, D3DXVECTOR3( 0, 0, 1 ) );
      break;
    case CS_NONE: CollisionResponseNone_Cart();
...
//--------------------------------------------------------------------------------
{
  ...
  switch ( m_State ) {
    case CS_BODY_FRONT:
    case CS_FOOT_FRONT: CollisionResponseBody_Cgear();
      //setModelNormal( m_hNodeModel, D3DXVECTOR3(0, 0, 1) );
      break;
    case CS_FOOT: CollisionResponseFoot_Cgear();
      //setModelNormal( m_hNodeModel, D3DXVECTOR3(0, 0, 1) );		
      break;
    case CS_NONE: CollisionResponseNone_Cgear();
      //ApplyTerrainNormal( m_vNext.x, m_vNext.y );
      break;
  }
}
...
//--------------------------------------------------------------------------------
{
  ...
  switch ( m_State ) {
    case CS_BODY_FRONT:
    case CS_FOOT_FRONT: CollisionResponseBody_Avatar();
      break;
    case CS_FOOT: CollisionResponseFoot_Avatar();
      break;
    case CS_NONE: CollisionResponseNone_Avatar();
      break;
  }
...
//--------------------------------------------------------------------------------
{
  ...
  switch ( m_State ) {
    case CS_FOOT: CollisionResponseFoot_Other();
      break;
    case CS_NONE: CollisionResponseNone_Other();
      break;
  }
}
...
//--------------------------------------------------------------------------------
{
  if ( !bIsAvatar ) {
    ...
    if ( m_pObj->GetResetPosZ( fResetPosZ ) ) {
      // 높이값이 갱신되어야 할 타이밍인가?
...
//--------------------------------------------------------------------------------
{
  ...
  m_Spheres.push_back( CollisionSphere( CS_FOOT, m_vFoot, AVATAR_FOOT_RADIUS ) );
  m_Spheres.push_back( CollisionSphere( CS_BODY_FRONT, m_vBodyFront, AVATAR_BODY_FRONT_RADIUS ) );
  m_Spheres.push_back( CollisionSphere( CS_FOOT_FRONT, m_vFootFront, AVATAR_FOOT_FRONT_RADIUS ) );
}
...
//--------------------------------------------------------------------------------
{
  ...
  m_Spheres.push_back( CollisionSphere( CS_FOOT, m_vFoot, CART_FOOT_RADIUS ) );
  m_Spheres.push_back( CollisionSphere( CS_BODY_FRONT, m_vBodyFront, CART_BODY_FRONT_RADIUS ) );
  m_Spheres.push_back( CollisionSphere( CS_FOOT_FRONT, m_vFootFront, CART_FOOT_FRONT_RADIUS ) );
}
...
//--------------------------------------------------------------------------------
{
  ...
  m_Spheres.push_back( CollisionSphere( CS_FOOT, m_vFoot, CGEAR_FOOT_RADIUS ) );
  m_Spheres.push_back( CollisionSphere( CS_BODY_FRONT, m_vBodyFront, CGEAR_BODY_FRONT_RADIUS ) );
  m_Spheres.push_back( CollisionSphere( CS_FOOT_FRONT, m_vFootFront, CGEAR_FOOT_FRONT_RADIUS ) );
}
    ...
    for ( std::vector<HNODE>::iterator itnode = m_ContactNodes.begin(); itnode != m_ContactNodes.end(); ++itnode ) {
      ...
      if ( sp->state == CS_BODY_FRONT ) {
        ...
        if ( intersectNodeTriSphereMoving( hNode, sp->center, sp->radius, prevCenter, closestCenter, MAX_DISTANCE_SQUARE ) ) {
          ...
          m_State = CS_BODY_FRONT;
          //LogString(LOG_DEBUG, "CS_BODY_FRONT\n");
          return;
        }
      } else if ( sp->state == CS_FOOT_FRONT ) {
        if ( isFalling ) continue;
...
//--------------------------------------------------------------------------------
{
  ...
  for ( std::vector<CollisionSphere>::iterator it = m_Spheres.begin(); it != m_Spheres.end(); ++it ) {
    ...
    if ( sp->state == CS_FOOT_FRONT && SphereResetOnOff )
