...
public :
  ...
  virtual bool IsInViewFrustum() = 0 { *(int*)nullptr               = 10; };
  virtual bool IsIntersect(float& fCurDistance) =0 { *(int*)nullptr = 10; };
  /// 카메라와의 사이에 있는 오브젝트는 다른 처리 필요.. 
  virtual bool IsIntersectForCamera(float& fCurDis) { return false; };
  /// 씬에 오브젝트 삽입.
  virtual void InsertToScene(void) =0 { *(int*)nullptr = 10; };
  /// 씬으로부터 오브젝트 삭제
  virtual void RemoveFromScene(bool bIncludeEFFECT = true) =0 { *(int*)nullptr = 10; };

  virtual int   Get_TYPE() =0 { *(int*)nullptr = 10; };
  virtual char* Get_NAME() { return nullptr; }
