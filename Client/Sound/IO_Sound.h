...
struct tagSndFILE {
  ...
  short        m_nMixCNT;
  short        m_nRefCNT;
               tagSndFILE() { m_pSoundData = nullptr, m_nMixCNT = 1, m_nRefCNT = 0; }
};
...
public:
  ...
  bool Load_FILE(tagFileDATA<tagSndFILE*>* pDATA) override;
  void Free_FILE(tagFileDATA<tagSndFILE*>* pDATA) override;
...
public :
  ...
  bool Load(char* szSTBFile, short nFileNameColNO = 0) override;
  ...
  bool KEY_PlaySound3D(t_HASHKEY HashKEY, const D3DXVECTOR3& posWorld, const D3DXVECTOR3* velWorld = nullptr, float fMinDistance = SOUND_DEFAULT_MIN_DISTANCE, float    fMaxDistance = SOUND_DEFAULT_MAX_DISTANCE);
  bool KEY_PlaySound3D(t_HASHKEY HashKEY, const D3DXVECTOR3& posWorld, int                iVolume, int              iPan         = SOUND_PAN_CENTER, const D3DXVECTOR3* velWorld     = nullptr, float fMinDistance = SOUND_DEFAULT_MIN_DISTANCE, float fMaxDistance = SOUND_DEFAULT_MAX_DISTANCE);

  bool IDX_PlaySound(short   nIndex);
  bool IDX_PlaySound3D(short nIndex, const D3DXVECTOR3& posWorld, const D3DXVECTOR3* velWorld = nullptr, float fMinDistance = SOUND_DEFAULT_MIN_DISTANCE, float fMaxDistance = SOUND_DEFAULT_MAX_DISTANCE);

  bool PlaySoundFile(char*   szFileName, short              nMaxMixCNT                 = 1);
  bool PlaySoundFile3D(char* szFileName, const D3DXVECTOR3& posWorld, short nMaxMixCNT = 1, const D3DXVECTOR3* velWorld = nullptr, float fMinDistance = SOUND_DEFAULT_MIN_DISTANCE, float fMaxDistance = SOUND_DEFAULT_MAX_DISTANCE);
