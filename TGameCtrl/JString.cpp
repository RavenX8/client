...
void      CJString::Draw(int x, int y, bool bTransform, DWORD force_text_color) {
  ...
  for ( iter   = m_Letters.begin(); iter != m_Letters.end(); ++iter ) {
    ...
    if ( force_text_color )
      IFont->Draw( iter->GetFont(), true, ptOffset.x, ptOffset.y, force_text_color, iter->GetLetter() );
    else
      IFont->Draw( iter->GetFont(), true, ptOffset.x, ptOffset.y, iter->GetColor(), iter->GetLetter() );
