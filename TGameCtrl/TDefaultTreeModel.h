...
public:
  ...
  ITreeNode* getChild(ITreeNode*       pParent, int&      index) override;
  void       getChildCount(ITreeNode*  pParent, int&      iCount) override;
  int        getIndexOfChild(CTObject* pParent, CTObject* pChild) override;
  ITreeNode* getRoot() override;
  bool       isLeaf(CTObject* pNode) override;
  int        getSelectRow();
