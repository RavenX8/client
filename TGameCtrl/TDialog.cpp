...
CTDialog* CTDialog::m_pProcessMouseOverDialog = nullptr;
...
unsigned int CTDialog::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if ( m_pCaption ) {
    ...
    switch ( iID ) {
      ...
      case CTCaption::IID_BTN_CLOSE: if ( uiMsg == WM_LBUTTONUP )
          Hide();
        return m_pCaption->GetControlID();
      default: if ( m_pCaption->IsClicked() && uiMsg == WM_LBUTTONDOWN ) {
...
void CTDialog::Update(POINT ptMouse) {
  ...
  if ( IsInside( ptMouse.x, ptMouse.y ) ) {
    if ( GetProcessMouseOverDialog() == nullptr )
