...
CTScrollBox::CTScrollBox(void) {
  ...
  //	m_iSelectDiff = 0;
  m_pDraw          = nullptr;
  m_pScrollBoxType = nullptr;
  m_bBlink         = false;
  m_pBlinkImage    = nullptr;
}
...
bool CTScrollBox::Create(int iWidth, int iHeight, int iGraphicID, int iModuleID, int iType) {
  ...
  switch ( iType ) {
    case CTScrollBarType::TSBT_VERTICAL: m_pScrollBoxType = new CTScrollBoxTypeVertical;
      break;
    case CTScrollBarType::TSBT_HORIZONTAL: m_pScrollBoxType = new CTScrollBoxTypeHorizontal;
      break;
    default: m_pScrollBoxType = new CTScrollBoxTypeVertical;
      break;
  }
  return true;
}
...
void CTScrollBox::MoveBoxPosition(int iValue, int iMax, int iExtent) {
  if ( iMax == iExtent || m_pScrollBoxType == nullptr )
    return;
  /*
...
void CTScrollBox::MoveBoxPositionByScreenPoint(POINT pt) {
  if ( m_pScrollBar == nullptr || m_pScrollBoxType == nullptr )
    return;
